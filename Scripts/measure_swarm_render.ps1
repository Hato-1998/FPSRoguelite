...
# CSV 플러시 대기(캡처 자동 종료 시 파일 생성) → 수거 → 종료
$waited = $ShotAtSeconds + 2
$csv = $null
while ($waited -lt $MaxWaitSeconds) {
    if ($proc.HasExited) { Write-Warning "[measure] game exited early (crash?)"; break }
...
if ($csv) {
    Copy-Item $csv.FullName (Join-Path $outDir "capture.csv")
    Write-Host "[measure] CSV: $($csv.Name) -> $outDir\capture.csv"
} else {
    Write-Warning "[measure] no CSV captured"
}
$gameLog = Join-Path $BuildDir "Windows\FPSRoguelite\Saved\Logs\FPSRoguelite.log"
if (Test-Path $gameLog) { Copy-Item $gameLog (Join-Path $outDir "game.log") }
