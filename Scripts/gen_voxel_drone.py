...
#   0  몸통 윗면(+Z 면)            BODY   #3A2748     6  로터 날개 ×4              ROTOR  #8A3A52  ← WPO 회전
#   1  몸통 옆·밑면(어두운 면)     BODY   #2A1E36     7  밑면 라이트 ×4(이미시브)  LIGHT  #FF3B4E
#   2  코어 프레임(눈 테두리)      BODY   #1A1024     8  꼬리 핀                   BODY   #6E2E44
#   3  코어(이미시브)              CORE   #FF3B4E     9~11 예비(엘리트 장식·2번 코어)
#   4  팔 ×4                       BODY   #6E2E44
#   5  로터 허브 ×4                HUB    #2A1E36
#   요소 0/1 은 같은 셀의 **면 방향**으로 갈린다(+Z 면 = 0, 나머지 = 1) — 셀 요소가 아니라 면 요소.
#   ROTOR(6) 정점의 UV.v 타일 = 허브 번호 k(0..3) + [0.01,0.99]: 머티리얼이 floor(UV.v) 로 허브를 알고, 허브 중심 =
#   (±HUB_CM, ±HUB_CM) 를 k 의 비트로 복원한다(k&1 → x 부호, k&2 → y 부호). **정점 위치로 그룹을 가르지 않는다.**
#   WPO: 요소 6 정점을 자기 허브 중심 축(+Z) 둘레로 angle = Time·RotorRate·2π + k·90° 회전(로컬 공간). 나머지 요소 0.
...
PREVIEW_KD = {
    E_TOP: hexc("#3A2748"), E_SIDE: hexc("#2A1E36"), E_FRAME: hexc("#1A1024"), E_CORE: hexc("#FF3B4E"),
    E_ARM: hexc("#6E2E44"), E_HUB: hexc("#2A1E36"), E_ROTOR: hexc("#8A3A52"), E_LIGHT: hexc("#FF3B4E"),
...
def write_obj(path, faces, name, with_mtl=False):
    lines = [f"# FPSR voxel enemy mesh: {name} (generated by gen_voxel_drone.py)",
             f"# ElementId = floor(UV.u); rotor hub = floor(UV.v) for element 6. HUB_CM={HUB_CM:.2f} ROTOR_RATE_TPS={ROTOR_RATE_TPS}"]
    if with_mtl:
    ...
    for (cell, d, elem, hub) in (sorted(faces, key=lambda t: t[2]) if with_mtl else faces):
        ...
        vb = hub if (elem == E_ROTOR and hub >= 0) else 0
        v0, v1 = vb + 0.01, vb + 0.99
        vt.extend([(u0, v0), (u1, v0), (u1, v1), (u0, v1)])
        vn.append(d)
