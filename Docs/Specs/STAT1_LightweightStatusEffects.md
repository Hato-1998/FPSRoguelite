...
> 🔴 **rev4 사후 정정 (2026-09-13, G2 P2-1)**: §6-2 도트 시간 앵커를 **"재생 재개 시각 보존"** 으로 고쳤다 —
> 시간축 처방의 **네 번째** 수정이다(rev4 처방은 직전 직격이 걸어 둔 재생 지연을 지웠다). 원장 = §12 G2.
>
...
🔴 **정정 (G2 P2-1, 2026-09-13) — 위 처방은 직전 직격이 걸어 둔 재생 지연을 지운다.** 도트 틱이 이전 앵커와
무관하게 `Now − Delay` 를 대입하므로, 직격이 실드를 깬 직후(재생은 6초 뒤) 첫 도트 틱이 재생을 **즉시** 재개시킨다
— 재생/틱 ≥ 도트/틱 인 실드 적에게는 지속피해 탄이 순손해가 된다. 레드팀 제안 `Max(이전 앵커, Now − Delay)` 는
그 소거는 막지만 **도트가 실드를 깨는 순간마다 재생을 최대 (파손 지연 − 부분 지연)만큼 더 멈춘다** — 값 앵커가
부분→파손으로 바뀌면서 이전 시간 앵커에 긴 지연이 붙기 때문이다. 직격 직후만이 아니라 **마지막 직격 10초 뒤
도트만으로 깨져도 2.5초** 멈춘다(float32 시뮬레이션, 재생 10/s · 부분 3s · 파손 6s). 그래서 시간 앵커가 아니라
**재생 재개 시각을 보존**한다(사용자 결정 2026-09-13):
```cpp
// FPSRVitals::ComputeRegenTimeAnchor — 🔴 ShieldAtLastDamage 를 덮기 **전에** 부른다(이전 값 앵커를 읽는다)
// Delay(x) = x <= 0 ? BrokenDelay : PartialDelay   (ComputeRegeneratedShield 와 같은 분류)
PreviousResume       = LastDamageCombatTime + Delay(ShieldAtLastDamage); // 이전 앵커 쌍이 약속한 재개 시각
LastDamageCombatTime = Max(PreviousResume, Now) - Delay(Shield);          // 도트: 미루지도 당기지도 않는다
ShieldAtLastDamage   = Shield;
// 직격(bDotRegenAnchorPolicy false)은 종전대로 LastDamageCombatTime = Now
```
= "**새 지연을 걸지 않고, 걸려 있던 지연은 지우지 않으며, 지난 시간을 공짜 회복으로 주지도 않는다.**"
파손 지연을 부분 지연보다 짧게 저작해도 성립한다(Max 안은 그 경우 소거가 일부 되살아나고 실드가 오르내린다).
테스트 = `FPSRoguelite.Combat.Vitals` ⑦(직격 파손 뒤 도트) · ⑧(§10-12) · ⑨(부분→파손 전이 2케이스) — 기존 코드 ·
Max 안 · rev3 동결안은 각각 최소 한 블록에서 실패하고 이 처방만 넷 다 통과한다(시뮬레이션 대조).
...
12. **실드 보유 적에 도트 → 재생 지연을 넘긴 뒤까지 굴려도 실드가 순감소**(§6-2 복리 회복 버그를 잡는
    유일한 테스트 — 지연 안에서 끝나면 통과해 버린다) — 구현 = `FPSRoguelite.Combat.Vitals` ⑧(2026-09-13 G2 P2-1
    수정 때 추가. 순수 함수라 `Status.Unit` 이 아니라 `FPSRVitals` 테스트에 들어갔다) ·
