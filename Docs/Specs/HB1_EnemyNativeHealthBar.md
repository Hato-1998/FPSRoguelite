...
**기전(소스 실측)**: 숨겨진 위젯은 스크린 레이어에서 빠져 슬레이트가 풀리고 컴포넌트가 스스로 틱을 끈다(§3-2). 틱을 되켜는 엔진 경로는 셋이다 — `OnHiddenInGameChanged`(`WidgetComponent.cpp:808-820`) · `OnWidgetVisibilityChanged`(`:1010-1033`) · `SetWindowVisibility`(`:1035-1064`). 뒤 둘은 **위젯 자신의 가시성이 바뀔 때** 도는 것이라 이 시나리오에서는 안 돈다. 앞의 하나는 `SetHiddenInGame` 이 **값을 바꿀 때만** 발화한다(`SceneComponent.cpp:3619`). 즉 **이 재사용 시나리오에서 도달 가능한 경로는 사실상 없다**(G2 P3 — rev3 의 "유일한 경로" 단정을 한정 서술로 정정). 그런데 비사망 재사용 경로에서는 **두 축이 모두 값 무변화**다:
...
> **중복 경고 노이즈(G2 P3) — 스로틀 미채택.** 경고는 `BeginPlay` 경유라 액터 **생애 1회**고(풀 액터는 Deactivate/Activate 로 BeginPlay 에 재진입하지 않는다), 최악은 풀 구축 시 멜리 수 × 1줄이며 **전환기 한정**(사용자 작업 2 이후 0건)이다. 진단이 목적이므로 시끄러운 편이 낫다고 본다 — 전환기가 길어져 거슬리면 그때 클래스 단위 1회 스로틀을 추가한다.
...
> ⚠️ **WBP 저작 계약 1건(G2 P3)** — `WBP_EnemyHealthBar` 의 **루트는 상시 `Visible` 로 유지**한다(자기-숨김 저작 금지). 위젯이 스스로 숨으면 `IsWidgetVisible()` 이 false 가 되어 컴포넌트가 틱을 자기-차단하는데, 그러면 **이후 액터가 휴면해도 스크린 레이어 엔트리를 제거할 틱이 없어** 파킹된 개체의 per-frame 투영이 풀 규모로 잔류한다(표시 자체는 `OnNativeVisibilityChanged` 로 자기치유되므로 비용 문제만). 바를 조건부로 숨기고 싶으면 루트가 아니라 **자식 요소**를 숨겨라. **현행 WBP 가 이 조건을 만족하는지는 미검증**(uasset) — §12-8 PIE 에서 확인한다.
...
| 8 | **PIE (사용자)** | ① 원거리(큐브형)·엘리트에 체력바가 뜬다 ② 멜리에 바가 **둘** 뜨지 않는다 ③ 피격 시 바가 줄고 사망 시 사라진다 ④ LOD1 §12-10 ①③⑥ 무회귀 ⑤ **비사망 드레인(rear-drain) 후 근거리 재사용된 적의 바가 즉시 뜬다** — §6-3 이 닫은 구멍의 확인 검사 ⑥ **풀 재사용 직후 체력바 잔존**(M1 백로그) 이 악화되지 않았는지 ⑦ **`WBP_EnemyHealthBar` 루트가 상시 Visible 인가**(§9 계약, G2 P3) — 자기-숨김 저작이면 휴면 풀의 스크린 레이어 엔트리가 잔류한다 |
