...
| # | 인자 | 답하는 질문 | ASC 틱 | `-MemReport` |
|---|---|---|---|---|
| ① | 300 | 기준선 (ASC 없음) | — | ✅ |
| ② | 300 `-AttachASC` | **ASC 를 붙이는 비용** (①↔②) | **0**(엔진 자기해제) | ✅ |
| ③ | ② `-MeasureLoadout -Cadence 1.0` | GAS 사용 — **하한**(Instant GE 만) (②↔③) | **0** | ✅ |
| ③′ | ② `-MeasureLoadout -Cadence 0.2` | 캐던스 민감도 | 0 | — |
| ③ᵀ | ③ `-MeasureTickable` | GAS 사용 — **상한**(틱 켜짐) (③↔③ᵀ) | **300** | ✅ |
| ④ | 500 · ③ᵀ 구성 | 스트레스 | 500 | — |
| ⑤ | ③ᵀ `-ClientCount 3` | 복제 비용(4인) | 300 | — |
| ⑤′ | ① `-ClientCount 3` | **⑤ 의 대조군** — 없으면 델타를 못 낸다 | — | — |

> **`-MemReport` 를 어디에 붙이나** (G2 가 지적한 명세 갭): **메모리 델타를 낼 네 구성(①②③③ᵀ)에만.**
> 메모리는 ①을 기준으로 빼야 의미가 생기므로 기준선에도 붙인다. ③′·④·⑤·⑤′ 는 프레임·복제가 목적이라
> 붙이지 않는다 — `-MemReport` 는 캡처를 시간 기준으로 닫고 `memreport -full` 을 기다리느라 실행이 길어진다.
> ⚠️ `-MemReport` 를 **안** 준 실행에서도 ASC 부착 게이트는 돈다(러너가 `ASCDump` 를 워밍업 절삭 구간에
> 따로 예약한다) — 게이트를 위해 `-MemReport` 를 줄 필요는 없다.
