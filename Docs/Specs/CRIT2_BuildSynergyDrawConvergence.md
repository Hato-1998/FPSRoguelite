...
1. 치명타 **기능** 카드를 1~2장 가진 상태에서 미션 클리어 오퍼에 **나머지 치명타 기능 카드가 눈에 띄게 자주** 뜨는가.
   ⚠️ **전제(G2 머지 게이트 P3-4)**: 태그가 서로 다른 무기를 **2정 이상 보유**한 상태에서 시험할 것(예: 라이플+SMG).
   라이플만 들고 시험하면 §11-2 현 콘텐츠 한계(라이플 기능 카드가 치명타 5장뿐)상 B 그룹 후보가 전부 같은 태그라
   재분배할 대상이 없다 — 시너지 유무와 무관하게 분포가 똑같아 보여서 "안 된다"로 오판하게 된다.
...
### G1 2회차 재제출 — *(생략)*
### G2 머지 게이트 (2026-09-06, `claude-fable-5`) — **통과** · P1 0 · P2 2 · P3 6

푸시 직전 커밋 범위(`22b8f023`) diff 적대 리뷰. **P1 0 건이므로 푸시 차단 없음**(§6-7).

| 등급 | 지적 | 처리 |
|---|---|---|
| P2-1 | **도메인 가드 부재** — `SynergyMaxStacks` 가 음수면 시너지 계수가 0 이 되어 미션 풀 전 후보의 가중치가 0 → 2단 추출이 **항상 그룹의 첫 원소**를 고른다(오퍼가 매번 같은 카드). `SynergyBonusPerCard` 음수 + 보유 4장이면 가중치 자체가 음수. 형제 `GetEffectiveWeight`(`:724`)는 `FMath::Max(...,0)` 로 막는데 신규 `GetUnlockDrawWeight` 만 원 곱을 그대로 반환했다 | **수용** — `ClampMin` 2개(슬라이더) + `GetUnlockDrawWeight` 클램프(런타임) + 풀 DA `IsDataValid` 음수 에러 2건(직렬화된 에셋·스크립트 쓰기). 3중 |
| P2-2 | **§12-6 등가 판정 자동화 부재** — 헬퍼를 `public static` 으로 연 유일한 이유가 외부 검증이었는데 그 테스트가 없었다 | **수용** — `FPSRCardSynergyTest.cpp` (G) 블록: ⓐ 균등 회귀(포함빈도 → `Count/N`=3/11, ±4σ, M=10000) ⓑ 그룹 몫 보존(B 그룹 일부에 3배 시너지, A 그룹 평균 픽 수 불변, Hypergeometric(11,6,3) 분산으로 ±4σ) |
| P3×6 | Weight 균일 조건 문구(주석 2곳) · 델리게이트가 형제 4개와 다른 패턴(네이티브·비-UFUNCTION → BP 정보창이 구독·조회 불가) · 기능 카드 판정 이원화(효과 클래스 vs Fragment 포인터 — `Fragment` null 에서 갈림) · PIE 스모크 1 의 "무기 2정" 전제 누락 · 어휘 검증기가 카드풀 DA 1개 전제 · `public static` 샘플러의 배열 길이 전제 미강제 | **전부 수용** — 각각 주석 보강 / `DECLARE_DYNAMIC_MULTICAST_DELEGATE`+`BlueprintAssignable`+`BlueprintPure` / `UFPSRCardDataAsset::GetBehaviorFragment()` 단일화 / §12 전제 명시 / 전 풀 `BuildTagVocabulary` 합집합 / 진입부 `ensure` |

**기각 0건.**

**G2 가 독립 검증한 것** — §13 "3회차 생략" 절이 G2 에 넘긴 집중 항목 ①(2단 추출 설계의 참·거짓)에 대해 G2 가 **200,000 회 시뮬레이션**을 돌려 "모든 시너지가 1이면 종래 균등 추출과 분포가 같다"가 **참**임을 확인했다(단, `Card->Weight` 가 균일할 때 — 이 조건이 rev3 문장에 빠져 있던 것이 P3-1 이다).
