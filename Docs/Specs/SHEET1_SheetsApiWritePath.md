...
| 상태 | `완료` — 사용자 지시로 마감(2026-09-13). C2 · C3 검증 · G2(P1 0) · 이관 seed ×4 · 회귀 2종 · U1 결정 · U4 수정 · 에디터 py 실행 대응(I-25). 경위 = `Docs/WorkLog.md` 「SHEET1 — 마감」 · 후속 = 보드 SHEET2(에디터 버튼) · 라이플 상태이상 빌드 |
...
| P3 | apply/seed 오케스트레이션(종료 코드 분기·다중 시트 부분실패·sync 사슬) 오프라인 회귀 0 | **수용·완료** — `T20ApplyOrchestration` 3건(가짜 `SheetsClient`·`_fetch_export`·`_run_sync`): ① 2시트 중 뒤 시트 재조회 불일치 → 3 · 앞 시트만 쓰기 · sync 0회 ② 첫 시트 쓰기 결과 불명(503) → 3 + 키 기준 차이 줄 · 뒤 시트 쓰기 0 ③ 비문자열 값 → 1 · 클라이언트 생성 0 · 잠금 해제 | 118/118 통과 · 변이 대조: 재조회 불일치를 1 로 바꾼 코드에서 ① 실패 · 결과 불명을 1 로 바꾼 코드에서 ② 실패 · 무변이 3/3 통과 |
...
- **G2 뒤 커밋(리뷰 밖)**: G2 지적 반영 커밋 · 이관 산출물 커밋 · 오케스트레이션 테스트(`6c0fa09f`) · 에디터 py 실행 대응(`dd80a4ee`) · U4 데이터(`7be74ae0`) · 마감 문서. Fable 상한(G1 2회 + G2 1회) 도달로 재게이트하지 않았다 — 검증 = 단위테스트(새 테스트 대조군 포함) · 이관 사후 대조 · 회귀 2종.
