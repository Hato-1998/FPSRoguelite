...
| 작성일 / 최종 갱신 | 2026-09-01 / 2026-09-13 |
| 상태 | **`완료`(코드 범위) — 보드 행 마감 2026-09-13**(사용자 판정 「남은 것은 검증이 아니라 새로 만드는 일」). 코드는 2026-09-02 main `29f035d7` `--no-ff` 머지·검증 완료 — C3 가 P1 1건·P2 2건, G2 가 P2 1건(+P3 6건)을 잡아 조건 전건 이행. 🔴 **런타임 검증은 하지 않았다** — §12-10 PIE 10항목은 후속 행 「VIT1 런타임 검증」 소관이고, 이 `완료`는 코드 범위 완료다. 콘텐츠는 §11-1 1·2·6-① 만 끝났고(=실드가 실제로 켜진 상태) 나머지는 후속 행 3개로 분리. 원장 = §13-0(G1) · §13-1(G2) · §13-2(C3 표적) · §13-3(C3 전수) · **§13-4(마감 정적 감사)** |
...
> ⚠️ **후속 저작 주의 2 (G2 P3, 2026-09-02)** — **`MaxShield` 를 0 으로 만드는 효과**("실드 포기" 카드)는 연동 규칙이 `Shield` 를 0 으로 내리므로 §5-5-1 의 **본인 파손 경고를 발화시킨다**. 그 순간 `IsShieldBroken()` 은 `MaxShield > 0` 조건 때문에 **false**(층 자체가 사라짐)인데 경고만 우는 불일치다. 그 카드를 만들 때 핸들러에 `GetMaxShield() > 0` 게이트를 함께 넣을 것. 이 유닛엔 하향 효과가 없어 지금은 도달 불가.
...
> 실행 2026-09-02 (Fable, 코어 갈래 **2회차 = 상한 소진**) · 대상 `d06bd030..HEAD` 35파일 +1887/−103 · 도구 43회 · 16분.
> **판정 = 조건부 통과.** P1 **0건** — 크래시·서버권위 붕괴·복제 불일치·성능 제1원리 위반을 6개 공격축 전부에서 찾지 못했다.

| 심각도 | 지적 (요약 + 파일:줄) | 처리 | 근거 |
|---|---|---|---|
| **P2**<br>(머지 조건) | **적 실드파손 코스메틱이 리슨서버 호스트·스탠드얼론에서 영영 안 난다.** `OnShieldBrokenCosmetic.Broadcast()` 호출처가 `FPSREnemyHealthComponent.cpp:261`(`OnRep_Shield` 안) **한 곳뿐**인데 RepNotify 는 권위 머신에서 안 돈다. 선례 U20 은 **두 반쪽**이다 — 클라 `OnRep_bDead` + 권위 `HandleDeath → HandleDeathCosmetic()` 직접 호출. C2 는 클라 반쪽만 복사했다 | **수용 · 수정** | 실증: `FPSREnemyBase.cpp:418` 주석 원문 *"the host is the one machine that never plays the death state"* + 히트플래시는 `:166-167` 로 **서버·클라 양쪽** 발화. 협동 기준 형태가 리슨서버라 호스트는 매 세션 존재하는 1/4 이고, 솔로는 전무 |
| P3 | `MaxShield → 0` 효과(향후 "실드 포기" 카드)가 본인 파손 **경고**를 발화한다 — 그 순간 `IsShieldBroken()` 은 층이 사라져 false 인데 경고만 운다. 이 유닛엔 그 카드가 없어 도달 불가 | **수용 · 명세 각주**(§5-4-1 후속 저작 주의에 병기) | 카드 저작 시점 함정. 코드 게이트(`GetMaxShield() > 0`)는 그 카드와 함께 |
| P3 | 1회용 힐팩(`RespawnSeconds=0`)이 수집 후 영원히 10Hz 로 틱한다 | **수용 · 후속** | 맵당 소수라 실해 미미. 수정 = 그 분기에서 `SetActorTickEnabled(false)` |
| P3 | 실드 전용 피격에 적 히트플래시가 안 뜬다(§13-3 무명세판단 3번의 딸린 공백) | **수용 · 후속(폴리시)** | 신규 콘텐츠 폴리시이지 기존 회귀가 아니라는 C3 판단에 G2 동의 |
| P3 | 스테이지 전환 중에는 전투시계가 흐른다(힐팩 틱은 멈추는데 리스폰 데드라인·실드 재생은 관통) | **수용 · 문서** | §5-6 계약은 "§2-2 프리즈에서 멈춘다"뿐이라 위반 아님. 시계 의미론을 명시할 가치 |
| P3 | §7 의 *"`MaxShield==0` → 영원히 dirty 안 됨"* 은 과장 — `BeginPlay`/`ResetForReuse`/`InitializeMaxHealth` 가 실드리스 적에도 dirty 를 찍는다 | **수용 · 문서 정밀도** | **대역 0 결론은 유지**(값 무변 = 송신 0). 정확한 문구 = "dirty 마킹은 있으나 셀 값 무변 = 송신 0" |
| P3 | `Health + Shield` 합산 무피격 판정(`FPSRMission_CarryNoHit.cpp:119`)은 실드를 **깎는** 효과가 생기면 가짜 "피격"이 된다 | **수용 · 후속** | §5-4-1 후속 저작 주의와 같은 계열 |
| — | (범위 밖) `FPSRCombatStatics.cpp:152·259` 의 U18a 시절 주석이 낡았다 | **수용 · 수정**(2곳) | 시그니처가 `FFPSRDamageSpec` 으로 바뀌어 오독 여지 |

**§12-9 지정 4건 — G2 명시 판정 (전건 통과)**
1. **프리즈 전투시계 = 성립.** `bRunPaused` 기록자가 `SetRunPaused` 1곳뿐이고 외부 호출자 3곳(`RefreshPauseState:349`·`EndRunFreeze:277`·디버그 `FPSR.Pause:535`)이 전부 이 단일 전이를 경유 — 엣지 누적이 샐 구멍 없음. 누적이 엣지 가드 뒤·값 변경 앞이라 중복 호출이 오염 불가, 프리즈 중 질의식이 열린 구간을 빼 경계에서 연속·단조. 드라이버가 프리즈 게이트를 따로 안 두는 설계는 이 "얼음"이 증명돼야 성립하는데, 증명됐다.
2. **적 복제 3→5 = 상쇄 성립.** G2 가 추가 실측한 두 가지가 논거를 보강 — 해제 시 `DORM_DormantAll`(`FPSREnemyBase.cpp:709`) + 활성 시 Significance LOD별 `NetUpdateFrequency` 하향(`FPSREnemySpawnSubsystem.cpp:570-575`). 패키지 빌드 최악 비용도 300 동시 기준 마이크로초대, 섀도 상태 증가분 = 4클라×300 ≈ 9.6KB.
3. **`DamageDealt` 단위 혼합 = 정의 유지가 옳다.** 대안(흡혈만 SDM 정규화)은 단일 정의를 둘로 쪼개 소비처마다 "어느 진실인가"를 묻게 만든다. 무한 펌프 없음(흡혈 `bWasEnemy` 게이트 + 플레이어 분기 도달 불가 + 구조물/보스는 `InitializeMaxHealth` 가 실드 0). 🔴 **단 이름 붙여 남긴다**: 실드 가진 적은 이 게임 **최초의 재생하는 흡혈원**이고 SDM>1 이 그 수익을 계수배한다 — 수익률이 `ShieldRegenPerSecond × 흡혈계수 × SDM` 으로 전부 저작 데이터에 유계라 코드 결함이 아니라 **밸런스 감시 항목**. §11-8 후속에 병기.
4. **디렉터 센서 기준선 이동 = 머지 차단 아님.** `NotifyPlayerDamageTaken` 의 소비처 실측 = `FPSRDirectorSensorSubsystem.cpp:188` 의 텔레메트리 누적 + EWMA 뿐이고, **이 스냅샷으로 난이도를 행동하는 소비자가 현재 리포에 없다**. 임계값 재보정은 폐루프 디렉터 본체 랜딩 시점 문제(§11-8-2 에 이미 등재).

**C3 수정 3건 — G2 적대 검증 결과**: P1 힐팩 틱 = **수정 충분**(엔진·선례 인용 정확, 클라 틱 차단·늦은 조인 OnRep·리슨호스트 직접 비주얼까지 경로 완결). P2 `bTargetIsPlayer` = **5경로 전부 덮었고 다른 소비처를 잘못 막지 않았다**(`DamageDealt` 독자 전수 grep, `bDealtEnemyDamage`·관통·OnMiss 회계 무변). P2 경고 배선 = **엔진 근거 실측 일치, 3형태 각 1회 성립**(리슨호스트=서버측·원격오너=OnRep·데디=`IsLocallyControlled` 차단), `EndPlay` 해제 + 재바인드 가드로 리스폰 누적 없음. — 다만 **같은 요구의 적측 반쪽이 위 P2 로 뚫려 있었다**(플레이어측은 3형태를 꼼꼼히 따졌는데 적측은 형태 1개를 놓쳤다).

**C2 무명세 판단 12건 재판정**: **전건 유지 동의.** 지목 2건 — 3번(`OnRep_Shield`→`OnHealthChanged` 재발화)은 없으면 실드 전용 피격이 클라 체력바에 영영 안 닿는 **필요악**이고 기존 구독자 전수가 실측으로 닫혀 있다. 5번(`ResetForReuse` 앵커=현재시각)은 `-1e9`(즉시 만충)보다 **안전한 방향의 편차**.

- **레드팀에 무엇을 줬나**: 명세 정본 + 델타 범위 + §12-9 지정 4건 + C3 가 고친 3건(수정 자체의 적대 검증 요구) + C2 무명세 판단 12건(유지 판정 재검토 요구) + 빌드/자동화 실측 + "PIE 는 사용자 몫이니 코드만으로 판정 가능한 결함에 집중" 지시.
- **G1↔G2 사이에 들어간 "플랜에 없던 구조 결정"**(§6-5-2 명시 요구) — **총 15건**: C2 12건(§13-3, 전건 유지) + C3 3건(`FDamageResult::bTargetIsPlayer` 신설 · §5-5-1 경고 배선 신설 · 힐팩 틱 활성화) . G2 는 15건 전부를 검토해 **신규 결함 1건(위 P2)** 을 냈고, 그것도 C2 12건이 아니라 **C2 가 §5-3 을 축자 구현하면서 선례의 반쪽만 복사한 지점**이었다.
- **조건 이행**: `UFPSREnemyHealthComponent::ApplyDamage` 서버 경로에 `Result.bShieldBroke → OnShieldBrokenCosmetic.Broadcast()` 추가(히트플래시의 양측 발화 선례와 동형, OnRep 은 권위에서 안 도니 이중발화 없음) + 헤더 계약 갱신 + U18a 낡은 주석 2곳 정리. 재검증 = 빌드 2회 `Result: Succeeded` · 자동화 3종 `Result={Success}`.
...
## 13-4. 마감 정적 감사 원장 (2026-09-13, Opus · 보드 행 완료 마킹)

사용자가 "PIE 에서 실드는 적용돼 있다"고 확인한 뒤 **§11-1 콘텐츠 7항목의 실제 상태를 리포에서 정적으로 전수 조사**했다(게임 미실행 — `[[do-not-launch-game]]`). 결론: 이 행에 남아 있던 것은 *검증*이 아니라 **없던 콘텐츠를 새로 만드는 일**이었고, 그래서 행을 `완료`로 닫고 남은 일을 후속 행으로 뺐다(§6-9 (5) 「범위 조정으로 남은 일을 신규 행으로 뺀 경우도 완료」).

### 끝난 것

| §11-1 | 항목 | 확인 |
|---|---|---|
| 1 | 적 BP 3종 프로파일 | `DA_Vitals_EnemyMelee`/`EnemyRanged`/`EnemyElite` 저작 + 각 BP 할당 (`776e8441`) |
| 2 | 플레이어 프로파일 | `DA_Vitals_Player` 저작 + `BP_FPSRPlayer` 할당 |
| — | 실드가 켜졌는가 | 프로파일 4종 전부 `MaxShield`·`ShieldRegenPerSecond`·이원 지연이 **기본값에서 덮여 있다**(이름 테이블에 직렬화됨) = 실드 층이 실제 동작. 사용자 PIE 확인과 일치 |
| 3 | 로스터 `VitalsModifier` | 미저작 = 항등 1.0 — 명세가 「등급별 덱 생기기 전까진 무방」으로 허용한 상태 |
| 6-① | HUD 실드바 | `WBP_PlayerVitals`·`WBP_TeammateVitals`·`WBP_EnemyHealthBar` |

### 안 된 것 — §12-10 PIE 4항목이 지금 판정 불가

| §11-1 | 항목 | 정적 근거 | 막히는 PIE |
|---|---|---|---|
| 4 | 무기 `ShieldDamageMultiplier` | **9종 전부 미저작 = 1.0.** 대조군으로 같은 DA 의 `Damage`·`FireRate`·`MagSize`·`ADSFieldOfView` 는 직렬화돼 있음을 확인했다(이 칸만 없다 = 명시 저작 안 됨) | **2** (이월 V3 만으로는 차이가 안 난다) |
| 5 | 힐팩 배치 | `776e8441` 이 `L_Map_1`·`DA_Map_1_ArenaBake` 에 배치했으나 **현재 `BP_HealthPack` 을 참조하는 에셋이 리포에 0건**(.umap 13개 전수). 맵 개편에서 소실. 게다가 그 액터의 메시(`SM_TargetBaseMesh`)가 `838c78c1` 대량 삭제로 끊겨 있다 | **7** |
| 6-②③ | 실드파손 마커·경고 | 마커는 배선까지(`776e8441`) 실측 미확인 / 본인 경고 위젯은 본체 없음(`OnShieldBroken` 구독 위젯 0) | **1 · 5** |
| 7 | `HealthRegen` 카드 풀 제거 | 🔴 **되돌아갔다** — 아래 참조 | — |
| (누락) | `+MaxShield` 카드 | 🔴 **§11-1 목록 자체의 갭** — 아래 참조 | **9** |

### 🔴 새로 잡힌 것 2건

1. **§11-1 7번(요구 3)이 회귀했다.** `DA_Character_CardPool` 에 `DA_Card_Character_HealthRegen` 이 다시 들어가 있다. `776e8441` 이 손으로 뺐고 `e6f8e6ea`(카드 CSV 임포트)가 되살렸다. **원인 = `Content/Authoring/Cards.csv` 의 그 행이 `Route`=전역 레벨업 풀로 남아 있고 임포터가 CSV 를 기준으로 풀 멤버십을 선언적으로 동기화한다**(`FPSRCardCsvImporter.cpp:608` 부근). **CSV 가 정본이므로 에셋만 고치면 다음 임포트에 또 되살아난다** — 이것이 이 결함의 재발 구조다. ⚠️ 행만 지우면 `.uasset` 이 고아로 남아 RoundTrip 이 조용히 빨개진다(카드 CSV 행과 .uasset 은 1:1).

2. **§5-4-1(앵커 재기저)을 깨울 콘텐츠가 0이다.** 그 규칙은 G1 P2-1 로 들어갔고 반례가 *레벨업 프리즈에서 `+MaxShield` 카드를 골랐을 때 증가분이 다음 틱에 증발하는 것* 인데, **그 카드가 존재하지 않는다** — 카드 DA 16장 중 실드 카드 0 · `Cards.csv` 에 실드 행 0 · `MaxShield` 를 만지는 GE 0(`MaxShield` 참조 에셋은 프로파일 4개 + HUD 위젯 3개뿐). **§11-1 의 7항목 목록에 이 카드 저작이 빠져 있었다** — 명세 갭이며, 그래서 §12-10 PIE 9 는 수행 불가이고 §5-4-1 이 살아 있는지 확인할 수단이 없다.

### 판정하지 못한 것 (에디터 육안 필요)

**`DA_Vitals_EnemyMelee` 의 `MaxHealth` 가 프로파일 구조체 기본값 50 그대로인데(직렬화된 값 없음) `BP_EnemyMeleeBase` 는 `MaxHealth` 를 덮고 있다.** 프로파일이 할당되면 BP 값은 무시되므로(`UFPSRVitalsProfileDataAsset::Resolve` — `Profile ? Profile->MaxHealth : FallbackMaxHealth`) BP 값이 50 이 아니었다면 **근접 적 체력이 조용히 바뀐 것** = §11-1 1번 「값은 현행 BP `MaxHealth` 를 그대로 옮기는 것부터(무회귀 기준선)」 위반이다.
- ⚠️ **정적으로 확정할 수 없다** — BP 컴포넌트는 값을 기본값으로 되돌려도 오버라이드 기록이 남을 수 있어, 이름 테이블에 `MaxHealth` 가 있다는 것이 "값이 다르다"를 증명하지 않는다. 에디터에서 두 값을 직접 대조해야 한다.
- 원거리는 **반대 패턴**(BP 미오버라이드 = 50 + DA 오버라이드)이라 의도 확인 필요. 엘리트는 양쪽 저작돼 비교 불가.

### 방법 메모 — 이 감사가 쓴 판정법

`.uasset` 이름 테이블에 프로퍼티 이름이 **없으면 그 값은 기본값**이다(UE 는 아키타입과 같은 값을 태그 프로퍼티 스트림에 쓰지 않는다). 그래서 "저작됐는가"는 이름 유무로 값을 열지 않고 판정할 수 있다 — 단 **대조군이 필수**다(같은 에셋에서 저작된 것이 확실한 다른 필드가 이름 테이블에 보이는지 먼저 확인). `[[uasset-strings-name-table-only]]` 의 한계(핀 배선·인스턴스 실제값은 못 읽음)는 그대로 적용되므로, **값의 크기를 알아야 하는 판정(위 근접 체력)은 이 방법으로 못 한다.**

### 후속 행 (보드)

1. [VIT1 잔여 콘텐츠 저작 — 무기 대실드 계수 · 근접 프로파일 체력 대조 · 힐팩 아레나 재배치](https://app.notion.com/p/3d93972ddd88813db5f4cd016a09e558) — 하이
2. [VIT1 `+최대실드` 카드 신규 — PIE 9 앵커 재기저 검증 선행](https://app.notion.com/p/3d93972ddd8881c99185e4fff7416a6f) — 하이
3. [HealthRegen 카드가 풀에 되살아남 — 요구 3 회귀 (CSV 멤버십 동기화)](https://app.notion.com/p/3d93972ddd888162881dc7f28e0e85e3) — 하이
4. [VIT1 런타임 검증 — PIE 10항목(§12-10) + i-frame 재조정(§11-2)](https://app.notion.com/p/3d93972ddd8881f0b1acc6eb88cdc7df) — 미듐 · 선행 = 위 3개 + 「VIT1 HUD 잔여」

기존 행 「VIT1 HUD 잔여」·「로비 맵 · 힐팩 · 로비 표시 폰 아케이드 재작성」이 나머지를 이미 들고 있어 중복 생성하지 않았다. 백로그 3행(「periodic GE × 전역 프리즈」 = §11-3 · 「VIT1 G2 P3 잔여 6건」 = §13-1 · 「디렉터 IncomingDamageRate 재보정」 = §11-8)도 그대로 둔다.
