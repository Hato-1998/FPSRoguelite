...
| **맵 승격** | `L_GameFloor` → **`L_Map_Center`로 리네임 후 맵1 베이스로 승격**. `RunMap`을 여기로 전환하고 **`L_Sandbox`는 제거**. ⚠️ 단순 리네임이 아니다 — §3 A-3 참조 |

> ⚠️ **`L_` 접두사**: 기존 맵 6개가 전부 `L_` 접두사를 쓰므로 `L_Map_Center`로 적었다. 접두사 없이 정확히 `Map_Center`를 원하면 **리네임 실행 전에** 바로잡을 것(리네임은 되돌리기 어려운 조작 — §5-5).
...
- `Config/DefaultGame.ini:51-57` `MapsToCook`에 **`L_RunPersistent`·`L_MapA`·`L_MapB` 3개가 등재돼 있는데 디스크에 없다**(맵은 6개뿐). → **§3 A-3 4단계에서 삭제**(사문 확정).
...
**A-3. 맵 승격: `L_GameFloor` → `L_Map_Center`, `L_Sandbox` 제거** ← ⚠️ 아래 순서 엄수

🚨 **이건 리네임이 아니라 "게임플레이 레이어 이식"이다.** 두 맵의 내용이 **상호 보완적**이라 그냥 바꿔치면 런이 깨진다. 실측:

| 액터 | `L_Sandbox` | `L_GameFloor` |
|---|---|---|
| **`AFPSRSpawnRoom`**(룸 점진 개방) | **25** | **0** |
| **`AFPSRMissionSpawnPoint`** | **8** | **0** |
| **`AFPSRDoor`** / SpawnZone 볼륨 | 1 / 2 | **0 / 0** |
| `BP_Enemy` 배치 | 6 | 0 |
| `AFPSREnemySpawnPoint` | 2 | 28 |
| `PlayerStart` | 5 | 14 |
| `FlowFieldBoundsVolume` | 1 | 4 |
| SRS 액터 / PostProcessVolume | **0** | 18 / 4 |

→ `L_GameFloor`에는 **룸·미션·문 레이어가 통째로 없다.** 그대로 승격하면 **미션이 스폰될 자리가 없고**(`AFPSRMissionActor`가 태그 매칭으로 스폰 지점을 찾는다) **룸 점진 개방이 사라진다**(밸런스 2차 `d285c69`의 핵심 산출물).

**순서:**
1. **리네임** — 에디터에서 `L_GameFloor` → `L_Map_Center`. 리다이렉터를 정리(Fix Up Redirectors)한다. 리네임 **직전에 커밋**해 롤백 포인트를 박을 것(§5-5).
2. **게임플레이 레이어 이식** — `L_Sandbox`의 룸 25 · 미션 스폰 8 · 문 · 스폰존 볼륨을 `L_Map_Center`의 새 지형에 맞게 **재배치**한다. 좌표 복사가 아니라 **새 레이아웃에 맞는 재저작**이다(지형이 다르다).
3. **`RunMap` 전환** — `Config/DefaultGame.ini:37` `RunMap=/Game/Maps/L_Sandbox.L_Sandbox` → `L_Map_Center`.
4. **`MapsToCook` 정리**(`:51-57`) — `L_Sandbox` 줄을 `L_Map_Center`로 교체 + **사문 3건**(`L_RunPersistent`·`L_MapA`·`L_MapB`, 디스크에 없음) 삭제.
5. **코드 주석 2건 갱신** — `FPSRFlowFieldBoundsVolume.h:31` · `FPSRFlowFieldSubsystem.h:22`가 예시로 `L_Sandbox`를 든다(주석뿐, 동작 무관).
6. **검증** — 이 시점에 PIE로 **런 1회 완주**(미션 스폰·룸 개방·보스까지). 여기서 통과해야 다음으로 간다.
7. **그때서야 `L_Sandbox` 삭제.** 6이 통과하기 전에 지우면 되돌릴 기준이 사라진다.

⚠️ **회귀 기준 상실 주의**: `L_Sandbox`는 지금까지 모든 게임플레이 검증의 기반이었다. 삭제하면 "예전엔 됐는데"를 대조할 맵이 없어진다 → **6번 검증을 형식적으로 넘기지 말 것.** 삭제는 git 히스토리에 남으니 정 급하면 복원은 가능하다(`git checkout <이전커밋> -- Content/Maps/L_Sandbox.umap`).

**A-4. 맵 저작** — 아래 콘텐츠 계약을 지킬 것

**A-5. SRS 액터 4종 배치 확인** — `L_GameFloor`(→`L_Map_Center`)에 이미 있으므로 리네임으로 딸려온다. 배치가 유지됐는지만 확인.
...
5. 🔴 **`L_Map_Center`에서 런 1회 완주** — 미션 스폰 · 룸 점진 개방 · 문 파괴 · 보스까지. **A-3의 이식이 제대로 됐다는 유일한 증거**이고, `L_Sandbox` 삭제의 전제 조건이다
6. **A-0 대비 perf 회귀 없음**(CSV 5스탯 비교, 수치를 PROGRESS에 기록)
7. **사용자 육안 게이트** — 룩 승인
8. **패키지 쿡 1회 성공** (§5-4) — `MapsToCook` 정리가 반영됐는지 여기서 드러난다
...
2. **맵 크기** — 264m 단일맵 유지 vs U 3×3 다중맵(슬롯당 코드 상한 132m). 셀 상한 40,000이 하드 잠금이라 되돌리기 어렵다.
3. **보스(Paragon Prime_Helix)를 셀/툰 통일 범위에 넣을지** — 과제 서술의 "캐릭터"에 보스가 명시돼 있지 않다.
4. **`MuzzleFlash` Cascade 유지 vs Niagara 전환**(후자는 C++ 타입 변경 + DA 8종 재저작). U13과 공동 결정.
5. **`Content/_SyntyPilot/`의 리타겟 자산(`IK_Blu`·`IK_Manny`·`RTG_Manny_to_Blu`)을 정식 경로로 승격할지** — 지금은 gitignore라 커밋되지 않는다.
6. **자체 절차적 크로스헤어(U12/U17 완료분)와 Synty 레티클의 중복 정리 방향** — 현재 무기 8종 전부가 자체 크로스헤어를 쓰는데 Rifle만 Synty 레티클도 참조한다.

> **해소됨(2026-07-19 사용자 결정)**: ~~`L_GameFloor` 승격 여부~~ → §1-1·§3 A-3으로 확정. ~~맵 이름 규약~~ → `L_Map_Center` 채택 + `MapsToCook` 사문 3건 삭제로 확정.
