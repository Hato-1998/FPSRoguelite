...
- 분산 판타지 = **하이브리드**(짧은 정찰 분산=자유·멋짐 / 장기 완전분산=은근히 비효율, 정찰 모드 수렴; 명시 확정 2026-07-05).
- 은근한 비효율(Tier 2, 명시 페널티/그룹 버프 아님): 최대 그룹 우선 미션 큐·솔로맵 정찰 태그·다역할 오브젝트·연쇄 미션 거리·문 시간세·보스 추적선·스카우트=정보. **⛔ 제외**: 전투 열기/소음 누적·고가치 이벤트 최소인원 조건(게이트/페널티 인접). 상세·금지선 = 리포트 §✅.
