...
7. 빈 맵 target=0 + 빈 맵 적 하드 드레인: 아무도 점유 안 한 맵의 적은 디스폰(예산 회수). ※ 점유맵의 원거리 적 recycle(안전게이트·drain rate)은 Tier 1 — Tier 0는 완전 빈 맵만 드레인(목격자 없음).
...
## 🙋 착수 전 디렉터 결정 필요 (Tier 0 비블로킹, Tier 2 튜닝 방향)
1. 솔로 분산 허용 강도(정찰 수용 vs 강한 억제) 2. 2인+ 전선 보상 강도("효율 좋음" vs "여기가 정답") 3. rally pad 정체성(구제 vs 전략 재집결 vs 페널티 회피) 4. 분산 판타지 태도 5. 보상 페어+ 체감 강도 6. **XP/자석/드롭 맵 경계 정책**(문 너머/빈 맵 drain 시 XP 소멸 vs 잔존 vs 근접 점유맵 이관). 상세 = `Docs/Review/20260705-multimap-budget-regroup.md` §🙋.
