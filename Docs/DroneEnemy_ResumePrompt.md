...
| **⚠️ BP 교체 함정 3 — 메시 절대 회전(2026-09-07 실측)** | Yaw 0 으로 고쳐도 PIE 에서 꼬리가 진행 방향을 봤다. 전 적 BP 3종의 Mesh 컴포넌트가 **`Absolute Rotation = True`**(회전 드롭다운 "절대") — 액터가 `SetActorRotation` 으로 돌아도 메시는 월드 방향 고정. C++ 에는 없고 BP 값(옛 회전 대칭 메시 시절엔 무해). 해소 = Mesh 컴포넌트 트랜스폼 → 회전 옆 드롭다운 **절대 → 상대**. 향후 정면이 있는 적은 전부 이 확인 필요(랭지드·엘리트 BP 도 True). BP 편집 = 사용자 |
| **C++ 변경 1건(2026-09-07)** | `FPSREnemyBase.cpp TickServerMovement`: 페이싱을 이동 블록 밖으로 — 정지 링에서 이동 벡터 0 이면 회전 갱신이 안 되던 버그(회전 대칭 적 시절엔 무해). 넉백 중 유지는 종전과 같음. 빌드 Succeeded · 스모크 통과 |
| **재임포트 보류** | 생성기 `file_v()`(UV V 선반전)는 코드에만 반영. 현 에셋은 미반영이지만 로터는 거울 대칭으로 정상 회전. 재임포트 시 `author_voxel_drone_material.py` 가 M/MI 를 **지우고 다시 만드니** 사용자 MI 조정값이 날아간다 — 재실행 전 MI 오버라이드를 메모하거나 material 단계를 빼고 import 만 돌릴 것 |
