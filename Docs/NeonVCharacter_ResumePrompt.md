...
> 복사 전용 파일 = **`Docs/Handoff/NeonV_Arcade/ImagePrompt_v1.txt`**(§4 프롬프트 2벌 + §6 체크리스트만 담은 평문. 사용자가 그대로 붙여넣는 용도).
> 도구 1순위 = Gemini(Nano Banana 계열) · 대안 = ChatGPT(GPT Image) — 참조 구조 보존력 근거는 `Docs/Research/ExternalModelingTools_2026-09-06.md`. **영어판 권장**(이미지 AI 는 영어 학습량이 압도적이라 디테일을 더 잘 지킨다).
>
