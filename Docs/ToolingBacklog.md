...
| 툴 | 유형 | 공수 | 임팩트 | 왜 (병목) | 상태 |
|---|---|---|---|---|---|
| Card Catalog & Balance Matrix (Blutility) | 뷰어 | M | high | U18d 미구현. 폴리모픽 카드 magnitude가 ~29 에셋 인라인 서브오브젝트에 묻힘 → EUW 그리드+이상치. §2-3-8 지시 | 📋 |
...
| Run Schedule Timeline Editor | 저작 | M | med | MissionWindows+BossTime+density 텍스트→EUW 타임라인+prod/test 프리셋. §2-8 압축값 리스크 완화 | 📋 |
