...
> ✅ **해소됨 (2026-08-11)** — 아래 12건 전부 SSOT에 반영했다. **D7만 코드가 아니라 문서를 바꿨다**(FF 기본값 = 사용자 결정으로 **OFF 확정**, 2026-07-01 "기본 ON" 설계를 뒤집음). 나머지는 문서를 코드에 맞췄고, D12는 "일치"임을 오해 방지용으로 명기했다. 감사 중 **D14(=`Concept.md`가 폐기본 문서를 살아있는 참조로 지목)** 1건이 추가로 나와 함께 고쳤다. 이 표는 **당시 스냅샷으로 보존**한다(왜 그렇게 고쳤는지의 근거).
