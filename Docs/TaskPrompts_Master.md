...
### 미배선 핵심 (게임 루프의 진짜 닫기)
> 갱신 2026-06-20: 패배(U2)·약점(U3a) 완료, 트랙 B 로비/세션/트래블 루프(U11a) main 머지(`b3b364e`) 완료. 승리(보스)는 트랙 A U3→U4에서 닫는다. **다음 차례=U18(카드 규약 정비, 사용자결정 U3 앞)→U3→U4.** 완료 유닛 전체는 §B 표(✅)·`PROGRESS.md`·`git log` 참조.
| 항목 | 근거 | 상태 |
|---|---|---|
...
- **순서 확정**: 게임 플로우 완성 먼저(트랙 A ✅U2→✅U3a→**U18 카드정비**→U3→U4 · 트랙 B ✅U11a 병렬 완료), 그 후 양 트랙 합류 → W1 전체 검증, 그 후 U1 게이트 → V2 검증. (2026-06-16부터 2-클론 병렬 — §B-3 / U18=2026-06-20 사용자 추가, U3 앞)
...
[1차 — 게임 플로우 완성. ✅U2 후 2-클론 병렬(2026-06-16 사용자 결정, 충돌점=GameMode 1파일)]
 ┌─ 클론 A (순차, 전투/보스+카드): ✅U3a 약점 → **U18 카드 규약 정비** → U3 보스 스캐폴드+승리 → U4 보스 콘텐츠+PIE   ← U18 진행 차례(사용자결정 2026-06-20: U3 앞 정비)
...
| U18 | **카드 시스템 규약 정비**(효과 적용모델 통일·검증강화·명명/중복 정리·확장 시임) — 토론(SSOT 선반영)→리팩토링 2페이즈. 콘텐츠 양산 전 규약 잠금 | 설계/C++/콘텐츠 | — (트랙 A 현재) | U6(소프트)·D6(시너지축 시임 제공)·U10(CardKey 시임). **U3 앞 시퀀싱**(사용자결정, 하드의존 아님) | `phase/card-conventions` |
...
### U18 — 카드 시스템 규약 정비 (토론 → SSOT 확정 → 리팩토링)

> **2026-06-20 사용자 추가, U3 앞 시퀀싱.** 콘텐츠(새 카드) 양산 전 느슨한 규약을 조이는 정비 유닛. 감사 근거: 효과 적용모델 3종 혼재 + 검증·명명·콘텐츠 규약 부재(상세는 프롬프트 [감사로 확정된 느슨한 지점]). 컨설트 D6(시너지축)와 직결하나 **상태시스템 본체(D3)는 G1 후 게이트라 여기선 확장 시임만**.

```
Game.md + PROGRESS.md 먼저 읽어. 그다음 Docs/SSOT/CombatWeaponCard.md §2-3(카드)·§2-4-1(무기 모디파이어/Fragment)를 정독해. 이 작업은 **카드 시스템 규약 정비**다 — 새 기능 추가가 아니라, 콘텐츠(새 카드) 양산 전에 느슨한 규칙을 조여 유지보수 가능하게 만드는 게 목적. 플랜모드 우선, HIGH_RISK 승인 후, 구현=Haiku 위임/검증·설계·토론=Opus 직접(CLAUDE.md·§6-5).

[배경 — 왜 지금]
볼륨업 컨설트(Docs/Review/20260616-volumeup-design.md)와 코드 감사가 공통 결론: 카드 메커니즘 골격(RarityTiers SetByCaller·Scope·서버권위 선택보안)은 데이터드리븐으로 잘 됨. 그러나 효과 적용모델이 3종(Character=GE / Weapon=Stat모드 / ThisWeapon=Fragment) 혼재하고 검증·명명·콘텐츠가 느슨해 카드 20장+ 추가 시 급속 복잡화·데이터오염 위험. §2-3가 스스로 "양산 전 축 잠금" 경고. 컨설트 액션 D6(시너지 축 정의)와 직결.

[현재 구조 — 실측됨, 재확인만]
- 코드: Source/FPSRoguelite/{Public,Private}/Card/(FPSRCardTypes.h·FPSRCardDataAsset·FPSRCardPoolDataAsset·FPSRCardSubsystem) + Weapon/FPSRWeaponFragment + Weapon/FPSRWeaponInstance(RecomputeResolved switch) + UI/FPSRCardEntryWidget·FPSRCardSelectWidget
- 콘텐츠: Content/Cards/Character/{Data,GameplayEffect} · Content/Cards/Weapons/{Card,Modifiers}
- 스키마: ECardScope(Character/ThisWeapon/AllWeapons) · EFPSROfferType(OpeningSeed/LevelUp/MissionReward) · ECardRarity(4종) · FFPSRCardRarityTier{Rarity,Magnitude} · UFPSRCardDataAsset(Scope+AppliedEffect(GE)/WeaponStat+Op/GrantedFragment 분기)

[감사로 확정된 느슨한 지점 — 토론·정비 대상]
1. 3종 적용모델이 한 DataAsset에 Scope+GrantedFragment null-ness로 분기(EditConditionHides 이미 복잡): Character=AppliedEffect / Weapon=WeaponStat+Op / ThisWeapon=GrantedFragment. 4번째 효과축(상태이상/원소) 추가 시 ApplyCard(FPSRCardSubsystem.cpp:230~284)·null체크(:270~278)·검증이 if/elseif 체인으로 붕괴.
2. 검증 공백(FPSRCardDataAsset.cpp IsDataValid): 빈 RarityTiers·Character무AppliedEffect·Fragment×비ThisWeapon만 잡음. Scope×필드 부정합(Character인데 WeaponStat 세팅 / AllWeapons인데 GrantedFragment), 빈 CardFamily의 GE-class fallback 의미, 음수 Magnitude 무검증 → 런타임 무음 무시.
3. 콘텐츠 중복·명명 불일치: DA_Card_<Stat>_AllWeapon ↔ _ThisWeapon 동일스탯 2자산 분리 / DA_CardModifiers_*(=카드정의 UFPSRCardDataAsset) ↔ DA_Fragment_*(=행동 UFPSRWeaponFragment, 카드가 GrantedFragment로 참조) 이중정의 — 관계가 이름에 안 드러남 / 'Bouns' 오타. 명명 규약 부재.
4. 매직값: SetByCaller 태그 "SetByCaller.CardMagnitude" 하드코딩(FPSRCardSubsystem.cpp:240, GE작성자 오타 시 무음 미적용) / 드로우 Count=3이 코드·UI(CardEntry_0/1/2) 양쪽에 박힘 / Rarity 표시문자열 UI switch 하드코딩(FPSRCardEntryWidget.cpp:70~77).
5. 무기 Stat 해석이 데이터 아닌 코드 switch(FPSRWeaponInstance.cpp:161~184) — 새 Stat축은 enum+case 코드수정 필요.
6. 안정 ID 부재: 카드=에셋참조로만 식별. U10 메타/SaveGame·밸런스패치 시 CardKey/StableID 필요(에셋 rename에 깨짐).
7. 확장 적합성: AllWeapons 스코프는 Stat모드만 가능 — non-Stat(상태이상) 광역효과 불가. "Effect Type 축(Stat/Behavior/StatusEffect)" 분리 필요 여부 결정.

[보존할 자산 — 절대 깨지 마라]
SetByCaller 1에셋 다Rarity 패턴 / 서버권위 인덱스-선택 보안(FPSRCardSubsystem.cpp:60~189, 클라가 포인터 아닌 인덱스만 전송) / CardFamily 상호배제 / Scope 적용경로 분리 / Fragment MaxStacks 누적 / AllWeaponsStatExclusions(ChargeLaser 반동 제외).

[작업 구조 — 2페이즈, 토론 먼저]
■ 페이즈 1 — 토론·설계 수렴 (코드 0, 산출 = SSOT 갱신 + 규약 스펙)
 - 플랜모드. 아래 쟁점을 /consult(백엔드 Claude × 클라 Codex, Docs/ConsultLoop.md)로 수렴하거나, 불가 시 Plan 에이전트 적대검증으로 결론.
 - 반드시 결론낼 쟁점:
   (a) 효과 적용모델 통일: 3종 분기 정리안 — ① Scope(범위)×EffectType(Stat/Behavior/StatusEffect) 2축 분리 / ② 폴리모픽 적용자 오브젝트 / ③ 현행 유지+검증강화. 제1원리(데이터드리븐·서버권위·적500예산·확장성)로 근거 3줄.
   (b) 확장 시임 vs 본구현 경계: 상태이상/원소축(컨설트 D3 상태서브시스템)은 G1 후 게이트 → 지금은 *스키마 시임*만(EffectType 자리·AllWeapons non-Stat 경로 결정), 상태시스템 본체는 만들지 마라. "지금" / "D3 후" 경계 선긋기.
   (c) 명명·폴더 규약 확정: 카드(정의) vs Fragment(행동) vs GE 네이밍 규칙, AllWeapon/ThisWeapon 자산 통합 여부, 폴더구조.
   (d) 검증 규칙 표: Scope×필드 합법조합 매트릭스 → IsDataValid가 강제할 규칙 목록.
   (e) 안정 ID: CardKey(FName/GameplayTag/enum) 지금 도입 vs U10 위임.
   (f) 매직값 데이터화 범위: SetByCaller 태그·Count·Rarity문자열 중 무엇을 데이터/설정으로.
 - 결론을 Docs/SSOT/CombatWeaponCard.md §2-3(필요 시 §2-4-1)에 선반영(SSOT-first) + 규약 스펙 명문화.

■ 페이즈 2 — 리팩토링 구현 (페이즈1 SSOT 확정 후)
 - 브랜치: main에서 phase/card-conventions 분기(§6-7).
 - 플랜 재확정(페이즈1 결론) → 승인 후 Haiku 구현 / Opus 검증.
 - 범위(확정된 것만): IsDataValid 검증강화 / 매직값 데이터화 / (결정 시)EffectType 시임 / 콘텐츠 명명·중복 정리(BounsDamage→Bonus, Modifiers/Fragment 정합, AllWeapon/ThisWeapon 통합) / (결정 시)CardKey.
 - 무회귀 절대조건: 기존 캐릭터카드 7종·무기카드·Fragment 4종 현행과 동일동작. 선택보안·CardFamily·SetByCaller 불변.

[함정/주의]
- 콘텐츠 .uasset 이름변경/이동=참조 끊김 위험. VibeUE MCP 또는 에디터에서 리다이렉터 동반, 에디터 종료상태 git mv 금지. (메모리 marketplace-asset-import-relocate / vibeue-mcp-capabilities)
- 에셋경로 C++ 하드코딩 금지(§6-2). 태그 데이터화해도 기본값은 DataAsset 필드.
- 검증강화가 기존 콘텐츠를 '에러'로 막아 게임이 안 뜨면 안 됨 — 기존 자산이 새 규칙 통과하도록 콘텐츠도 함께 교정.
- 충돌면: U3 보스(상태카드 욕구)·U11b(AllWeapons 모디파이어 복제)·U10(CardKey 소비) — 페이즈1에서 인터페이스 시임만 의식, 본구현 침범 금지.

[검증]
페이즈2 후 빌드(-WaitMutex)+헤드리스 스모크(FPSRoguelite.Smoke.ModuleLoads Result={Success}) → 사용자 PIE: 레벨업 3장 추첨·선택·적용(캐릭터GE/무기Stat/Fragment 각1) 정상 / 잘못된 테스트카드를 IsDataValid가 에디터에서 차단 / 미션보상 Fragment 오퍼·스택 정상 / 리롤 정상 / 기존수치 무회귀 → codex-review.ps1 -Base main → PROGRESS·TaskPrompts 갱신 + 사용자 콘텐츠 동반커밋 질문 + --no-ff 머지.

[완료 처리]
SSOT 규약 스펙 명문화 + 코드/콘텐츠 정비 + 검증강화. PROGRESS 완료절 + TaskPrompts §B ✅. 상태이상/원소축(D3/D6)이 이 시임 위에 얹힌다고 기록.
```
