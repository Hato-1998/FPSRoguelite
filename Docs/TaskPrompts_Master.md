...
### 장기 백로그 (유닛화 보류 — 기록만)
- **다중맵 심리스 (컨셉 피벗 2026-07-03 · SSOT 정식반영 2026-07-05)**(`RunFlow §2-1` · `Concept §1-C-9` · `Roadmap §8`): 맵 4방향 거대문 부수기 → 심리스 다음 맵(레벨 스트리밍/World Partition), 맵점유 미션/스폰, 통일 로우폴리 Synty Path A(Sci-Fi City/Nature/Sci-Fi Space). ⚠️ **#3 다중맵 아키텍처 = OPEN(P0급, 별도 설계)**: 적 예산 점유맵 공유 vs per-map(하드캡 500 재정의) · 플레이어 분산 vs back-to-back USP · RepGraph relevancy+스트리밍 앞당김. **착수 전 Synty 파일럿 검증**(적300+U7+20분 실측). 콘텐츠 단계(P6+/U14 이후).
