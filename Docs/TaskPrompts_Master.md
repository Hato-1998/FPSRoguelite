...
| U18 | **카드 시스템 재설계**(목표 사양 9조건: 3 카드군·멀티효과·무기해금·행동훅·이동속도 + 검증·명명·안정ID) — 토론(§2-3 선반영)→서브유닛 분해(U18a/b/c) 구현. 상태이상 처치·상태창=시임 | 설계/C++/콘텐츠 | — (트랙 A 현재) | U6(소프트)·D3(상태축 시임)·U10(CardKey 시임). **U3 앞 시퀀싱**(사용자결정). 분량 큼=서브유닛 분해 | `phase/card-*` |
