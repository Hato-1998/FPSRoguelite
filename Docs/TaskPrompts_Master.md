...
- **순서 확정**: 한 유닛씩 순차 — 게임 플로우 완성(U2→U3a→U3→U4)이 먼저, 그 후 W1 전체 검증, 그 후 U1 게이트 → V2 검증
...
[1차 — 게임 플로우 완성. ✅U2 후 2-클론 병렬(2026-06-16 사용자 결정, 충돌점=GameMode 1파일)]
 ┌─ 클론 A (순차, 전투/보스): U3a 약점 → U3 보스 스캐폴드+승리 → U4 보스 콘텐츠+PIE
 └─ 클론 B (병렬, 세션/로비): U11a 로비·세션·트래블 루프 (Play→로비 허브 / Steam / 보스 §3-5 제외)
        ↓ (양 트랙 합류 — GameMode EndRun 시임에서 머지)
...
| U3a | 약점 부위 데미지 시스템(헤드샷/디자이너 지정 존) | C++ | U2(순차) | U3/U4(보스 약점), U1(손맛 판정에 포함) | `phase/p6-weakpoint-damage` |
| U3 | D4 보스 스캐폴드+승리 배선 | C++ | U2, U3a | U4, U11b | `phase/p6-boss-scaffold` |
| U4 | 보스 콘텐츠+PIE(콜리전 정합·체력바) | 콘텐츠 | U3 | W1, U11b, U14 | U3 동승 또는 `content/boss` |
| U11a | **D5 세션+로비+트래블 루프**(Play→로비 허브, Steam 초대, 보스 제외 — 클론 B 병렬) | C++/콘텐츠 | U2 + 메뉴↔로비 결정(✅ Play→로비 허브) | U11b | `phase/p7-mp-loop-lobby` |
...
## §B-3 작업·검토·머지 프로토콜 (순차 — 한 유닛씩)

> **한 번에 한 유닛만** 진행한다(병렬 폐지, 2026-06-15). 메인 작업 디렉터리에서 phase 브랜치를 분기해 작업하고, 검증 통과 후 머지한 다음 다음 유닛으로 넘어간다. 단순하고 충돌이 없다.
...
- **순서**: 의존성을 따른다 — `U2 → U3a → U3 → U4 → W1` (남은 1차/보스 트랙). 그다음 게이트(U1) → 2차 트랙.
...
> **한 번에 한 유닛씩 순차 진행**(병렬 폐지 2026-06-15) — 의존 순서 `U2 → U3a → U3 → U4 → W1`.
