...
[1차 — 게임 플로우 완성. ✅U2 후 2-클론 병렬(2026-06-16 사용자 결정, 충돌점=GameMode 1파일)]
 ┌─ 클론 A (순차, 전투/보스+카드): ✅U3a 약점 → **✅U18 카드 시스템 재설계(목표사양 9조건, U18a/b/c 완료 2026-06-20)** → ✅U3 보스 스캐폴드+승리(2026-06-21) → ✅U4 보스 콘텐츠+PIE(2026-06-21) → ✅W1 1·2·3차 완료(2026-06-21~26, 머지 `b2c55d3`) → **U1 재미게이트(+V2) ← 현재**
...
| 🟡U1 | 재미 게이트(§7-5)+성능 검증(§5) — 조건부합격(2026-06-29)의 **MP 차단조건 해소**(Phase 1A 복제+1B DBNO 머지 `e38dfbe`+PIE 2-client 통과). **남은 것=§5 적500 정량 재측정(B7 픽스로 도달가능)+G1 ①~⑤ 최종 사인오프+V2** → Perf §5 하드캡 확정 → U1 ✅. ⚠️현 main으로 패키지 재빌드 필요 | 검증(사용자 PIE) | V0, V1, W1 | 2차 트랙 전체 | (검증·문서) |
| 🟡V2 | 2-client(FF·ChargeLaser) — U1과 동시. MP E2E 복제 결함(클라 Defeat UI·아군 사격음·클라 적 동기화·보스 체력바)=**Phase 1A/1B로 해소·머지(`e38dfbe`)**. 남은=2-client FF/ChargeLaser 최종 체크(U1 마무리와 동시) | 검증(사용자 PIE)+C++ | W1 | U11b(소프트) | ~~`fix/mp-steam-e2e`~~ 머지됨 |
