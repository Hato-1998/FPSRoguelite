...
| 🟡U1 | 재미 게이트(§7-5)+성능 검증(§5) — **조건부 합격(2026-06-29)**: 핵심 루프 동작, MP 동기화·피드백 결함 25건 발견(`Docs/U1_PostGate_Fixes.md`) → **Phase 1 MP 넷코드 수정 패키지 E2E 검증 후 U1 ✅** | 검증(사용자 PIE) | V0, V1, W1 | 2차 트랙 전체 | (검증·문서) |
| 🟡V2 | 2-client(FF·ChargeLaser) — U1과 동시. **MP E2E 테스트서 복제 결함 다수**(클라 Defeat UI·아군 사격음·클라 적 동기화·보스 체력바) → `fix/mp-steam-e2e` Phase 1로 처리 | 검증(사용자 PIE)+C++ | W1 | U11b(소프트) | `fix/mp-steam-e2e` |
