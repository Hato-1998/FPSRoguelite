...
```
[1차 — 게임 플로우 완성 트랙]
V0 무기 비주얼 베이스 ∥ V1 사각 경고 오디오   (병행 가능, 서로 독립)
        ↓
U2 패배 배선(전멸→EndRun Defeat)
        ↓
U3 보스 스캐폴드+승리 배선(D4)
        ↓
...
| # | 유닛 | 구분 | 선행 | 차단(블록) | 브랜치 |
|---|---|---|---|---|---|
| V0 | 무기 비주얼 베이스 적용(Infima 팩) | C++/콘텐츠 | — | U1(손맛 판정) | `phase/p6-weapon-visuals` |
...
| U3 | D4 보스 스캐폴드+승리 배선 | C++ | U2 | U4, U11 | `phase/p6-boss-scaffold` |
...
| U12 | UI/필 후속(크로스헤어·히트마커·카드 아이콘) | C++/콘텐츠 | U1 게이트 | — | `phase/p4d-ui-followups` |
...
> 모든 프롬프트 공통 전제(프롬프트에도 내장됨): `Game.md`+`PROGRESS.md` 선행 독해 → 해당 도메인 SSOT만 추가 독해(§0-1 라우팅) / `phase/` 브랜치 분기(§6-7) / 플랜모드 우선·승인 후 진행 / 구현=Haiku 위임·검증=Opus 직접(§6-5) / 빌드 `-WaitMutex`+헤드리스 스모크 / Codex는 머지 시점 일괄(`Scripts/codex-review.ps1 -Base main`) / 완료 시 PROGRESS 갱신+`--no-ff` 머지+브랜치 삭제+사용자 콘텐츠 동반 커밋 여부 질문.
...
- 플랜 우선 → 승인 후 Haiku 구현 / Opus 검증
- 선행: U2 머지 확인(전멸 판정·EndRun 배선 패턴 공유)
...
[검증 — 전체 플로우 PIE 체크리스트]
① 메뉴→Play→런 시작 ② 미션 발생·클리어·카드 보상 ③ BossTime 도달→보스 스폰 ④ 전 무기(8종)로 보스 데미지 ⑤-a 보스 처치→VICTORY ⑤-b (별도 런) 전원 사망→DEFEAT ⑥ 결과창→메뉴→재시작 무결성(상태 리셋).
...
[산출물 — 순서 고정]
1. 동적 스프레드 크로스헤어(원 레티클): 발사 랜덤(SpreadDegrees+VRandCone)은 이미 구현됨 — **시각만 신규**: GetCurrentSpreadDegrees() 게터 + WBP 원 표시(산탄 무기 체감). 발사 로직 중복 구현 금지(스펙시트 §4).
2. 히트마커 최종 연출 재확인 — 크로스헤어 작업 **후**(P4-D 이월 순서 고정).
