...
| V3 ✅ | 기본 크로스헤어 HUD(정적 레티클) — **완료 2026-06-15** (십자+점·ADS숨김·히트마커 X 4선+Hit흰/Kill빨강, PreConstruct 변수화, PIE 통과) | 콘텐츠(+디버그 cvar 게이트 C++) | — (1차 트랙 병행) | U1(손맛 ① 평가에 필수), U12(동적 스프레드 베이스) | `phase/p4d-crosshair` |
...
```
⚠️[병렬 작업 시] 다중 세션이면 이 문서 §B-2(병렬 가이드)·§B-3(검토·머지) 먼저 읽고 별도 worktree에서 진행. main 머지는 빌드+스모크 통과·PR·사용자 승인 후.
