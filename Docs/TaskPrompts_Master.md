...
[1차 — 게임 플로우 완성. ✅U2 후 2-클론 병렬(2026-06-16 사용자 결정, 충돌점=GameMode 1파일)]
 ┌─ 클론 A (순차, 전투/보스+카드): ✅U3a 약점 → **✅U18 카드 시스템 재설계(목표사양 9조건, U18a/b/c 완료 2026-06-20)** → ✅U3 보스 스캐폴드+승리(2026-06-21) → ✅U4 보스 콘텐츠+PIE(2026-06-21) → ✅W1 1·2·3차 완료(2026-06-21~26, 머지 `b2c55d3`) → **✅U1 재미게이트(+V2) 합격(2026-06-30, §5 정량 보류) → 2차 트랙 해금 → ✅U5 원거리적(2026-07-01, 머지 `cd7de43`) → 2차 잔여(U6/U7/U8/U10/U15/U17) ← 현재**
...
U1 재미 게이트(§7-5)+성능 검증(§5)  →  V2 2-client 정합 검증
        ↓ (게이트 통과 시 해금)
[2차 — 의존 없음, 순서 자유]  ✅U5 원거리 적(2026-07-01) / U6 Fragment 마무리 / U8 GMS / U10 SaveGame
...
| U6 | A4 Fragment 마무리+AvailableModifiers 확장 — 🔧**코드완료·검증**(근접 PreFire/OnHitActor/PostFire 훅 + 슬롯상한 MaxFragmentSlots/제거/교체 안티치트 ServerSelectCardReplacement; 빌드+스모크+Codex P1/P2 교정). 다음=콘텐츠(무기별 UnlockableFeatures 매트릭스, [[card-pool-routing]])+사용자 PIE→머지. 교체 UI=후속 | C++(+콘텐츠 꼬리) | U1 게이트 | — | `phase/p4b-fragment-finish`(활성) |
