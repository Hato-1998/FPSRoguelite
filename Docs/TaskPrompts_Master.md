...
[1차 — 게임 플로우 완성. ✅U2 후 2-클론 병렬(2026-06-16 사용자 결정, 충돌점=GameMode 1파일)]
 ┌─ 클론 A (순차, 전투/보스+카드): ✅U3a 약점 → **✅U18 카드 시스템 재설계(목표사양 9조건, U18a/b/c 완료 2026-06-20)** → ✅U3 보스 스캐폴드+승리(2026-06-21) → ✅U4 보스 콘텐츠+PIE(2026-06-21) → **W1 ← 다음 차례**
...
[범위·방법] Docs/SSOT 전 파일(RunFlow/CombatWeaponCard/Enemy/PlayerFeel/Architecture/Performance/Workflow/Roadmap)을 도메인별로 읽고, 대응 코드(Source/FPSRoguelite/{Core,Run,Enemy,Boss,Weapon,AbilitySystem,Card,UI,Hero,Combat,Pickup})를 서브시스템 단위로 대조하라. **Boss/(U3 신설)·Card/ v2 효과레이어(U18 UFPSRCardEffect 폴리모픽)·무기 행동훅(U18c 5경로 브릿지)는 최근 추가분이라 우선 점검.** 필요 시 서브에이전트로 영역 분할(단 판정은 직접).
...
8. 알려진 수용 설계를 재발견해 "버그" 보고 금지: 관통+ExplosiveRounds 탄착점 1회 폭발 / ExplosiveRounds 직격 히트마커 2회 / ChargeLaser 차징 중 프리즈=데미지만 스킵 / ChargeLaser Finding A(원격 클라 코스메틱 레이스, U13에서 notify로 해소 예정) / **보스 WBP=범용 `UFPSREnemyHealthComponent` 바인딩**(U4, 엘리트 재사용 가능·잡몹 전체는 perf·원칙1 위배로 의도적 보류) / **보스 C++ 폴백 `DefaultMaxHealth`**(콘텐츠 `DA_BossDefinition`가 override) / **U18 멀티효과 Instanced `UFPSRCardEffect` 직렬화**(cook/load 라운드트립 검증됨) / **`bJustKilled`**(U18c, 코프스 재타격 중복 처치·킬마커 차단=정상) / **보스 킬이 무기 OnKill fragment 발화**(U3+U18 시임=의도) / **OnStatusKill·상태창·elemental 거동=빈 시임**(D3/후속 미구현=의도) / **카드 v2 레거시 필드 `meta=(DeprecatedProperty)` 잔존**(마이그레이션 후 U18 후속 삭제 예정).
...
[방법 B — 영역 분할 직접 호출] codex exec(비대화)로 영역 스코프 프롬프트 실행(절대경로 C:\Users\koras\AppData\Local\OpenAI\Codex\bin\codex.exe, -C 리포루트, -s read-only, -a never — pwsh 없음·PS5.1 주의):
  영역 순서(중요도순): ① Core/Run(GameMode/GameState/RunDirector/미션/보스 스폰·승리) ② Enemy+Boss(스폰/플로우필드/HealthComp/BossBase/BossDefinition/BossSpawnPoint) ③ Weapon+AbilitySystem(FireComponent/GA/CombatStatics/Projectile/U18c 행동훅 5경로 브릿지) ④ Card+UI(v2 효과레이어 UFPSRCardEffect·멀티효과·무기해금 라우팅) ⑤ Hero/Pickup.
