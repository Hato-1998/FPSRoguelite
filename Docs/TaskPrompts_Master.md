...
| ✅U17 | 플레이어 설정 시스템(크로스헤어 크기) + 메인메뉴 Settings 진입 — **완료(main `--no-ff` 머지, 2026-07-03)**: GUS `CrosshairScale`(0.5~2.5·기본1.0)+`OnCrosshairSettingsChanged` 델리게이트 + 설정오버레이 슬라이더(GUS 직접구동) + 런HUD `CrosshairImage` RenderScale 실시간반영(Construct구독/Destruct해제). 빌드+Smoke 4/4+사용자 PIE(메뉴/인게임 실시간·세션영속·기본무회귀) 통과. **동반=U12 크로스헤어 동적Spread 대각선 아티팩트 완화**(M_DynamicCrosshair Custom HLSL 45°웨지 각도정규화 smoothstep — ⚠️오프라인 재현불가라 PIE 최종확인 필요)+레티클 텍스처20 Wrap→Clamp. Codex 머지게이트 | C++/콘텐츠 | V3(크로스헤어 위젯) / 소프트 U1 게이트 | (감도·FOV·오디오 설정 확장) | ~~`phase/settings-system`~~→main `--no-ff` |
