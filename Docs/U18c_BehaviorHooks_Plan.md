...
- **OnMiss**: 동기 경로(Hitscan/ChargeLaser/Melee)=활성화 종료 시 `!bServerHit && !bImpactHitEnemy` 동기 판정. **Projectile=ReleaseToPool서 단발 활성화(`bSingleProjectileActivation`)+적 무피해 종료 시 NotifyMiss**(머지게이트 R4서 시임 구현·R5서 per-activation 정합). 멀티샷 투사체 볼리=미발화(AmmoOnMiss+MultiShot 투사체 미지원, rare). Explosion 자체경로=투사체가 소유.
...
## 무회귀 절대조건
- 기존 7훅(PreFire/ModifyShotCount/OnHitActor/PostFire/OnProjectileSpawn/ModifyChargeTime/OnImpact)·U18b 거동(무기해금·라우팅·탄도) 유지. ⚠️OnImpact 시그니처 `bool& bOutHitEnemy` 추가(머지게이트 R1) — 기존 거동 무변경.
...
## 구현 현황 (2026-06-20) — 코드 완료·검증 통과, 콘텐츠+PIE 잔여
- **코드 5커밋**(br랜치 `phase/u18c-behavior-hooks`): c1 `ca8ea5e`(훅+bJustKilled+브릿지) · c2 `8ddfd77`(GAS 패시브) · c3 `a986228`(B/C fragment) · 머지게이트 fix `d850800`/`2355367`/(R3)/(R4)/(R5). 빌드 Succeeded(0warn)·헤드리스 스모크 Result={Success}(각 단계).
- **Codex 게이트**: 플랜↔목표(P1×2: DamageDealt 실측 이벤트·문서정합) + 머지게이트 6라운드 — R1(OnMiss×ExplosiveRounds 스플래시)·R2(패시브 OnGiveAbility·ReloadOnKill 무기타겟)·R3(TryDamageActor 반환계약/친선사격)·R4(투사체 OnMiss 시임)·R5(투사체 OnMiss per-activation 정합) **전 교정 + R6 클린**.
- **잔여=콘텐츠 저작 + 사용자 PIE** → 콘텐츠 동반커밋 질문 + --no-ff 머지.

### 콘텐츠 체크리스트 (에디터/VibeUE 또는 헤드리스 스크립트)
- **c2**: ① `GE_Card_LifestealHeal`(Instant, SetByCaller.CardMagnitude→+Health) ② `GE_Card_HealthRegen`(Infinite·Period 1s·+Health/period) ③ `GA_Lifesteal`(BP:UFPSRPassiveAbility_Lifesteal, HealEffect=①·HealRatio=0.05) ④ `DA_Card_Character_Lifesteal`(Group=Character, Effects=[CardEffect_CharacterPassive{PassiveAbility=③}]) ⑤ `DA_Card_Character_HealthRegen`(Group=Character, Effects=[CardEffect_CharacterGE{Effect=②}]) ⑥ 캐릭터 카드 풀에 ④⑤ 등록.
- **c3**: ⑦ `DA_Frag_LMG_AmmoOnMiss`(UFPSRFragment_AmmoOnMiss) + 카드(WeaponBehavior→fragment)→LMG UnlockableFeatures ⑧ `DA_Frag_Shotgun_ReloadOnKill`·`DA_Frag_Bazooka_ReloadOnKill`(UFPSRFragment_ReloadOnKill, bInstantRefill) + 카드→Shotgun/Bazooka UnlockableFeatures. (헤드리스=`Scripts/u18b_migrate.py` 패턴; GE/GA BP=에디터/VibeUE.)

### 사용자 PIE 체크리스트
- **c1 무회귀**: 5무기(연사/점사/스나이퍼/샷건/바주카/유탄/차지빔/근접) 정상 데미지·킬마커. **코프스 재타격→킬마커/마커 없음**(bJustKilled). 친선사격(FF on) 관통/소비 정상.
- **c2**: 흡혈 카드픽→적 데미지 시 비율 회복(오버킬/코프스 무회복)·재생 카드픽→초당 회복·런 종료(로비 복귀)→패시브 클리어(다음 런 무흡혈).
- **c3**: LMG AmmoOnMiss 해금→빗나감 시 탄 리필 / Shotgun·Bazooka ReloadOnKill 해금→처치 시 재장전.
