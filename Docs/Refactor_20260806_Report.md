...
소량의 확실한 수정(18파일) + 사용자 결정이 필요한 항목 9건 + 문서 드리프트 13건**이다.
...
### 6-2. 이 표에서 제일 중요한 한 줄
**`GlobalAliveCap = 200`** 은 승격 후보이기 이전에 **문서와 어긋난 값**이다.
`Game.md §1` 은 "동시 ~200-300, 폴백캡 300, 하드캡 500"이라 적었지만,
이 상수가 **모든 스폰 경로를 무조건 게이트**하므로 실효 상한은 200이고
단일맵에서는 예약분 8을 빼 **192**다. `MaxAliveCount=300` 은 도달할 수 없는 스케줄 클램프다.
→ 승격을 논하기 전에 **문서와 코드 중 어느 쪽이 의도인지부터** 정해야 한다.
...
**문서를 고치지 않았다** — 어떤 건 문서를 코드에 맞춰야 하고(D1·D2·D3·D5·D8·D9·D10),
어떤 건 코드를 문서에 맞춰야 하기(D7 프렌들리 파이어 기본값) 때문에 일괄 처리할 수 없다.

가장 시급한 3건: **D8**(적 상한 200 vs 문서 300/500) · **D5**(있다고 적힌 크로스헤어 크기 설정이 코드엔 없고,
코드는 "의도적으로 안 만든다"고 명시) · **D6**(1인칭 팔 "폐기"라 적혔는데 현재 트랙이 정반대).
