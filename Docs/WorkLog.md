...
### 판단 보류 2건 (임의로 옮기지 않음 — 사용자 확인 대기)
- **「4b 로코모션 잔여」** — 폐기가 아니라 **완료 근거가 없는** 케이스다. 본문이 "완료커밋 미기재 — git 전체 로그를 훑었으나
  이 행의 작업을 담은 커밋을 못 찾았다"고 자백한다. 다른 클론/브랜치에서 커밋을 찾아 채우거나, 정말 안 했다면 **재오픈**이 맞다.
- **「총 모션 클립 저작 파이프라인 + 에디터 탭 툴」** — 저작 툴(4세대)은 **결과물 0으로 폐기**(`cdab7f2a`)가 맞지만,
  이 행이 담은 30커밋 중 상당수가 **지금도 쓰이는 산출물**(P2 커브 런타임 `75db21f7`·A16/A17 규약·gun-anchor 구조·DA 스키마)이다.
  위 기준("산출물 없이 접었다")에 안 맞아 일단 완료 유지 — 행을 **툴/런타임으로 쪼갤지**가 실제 갈림길이다.
