...
**■ 남은 것**
- ⚠️ **추적 필요 3건** (로그에 "미착수"로 반복되다가 완료 언급 없이 사라진 것 — **"해소됨"으로 단정하지 말 것**): 보드 `영역` 공백 24행의 해소 여부 · `관련문서` 속성으로 보드↔문서 DB 를 잇는 배선(대체 관계 7쌍은 완료가 명시됐는데 이건 어디에도 없다) · `Workflow.md`·`CodexRedTeamPersona`·`InternalRedTeamReview` 가 문서 DB 에 색인 행으로 등록됐는지.
