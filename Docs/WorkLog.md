...
**남긴 것(후속 행)**: **ADR 0017 아레나 = 팩맨 미로**(타일 크기·위상·구석 에리어 내용) · 쩝쩝이 MI 색 확정(ART-후속, 사용자) · 아레나 블록 복셀 면 프로토(5cm) · 복셀 무기(2.5cm, 하드서피스 트랙 방향 전환) · 픽셀 스프라이트 VFX 세트 · HUD 재스킨 · 한글 픽셀 폰트(Galmuri 후보) 확정. 격자 수치·팝 면적 10% 는 PIE 육안 조정 대상(사용자).
