...
- **노션 AI 분야 페이지 드리프트 5건** (2026-09-09 확인) — 「온보딩·착수 절차」·「함정 사전」이 09-08 최종수정이라 09-09 신설분을 못 담았다: §6-9 (9) 의 영역→분야 매핑표 없음 · 구조 ID 표 없음 · **아키텍처 결정의 정본 방향이 반대로 적혀 있음**("정본 = 노션 결정 기록 DB") · `Docs/Specs/*` 경로 누락 · **K2~K5 전무**(K1 만 부분 반영). §6-9 (9) 상 충돌 시 repo 가 정본이라 오독 위험은 막혀 있으나, 방치하면 다음 AI 세션이 낡은 절차서로 온보딩한다.
