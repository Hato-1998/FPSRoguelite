...
- ✅ **추적 필요 3건 실측·조치 (2026-09-09)** — 셋 다 **해소된 적이 없었다**(SQL 전수, `has_more:false`).
  ...
  - ⚠️ **남은 repo 반영 1건** — 위 폐기 결정을 `Workflow.md` §6-9 (9) 에도 한 줄 박아야 다음 세션이 다시 열지 않는다. **이번 세션은 못 했다**: 같은 파일·같은 절을 다른 세션이 `진행중`으로 잡고 있다(보드 「PM 보드 시작 지연 3단 개선」, `주요경로` = `Docs/SSOT/Workflow.md §6-9`). §6-9 (3)-5 병렬 충돌 검사의 기본값대로 **대기**했다.
