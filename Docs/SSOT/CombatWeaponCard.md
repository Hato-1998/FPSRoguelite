...
- **배제 규칙 v4 (2026-08-13 사용자 확정 — v3의 "같은 속성=한 제시 1장" 대체)**: **배제 키 = (family, 굴린 레어도) 쌍.** 같은 family라도 레어도가 다르면 한 제시에 공존 가능(같은 카드가 레어도만 달리해 2장 등장 가능 — 종전 동일 카드 포인터 차단은 이 규칙이 대체·폐지). **family 자동 파생에 스코프 구분 포함**: WeaponStat 카드 = `<E1_Attr>.<all|this>`(전체무기와 개별무기는 서로 다른 카드 — 동시 제시 허용), 그 외 = `<E1_Attr>`. 판정표: All레어+All레전더리 ✅ / All레어+All레어 ❌ / All레어+This레어 ✅. **배제 키는 대상 무기까지 구분한다**(사용자 확정 2026-08-13 — 다중 무기 공유 카드의 Rifle용·SMG용 오퍼는 같은 레어도라도 별개 선택지로 공존. 키 = family × 레어도 × TargetWeapon; 캐릭터/전체무기 오퍼는 TargetWeapon=null이라 종전 의미 유지). 같은 카드+같은 레어도+같은 무기는 family 유무와 무관하게 무조건 배제(P2 가드). ⚠️ v4 이전에 파생값과 묶을 목적으로 명시 Family(`weapon.firerate` 등 무접미)를 저작했다면 스코프 접미 도입으로 묶음이 조용히 풀린다 — 재점검할 것(현 콘텐츠는 명시 저작 0건).
...
- **캐릭터 행동 = GAS-native**(무상태 Fragment 미러 아님 — 플레이어는 ASC 보유·4인뿐, 적500 perf 이유 없음): ① **흡혈** "데미지 줄 때 회복" = **단일 브릿지 `FPSRCombat::ApplyDamage`**에서 instigator ASC에 `GameplayEvent`(`GameplayEvent.Player.DealtDamage`, 페이로드=**실제 입힌 데미지 `DamageDealt`**, 오버킬/코프스 제외) → 트리거 패시브 GA가 회복 GE(힐=DamageDealt×비율, **모든 무기경로 균일** — 브릿지가 공통이라 캐릭터 효과엔 충분, 무기 Fragment 훅과 달리 FireCtx 불요). ⚠️적500: 이벤트송신은 플레이어-instigator·`DamageDealt>0`·**리스너 카운트>0**(흡혈 grant 시 캐릭터에 set)일 때만. ② ~~**초당 체력 재생** = **기존 `UCardEffect_CharacterGE` + periodic infinite GE**(패시브 GA 불요, 코드 0)~~ → 🔴 **은퇴(사용자 결정 2026-09-01, VIT1)**. 아래 「체력 회복 = 조건부」 참조. `UCardEffect_CharacterPassive`(흡혈용)가 **신규 `UFPSRPassiveAbility` 베이스**(auto-activate-on-grant·`OnRemoveAbility` 정리) 서브클래스를 grant, 핸들은 PS 추적·런종료 ClearAbility.
  - ~~"N초 유휴 회복"(reset 루프)~~ 폐기(사용자 2026-06-20). ⚠️ **이 폐기는 *카드*의 구현 방식 판정이었다** — VIT1 의 **실드** 유휴-후-재생과 모순되지 않는다(그건 카드가 아니라 코어 기제이고, 구현도 GE 가 아니라 프리즈-멈춤 서버 누산기다).
- 🔴 **체력 회복 = 조건부. 자동재생 없음 (확정 2026-09-01, VIT1 — `Docs/Specs/VIT1_ShieldHealthTwoLayer.md`)**
  플레이어 **체력은 스스로 차지 않는다**. 살아 있는 회복 경로는 **정확히 넷**이다: ①맵 힐팩(`AFPSRHealthPickup`, 신규) ②흡혈 카드 ③부활 50%(§2-13) ④`MaxHealth` 증가분 즉시 회복(§2-13). **초당재생 카드(`DA_Card_Character_HealthRegen`)는 카드 풀에서 뺀다** — 효과 클래스(`UCardEffect_CharacterGE`)는 다른 카드가 쓰므로 코드는 남는다.
  - **대신 실드가 자동재생한다** — 피해를 안 입은 뒤 일정 시간이 지나면 차오르고, **부분 손상과 완파의 지연이 다르다**(헤일로식 이원 지연). 즉 "지속 생존력"의 담당이 체력에서 실드로 옮겨간 것이지 사라진 게 아니다.
  - ⚠️ **하강 나선 주의** — 체력이 단조 감소 자원이 되면 4인 협동에서 한 명이 계속 다운되는 스노우볼이 생긴다(부활은 50%만 준다). **힐팩 밀도와 흡혈 회복량이 이 시스템의 생사를 가른다.**
  - 🔴 **미해결(별도 행)**: `UCardEffect_CharacterGE` + periodic GE 조합은 엔진이 월드 `FTimerManager` 로 돌려 **§2-2 전역 프리즈를 뚫는다**(`Enemy.md` §2-6 이 엘리트 ASC 에 대해 금지한 것과 같은 기제인데, **플레이어 ASC 에는 그 가드가 없다**). 이번 카드 은퇴로 우회했을 뿐 조합 자체는 남아 있다.
- 🔴 **흡혈의 데미지 정의 갱신 (2026-09-01, VIT1)**: 페이로드 `DamageDealt` 가 종전 `HealthBefore - HealthAfter` 에서 **`실드소진 + 체력소진`** 으로 재정의된다 → **흡혈이 실드에 입힌 피해로도 발동한다**(사용자 결정). ⚠️ 대실드 계수가 높은 무기(저격)는 같은 명목 데미지로 실드를 더 많이 깎으므로 **흡혈량도 그만큼 커진다** — 의도된 시너지로 두되 밸런싱 대상이다.
  - ⚠️ **FF/자가 흡혈은 계속 닫혀 있다**(사용자 결정 2026-09-01). 이벤트는 `bWasEnemy` 게이트를 유지한다 — 열면 아군 실드가 손실 0 의 회복 펌프가 되고, **자폭은 FF 플래그와 독립**이라 솔로에서도 자기 발밑 폭발 → 자기 실드 흡혈 → 재생 루프가 성립한다. 아래 「협동 카드 시임」의 *"맞은 아군을 회복"* 과는 방향이 반대다.
- **서버권위**: 무기 해금·캐릭터 행동·행동 훅·이동속도 전부 서버권위(클라 보고 금지). [[freeze-gate-client-server-symmetry]]
- **협동 카드 시임 (컨설트 2026-07-01 F7 — `Docs/Review/20260701-concept-conclusions.md`)**: FF를 **켠** 세션의 치사 규격(Enemy §2-10)과 짝 = **FF 데미지를 아군 회복으로 전환하는 카드**(사선을 벌이 아니라 지원으로). ⚠️ FF 기본값은 2026-08-07부터 **OFF**라, 이 카드류는 호스트가 FF를 켰을 때만 의미가 있다. 캐릭터 행동(GAS-native, 흡혈과 동일 `ApplyDamage` 브릿지 경로 재사용 가능 — 아군 타겟 분기). **제약**: 오버힐 없음·틱/초당 힐 상한·다운 직전 세이브 용도(지속 '힐건'化 금지 = "아군을 쏴서 힐하는 게임" 붕괴 방지). 협동 유도 가성비 = 협동유도 스페셜 적(§2-6) > 어그로 룰/카드 > FF→회복 카드.
...
- **컨벤션 쿡북**: 새 카드=5분(복제) / 새 효과 타입=~40줄 1파일 / 새 무기 Fragment=15분(서브클래스+DA) / 새 무기 stat mod(기존축)=5분 / 새 무기 stat **축**=enum+`RecomputeResolved` switch case(**컴파일체크 유지**, 데이터화 금지) **+ 🔴 `ST_CardEffect` 에 `Stat.<enum 원이름>` 키**(2026-09-05 추가) — `UCardEffect_WeaponStat::GetDescription` 이 그 키를 조회하므로 **빠뜨리면 그 축을 쓰는 카드가 `IsDataValid` 에서 떨어져 풀에서 조용히 빠진다**. WITH_EDITOR 검증이라 C++ 빌드·자동화로는 절대 안 잡힌다(CRIT1 의 `ADSFieldOfView` 축에서 임포터가 실제로 잡았다). 라벨은 이득이 아니라 **숫자가 재는 것**을 적는다(감소가 이득인 축은 음수 매그니튜드로 저작 — `RecoilVertical`·`ADSFieldOfView` 선례). ⚠️ 같은 갑이 `Stat.ShieldDamageMultiplier`(VIT1 축)에 **남아 있다** / 새 캐릭터 속성=C++ AttributeSet(예약 슬롯 활용) / 새 캐릭터 패시브=`UFPSRPassiveAbility` 서브클래스 / 새 카드 스코프/오퍼타입=v2 후 동결.
...
- **슬롯 역할 분리 (확정 2026-07-29)** — 슬롯은 **아키타입별로 받는 것이 정해져 있다**. 1·2번 = 원거리 전용, **3번 = 근접(`Melee`) 전용**. 근접무기가 1·2번에 섞이지 않고, 원거리 무기가 3번을 차지하지도 않는다. 따라서 **원거리 무기 보유 상한은 2정**.
  - **3번은 근접무기가 없어도 항상 장착 가능** — 그 칸의 **기본 무기 = 맨손**(`DA_Weapon_Unarmed`: Archetype `Melee`, 메시 없음, 약한 데미지, 기존 `UFPSRGA_WeaponMelee` 재사용). "빈 슬롯을 장착 가능하게" 만든 게 아니라 **칸이 애초에 안 비는** 구조라 발사/시각/GA 경로에 예외 분기가 없다.
  - **규칙은 데이터** — 인벤토리 컴포넌트의 `SlotDefinitions`(`EditDefaultsOnly TArray<FFPSRWeaponSlotDefinition>`): 칸마다 `AcceptedArchetypes`(명시 목록 = 전역 전용 선언 / 빈 목록 = 아무 칸도 선언 안 한 나머지 전부, **슬롯 순서 무관**) + `DefaultWeapon`. 새 아키타입이 늘어도 중앙 코드는 안 바뀐다(enum+switch 금지 — [[extensibility-first-designer-tooling]]).
  - **무기 해금 카드는 아키타입별로 판정** — "원거리 칸이 꽉 찼는데 근접 칸만 비었다"면 원거리 해금 카드는 후보에서 빠진다(뽑아도 들어갈 곳이 없어 카드가 증발하는 것 방지).
- **무기 버림·교체 시스템 없음**(확정 2026-05-30) — 기존 무기로 시작해 슬롯 확장으로만 추가(최대 3). 환불/카드치환 보완 불필요
  - **예외 1건(2026-07-29)**: 3번 칸의 **맨손 → 근접무기 대체**. 맨손은 "주운 무기"가 아니라 빈 칸을 막는 기본값이므로, 근접무기를 얻으면 빈 칸을 찾지 않고 그 자리를 덮는다. 그 외 무기 간 교체는 여전히 없다.
...
- **구현 확정 (P4-B-2, 2026-06-08)**:
  ...
  - **출처 풀 라벨 (확정 2026-08-13 — 구 "무기 귀속 표시" 계획을 대체)**: 모든 제시 카드에 **어느 풀에서 나왔는지**를 표시한다 — 무기 풀 카드 = `FFPSRCardDraw.TargetWeapon->DisplayName`(현행 종류군 이름: Rifle 등), 캐릭터 풀 카드(TargetWeapon=null) = "캐릭터"(`Widget.CardEntry.SourceCharacter` 키, ko/en/ja). 이에 따라 무기스탯 설명의 `(전체 무기)` 접미는 **제거**(전체무기 스탯 카드 = 캐릭터 풀 소속 → 라벨이 스코프 정보를 대체). 구현 = `UFPSRCardEntryWidget.SourcePoolText`(BindWidgetOptional, A-1b `phase/loc-ui-migration`) + WBP_CardEntry에 TextBlock 배치(WBP 패스에서). 무기 **아이콘**(`UFPSRWeaponDataAsset.Icon` 신설 + 콘텐츠)은 후속 유닛으로 유지.
...
- **구현 상태**: FullAuto hold-to-fire 루프 = `UFPSRWeaponFireComponent`. **반동·확산 = CrystalRecoil 플러그인 어댑터**(`UFPSRRecoilComponent` 서브클래스, heat 단일소스 + `RP_*` 패턴, P0~P4 머지 `6f1a981`). 탄약(MagSize/재장전, 예비 무한)/ADS 구현됨. 조절가능 전수치 카탈로그 = `Docs/BalanceTuning_Reference.md §2`
...
- `UHeroDataAsset`(PawnData 개념) Base 구조 — DefaultWeapon, BaseAttribute, PassiveAbilitySet, FP팔/3P바디 메시. ⚠️ **미구현 계획(2026-08-11 확인)** — 그런 클래스는 아직 없고, `FPSRCharacter.h`에 *"나중에 HeroDataAsset 으로 접을 것"* 이라는 주석만 있다. 지금은 캐릭터 BP의 `EditDefaultsOnly` 값이 그 자리를 대신한다.
