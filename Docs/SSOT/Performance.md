...
>
> **📊 M0 공식 베이스라인 실측 — EC ① 충족 (2026-08-31, `docs/m0-ec1-baseline`)** — 패키지 `Packaged/26_8_30_BuildTest_1`
> (**Development**, Push Model off — §5 의 측정 구성 그대로) · 시나리오는 아래 2026-08-28 블록과 **동일**
...
> | 300 · 셀 off | 300 | **254** | 4.26ms(235fps) | 4.88ms | 2.67ms | 3.84ms | 1.23ms | 180 |
> | 500 · 셀 on  | 500 | **423** | 4.59ms(218fps) | 5.31ms | 3.82ms | 4.10ms | 1.25ms | 186 |
> | 500 · 셀 off | 500 | **428** | 4.40ms(227fps) | 5.43ms | 3.87ms | 3.79ms | 1.18ms | 175 |
>
> **판정: 통과.** 적300 평균 ≥60fps·P95 ≤20ms → **227fps · 4.82ms**(여유 약 3.8배 / 4.1배) · 스웜 렌더 ≤4ms@300 → **1.19ms**(3.4배) ·
> 적500 ≥30fps → **218fps**(7.3배). 유효성 게이트 4회 전부 통과(런 종료·다운 이벤트 0, FREEZE/RESUME 1/1 균형).
>
> ⚠️ **「요청」과 「정착 생존」이 다른 이유 — 반드시 함께 읽을 것.**
> 스폰 명령은 **요청 수를 정확히 채웠다**(`delivered 300/300` · `500/500`, 4회 전부 · 2026-08-30 `ac5ff629`). 그런데 t≈3~4초에
> 한 번에 300→260 / 500→429 로 떨어지고 이후 평평하다. 원인 = **60m 스폰 링의 일부가 아레나 바닥 밖에 걸린다.**
> `SnapToGround` 는 아래 100m 안에서 스태틱 바닥을 못 찾으면 후보 좌표를 **그대로 돌려주고**(`return Location`),
> `AcquireEnemy` 는 그 공중 좌표에 그냥 스폰한다 → 낙하 → `Z < WorldKillZ(-10,000cm)` 에서 회수. 102m 자유낙하 ≈ 3~4초로
> 관측 시점과 일치한다. 종전엔 이 회수에 **로그가 한 줄도 없어** 손실이 완전히 무음이었다(2026-08-31 `[Spawn] Kill-Z recycle:`
> 패스당 집계 로그 신설로 해소).
>
> 🔒 **사용자 결정(2026-08-31) = 이 값으로 EC ① 을 마감한다.** 즉 위 프레임값은 «요청 300/500 · 정착 259/428» 에서 잰 것이고,
> 판정선의 문자 그대로의 «적 300/500» 은 아니다. 근거 = 판정 여유가 **3.8~7.3배**라 41마리(13.7%) 차이로는 결론이 뒤집히지 않는다
> (GameThread 가 적 수에 거의 선형: 259→423 에서 2.58→3.82ms, 300 환산 ≈2.9ms · 500 환산 ≈4.5ms 로 모두 예산 안).
