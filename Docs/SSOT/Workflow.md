...
- ⚠️ **이 파이프라인의 구조적 약점 = C1 설계자와 C3 판정자가 같은 Fable이다.** 자기 설계를 자기가 통과시키는 셈이라, **독립된 시각은 C3의 레드팀 서브에이전트 하나뿐**이다(§6-6-1). 그래서 레드팀에 설계 근거를 넘기지 않는 화이트리스트가 선택이 아니라 **게이트 성립 조건**이다 — 어기면 검증 단계 전체가 자기검토가 된다.
...
**어떻게** — 검증 주체(코어=Fable / 그 외=Opus)가 `Agent` 도구로 **`model="fable"` 레드팀 서브에이전트**를 띄운다. 프롬프트 원문 = **[`Docs/InternalRedTeamReview.md`](../InternalRedTeamReview.md)**(페르소나 + 출력 계약 + P1/P2/P3 심각도 정의).

**🔒 독립성 화이트리스트 — 이게 게이트의 전부다.** 설계한 본인이 자기 설계를 공격하면 게이트는 헛돈다. 그래서 레드팀은 **깨끗한 컨텍스트의 별도 인스턴스**여야 하고, 아래만 받는다:

| | |
|---|---|
| ✅ 준다 | diff 원문(`git diff main...HEAD` 등) · `Docs/InternalRedTeamReview.md` · 해당 `Docs/Specs/<유닛>.md`(있으면) · 리포 읽기 권한(인용 근거를 대야 하므로) |
| ❌ **안 준다** | **설계 근거 · 명세를 그렇게 쓴 의도 · 토론 이력 · "왜 이렇게 했는지"** — 주는 순간 앵커링되어 자기검토가 된다 |
| ⚠️ 명세 취급 | 명세는 **전제가 아니라 안건**. "명세대로 구현됐나"(정합성 대조)는 검증 주체가 직접 하고, 레드팀은 **명세 자체가 틀렸을 가능성**을 본다 |
...
- **컨설팅 토론(제3 채널, ConsultLoop)**: 위 둘과 별개로, 사용자가 주제를 지목하면 **안건 소유자(Claude) × 웹/앱 적대 레드팀(Codex)** 라이브 토론으로 설계·구조를 자문받는다. 프로토콜=`Docs/ConsultLoop.md`, 페르소나 원문=`Docs/CodexRedTeamPersona.md`, 호출=`Scripts/consult-codex.ps1`(`codex exec` — 래퍼가 페르소나를 자동 prepend), 트리거=`/consult <주제>`·`/plan-consult <작업>`, 산출=`Docs/Review/`(추적, **프롬프트 매니저 `TaskPrompts_Master.md` §E가 읽어 백로그 인입**).
  **최종 결정권은 안건 소유자(Claude)** — 언제든 토론을 끝낼 수 있으나 `종료 사유` + `기각 원장`(근거 = 제1원리 조항/코드 인용/실측치)을 남겨야 성립(ConsultLoop §3-1). **자문 전용**(코드 무변경) — 채택 설계는 이 문서/도메인 SSOT를 먼저 갱신 후 구현(§6-4)하며, HIGH_RISK는 여전히 사용자 승인 뒤.
  ⚠️ **머지 게이트가 내부로 옮겨간 뒤, 이 토론 채널이 프로젝트에 남은 유일한 외부 모델(Codex) 교차검증 지점이다.** 여기서까지 빼면 Claude 아닌 목소리가 0개가 된다.
