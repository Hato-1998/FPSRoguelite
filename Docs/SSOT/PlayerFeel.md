...
> 🔄 **2026-09-13 재재정정 — [ADR 0018](../Architecture/0018-first-person-dedicated-arms-mesh-restored.md): 1인칭은 전용 팔 메시를 다시 그린다.** 아래 0015 항목의 "총만 그린다"는 **폐기**다. `FirstPersonArms` 는 `Hidden in Game = false` 로 **보이는 팔**이고, 몸은 `bOwnerNoSee` 로 소유자 시점에서만 숨는다(머리 본 숨김 `HeadBoneName` 은 None 으로 퇴역 — 아래 True First Person 항목도 그만큼 무효). 스켈레톤은 3인칭과 **공유**하고, 시점 차이는 **메시 소켓 오버라이드 + 1P 전용 그립 오프셋 필드**로만 낸다. 손 IK 는 왼손만 재가동(핸드가드 `SOCKET_LeftHand`), 오른손은 총 앵커 CopyBone 유지.
>
> 🔄 **2026-09-04 재정정 — [ADR 0015](../Architecture/0015-first-person-gun-only-hidden-arms-driver.md): 1인칭은 총만 그린다.** ⛔ **대체됨(→ 0018), 아래 내용은 이력으로만 읽을 것.** `FirstPersonArms` 는 살아 있되 `Hidden in Game` 으로 숨긴 **모션 드라이버**다(총은 여전히 팔의 `ik_hand_gun` 에 붙어 PWAS 리타깃 모션을 받는다). 손 IK 는 무기 DA 양손 그립 소켓 None 으로 퇴역. 0006 의 리그·리타깃 체인은 유지, "팔을 그린다"는 절반만 대체.
...
- **조준 중에는 느려진다 — 그리고 그 감속은 예측된다**(ADS1, 2026-09-12): 조준 중 걷기 상한에 무기가 정한 배율을 곱한다(`FFPSRWeaponStatBlock::ADSMoveSpeedMultiplier`, 기본 **0.5** → 서서 900→**450**, 웅크려 300→**150**). ADS가 없는 무기(`bHasADS=false`)는 감속 0. **조준 의도는 `FLAG_Custom_0`으로 이동 패킷에 실려** 클라와 서버가 같은 move에서 같은 속도를 쓴다 — 플래그 없이 RPC로만 알리면 조준할 때마다 고무줄이 난다(불변식 2). 곱하는 지점은 `GetMaxSpeed()`(백페달 배수와 같은 층)이라 **서기·웅크림 둘 다** 덮이고, **슬라이드 중·공중에는 걸리지 않는다**(둘 다 별도 속도 경로). 상세·수용된 위험(무기 교체 창) = `Docs/Specs/ADS1_AimWalkSpeedPrediction.md`. 3P 조준 이동 애니(조준 전용 블렌드스페이스 2개 + `ABP_TPS` 2×2)는 그 뒤 콘텐츠 작업.
...
> 🔴 **부활 경제 추가 2026-09-10 (코인 경제 · `RunFlow.md` §2-2-1 (5))** — 부활 경로가 **셋**이 된다.
> ① **근접 아군 부활 = 무료**(아래 `UFPSRReviveComponent` 그대로) ② **원격 아군 부활 = 상점에서 코인**(멀리 있는 아군을 즉시) ③ **자가 부활 = 자기 코인, 누진, 상한 5코인**(밸런스 미확정) — 🔒 **코인이 없으면 부활 불가.**
> ①과 ②는 대체가 아니라 **선택지**다(가까우면 걸어가서 공짜, 멀면 돈으로).
> ⚠️ **아래 두 규칙이 재정의된다** — *"솔로 다운 = 즉시 Defeat"* 와 *"팀와이프 = 생존(Alive) 0 → 전원 Dead → EndRun(Defeat)"*. 코인을 쥔 채 다운되면 솔로도 살아난다.
> ⚠️ **`PostFreezeInvulnSeconds`(카드선택 프리즈 종료 후 grace)는 존재 이유가 사라진다** — 프리즈가 폐기되기 때문이다. `PostReviveInvuln`(부활 직후 무적)은 그대로 산다.
...
#### 2-13-1. 플레이어 디버프 + 오브 사격 해제 (신설 2026-09-10, 사용자 결정)

> 지금까지 상태이상(STAT1)은 **적에게만** 걸렸다(`UFPSREnemyHealthComponent::StatusBits`). 이 절이 **반대 방향**을 연다.

**(1) 무엇이 걸리는가 — 약함 4종만**
- 플레이어에게 걸리는 것은 STAT1 카탈로그의 **약함(Weak) 4종**: **둔화 · 지속피해 · 방어저하 · 공격속도저하**. 4종이 **동시에 전부 걸리는 것은 허용**한다.
- 🔒 **강함(Strong) 2종(실명 · 속박)은 플레이어에게 쓰지 않는다. 약화 변환도 하지 않는다**(속박→강한둔화 같은 대체 없음). 근거 = 적 200~300이 몰려오는 1인칭에서 **이동 불가는 사망 선고**이고, 실명도 같은 부류다.
- 카탈로그를 둘로 쪼개지 않는다 — **같은 자산에 대상별 필터**(플레이어 경로는 `Weak` 만 통과)를 둔다.

**(2) 누가 거는가**

| 주체 | 비고 |
|---|---|
| **상태이상 무기를 든 아군의 사격** | 그 무기에 달린 디버프가 **그대로** 아군에게도 걸린다(전용 항목을 새로 만들지 않는다) |
| 특정 **엘리트** · **보스** · **강화 일반몹** | |
| **디버프 전문 일반몹** | 체력이 약한 대신 디버프를 건다 |

- 🔒 **친선사격(FF) 플래그와 무관하게 아군 사격의 디버프는 통과한다**(데미지는 FF 설정대로 0). 그러지 않으면 FF 기본값이 OFF 라(2026-08-07) **기본 세션에서 한 번도 발동하지 않는 죽은 기능**이 된다. `CombatWeaponCard.md` §2-3-5 가 FF 를 닫아 둔 이유는 *"아군을 쏘면 회복 펌프가 된다"* 는 악용 유인이었는데, **디버프는 벌칙이라 유인 방향이 반대**다.
- 🔴 **자기 자신은 제외한다.** 자폭(로켓 점프 등)은 FF 설정과 독립이라(`FPSRCombat::ResolveDamage` 의 `Target == Instigator` 분기) 게이트를 안 두면 **자기 폭발로 자기 디버프가 걸린다.** VIT1 C3 가 히트마커에서 밟았던 것과 같은 함정이다(§2-14).

**(3) 해제 미니게임 — 오브 사격**
- 디버프가 걸려 있는 동안 **플레이어 주위 바닥에서 디버프 아이콘 오브가 솟아오른다.** **본인이 쏘아 맞히면 잔여 시간이 줄어든다.**
- 🔒 **오브는 내 화면에만 보인다** — 로컬 전용, **복제 0**. 4인 × N개가 월드에 상주해 스웜 가독성을 먹는 문제가 원천 소멸한다.
- 🔒 **적중은 클라, 권위는 서버.** 클라는 **「오브 id 를 맞혔다」만** 보내고, 서버가 ① 그 플레이어의 오브인가 ② 아직 살아 있는가 ③ 이미 소진되지 않았는가 를 검증한 뒤 **감소량을 서버가 정한다.** 클라가 보낸 시간값은 쓰지 않는다 — 그러지 않으면 클라가 자기 디버프를 마음대로 지울 수 있고, `CombatWeaponCard.md` §2-3-5 의 *"서버권위 … 클라 보고 금지"* 와도 어긋난다.
- 🔴 **전용 콜리전 채널이 필요하다**(`FPSRCollisionChannels.h`) — 오브가 적과 같은 채널에 있으면 **스웜 사격을 가로챈다**.
- **히트마커** — VIT1 C3 이 `bTargetIsPlayer` 로 「플레이어를 때리면 마커 없음」을 명시 게이트했다(§2-14). 자기 디버프 오브가 어느 쪽인지 정할 것.

🔒 **착수 전 사용자 토론 필수**(2026-09-10 지시) — 보드 행 「[기획] 디버프 오브 사격」.

### 2-14. 게임필 / 피드백 / 공간 지각 (확정 2026-05-30)
> 🔴 **위협 인디케이터 통합 2026-09-10 (사용자 결정)** — **조준점 링은 채택하지 않는다.** 아래 「화면 테두리 방향성 위협 인디케이터」와 §2-14 구현 상태의 「피격 방향 인디케이터」를 **하나의 테두리 위젯**으로 합치고, **색으로 구분**한다: 🔴 **빨강 = 피격 방향** / 🟠 **주황·노랑 = 나를 겨누는 적**.
> **신규 서버 상태 0 · 신규 복제 0 · 신규 RPC 0** — 배관이 이미 있다. `UFPSRPlayerFeedbackComponent` 의 `OnDamageDirection`(피격)과 `OnRangedTargetWarning`(예고, 활성 소스별 카메라 상대 각도 배열을 매 프레임 갱신)을 **같은 위젯의 두 색 채널**로 구독하면 된다.
> 🔑 **하드캡은 이미 게임플레이가 강제한다** — [ADR 0013](../Architecture/0013-enemy-tier-axis-and-elite-gas.md) C1 / 커밋 `f5b0a78d`(2026-08-14 「전 몬스터 원거리화」) 이후 **모든 적이 원거리**이고 근접 접촉 데미지 축은 죽은 코드로 제거됐다(C0). 그리고 `UFPSREnemySpawnSubsystem::RangedAttackTokenLimit = 3` 이 **플레이어당 동시 충전 적을 3마리로** 묶는다 → 아래 *"시각 위협 큐 하드캡 = 동시 ≤3"* 과 **정확히 일치**한다. 별도 UI 캡을 설계할 필요가 없다.
> ⚠️ `AFPSREnemyBase::AttackRange`(150cm)는 **클라 코스메틱(근접 tell 애니메이션) 판정에만** 남은 잔존 필드다 — 데미지 경로가 아니다. 이 필드를 위협 판정에 쓰지 말 것.
> 정할 것 = 색·수명·우선순위(피격이 예고를 덮는가). 🔒 **착수 전 사용자 토론 필수**(2026-09-10 지시).
...
- 🔴 **실드 파손 피드백 (확정 2026-09-01, VIT1)** — 3방향 전부 **신규 복제 프로퍼티 0 · 신규 RPC 0** 으로 만든다(전부 이미 복제되는 값의 `OnRep` 파생이다).
  ...
  | **본인** (내 실드가 깨졌다) | 위험 경고 | 클라가 복제된 `Shield` 어트리뷰트에서 스스로 0 교차를 판정 → `UFPSRPlayerFeedbackComponent::OnShieldBroken`(`BlueprintAssignable`, 페이로드 없음). 서버 RPC 불요. 🔁 **이관(2026-09-02, `feat/hud-surface`)** — 종전 GMS 로컬 pub/sub(`Message.Player.ShieldBroken`, 페이로드 `FFPSRCosmeticEventMessage`)에서 옮김: `UFPSRGameplayMessageSubsystem` 은 `Get`/`RegisterListener` 가 전부 C++ 템플릿이라 `UFUNCTION` 이 없어 WBP 가 구독할 수 없었다(`Docs/VIT1_ContentGuide.md` §5 ⑥-④). 🔴 **훅 = `ASC->GetGameplayAttributeValueChangeDelegate(Shield)`** (`AFPSRCharacter::HandleShieldValueChangedForWarning`), **`UFPSRHealthSet::PostAttributeChange`/`OnShieldBroken` 이 아니다** — 후자는 그 클라가 해당 어트리뷰트의 애그리게이터를 들고 있을 때만 도는 경로라(엔진 `GameplayEffect.cpp:3682` `SetBaseAttributeValueFromReplication` 의 else 분기는 `AttributeValueChangeDelegates` 만 브로드캐스트한다) **원격 협동 플레이어가 경고를 못 받는다**. C3 전체대조에서 잡힘(2026-09-02) |
...
- 🔴 **HUD 레이아웃 레퍼런스 (확정 2026-09-02, 사용자 제공 스크린샷 2종)** — 지금까지 §2-14 는 *무엇을 보여줄지*(위협 큐·마커·인디케이터)만 정하고 **화면 어디에 어떻게 놓을지**는 비어 있었다. 그 자리를 사용자가 레퍼런스로 확정했다.
  | | 레퍼런스 | 좌하단(본인 바이탈) | 우하단(총기) |
  |---|---|---|---|
  | ① | **Apex Legends** (Game.md §1 의 움직임·사격 레퍼런스와 동일 작품) | 실드(위) + 체력(아래) 가로 바 · 레전드 아이콘 | 무기 실루엣 + 장탄수 + 슬롯 |
  | ② | 로우폴리 카툰 FPS (**작품명 미기재** — 사용자 제공 스크린샷 2번) | 캐릭터 초상 + 실드/체력 2단 가로 바 · 🔴 **게이지 우측에 `현재/최대` 숫자 병기** | 무기 아이콘 + 탄약 |
  - ⚠️ **① Apex 칸 주의**: Apex 실드는 등급별 **세그먼트 바**(미장착이면 X 표시)다. 우리 실드는 float 어트리뷰트 1개라 세그먼트 개념이 없다 — 그대로 베끼지 말 것.
  - **채택** = 두 레퍼런스 공통의 **좌하단 바이탈 / 우하단 총기** 배치 + ②의 **숫자 병기**. 게이지만으로는 "얼마나 남았나"가 안 읽히고, 숫자만으로는 한눈에 안 들어온다.
  - 🔴 **실드가 체력 위** — 실드가 먼저 닳고 초과분이 체력으로 이월되는 2층 구조(VIT1)라, **시각 순서가 소모 순서와 같아야** 읽힌다. 양 레퍼런스도 그 순서다.
  - **총기 HUD 요구 3종**: **무기 형태(아이콘)** · 장탄수 · **서브 무기 존재유무**(무기는 숫자키 1/2/3 슬롯, `GetOwnedWeapons()` 가 소유분만 슬롯 순서로 준다). 🔴 **예비탄약 칸은 불필요** — `FFPSRWeaponStatBlock` 이 `reserve ammo is infinite (always refills to MagSize)` 로 이미 결정했다. 현재탄 / `MagSize` 만 그린다.
...
- 🔴 **HUD 아트 = Synty `UISciFiSoldierHUD` 팩 채택 (2026-09-03, 사용자 지시)** — 위 레퍼런스가 정한 건 **배치**였고, **룩**을 정하는 기록은 여기까지 없었다(팩은 아트 스택 커밋 `ff191fec` 에 딸려 들어와 있었을 뿐). 이 항목이 그 결정이다.
  - ⚠️ **월드 아트 방향과의 관계** — 같은 날(2026-09-03) 월드 모티프는 **레트로 아케이드·복셀**로 전환됐고 종전 Synty 셀/툰 락은 폐기됐다(`ArtDirection.md` 확장 트랙). **이 항목은 그 전환 이후에 사용자가 지시한 것이고, 대상은 월드가 아니라 스크린 공간 UI 레이어다.** 둘이 충돌하지 않는 근거 = 색 SSOT `§A-3-7` 의 UI 대역은 *모티프와 무관한 가독성 공학*이라 아트 방향이 바뀌어도 살아남는다. **HUD 룩을 나중에 아케이드/복셀 톤으로 다시 입히더라도 아래 배선(세터 호출·값 소스·가시성 규칙)은 그대로 산다** — 갈아끼우는 것은 자식 위젯과 색뿐이다.
