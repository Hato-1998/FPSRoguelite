...
```
- 🔴 **레벨 구조 전면 재결정 (사용자 확정 2026-08-16 — [ADR 0010](../Architecture/0010-arena-topology-and-stage-transition.md))**
  현행 = **고정 크기 단일 아레나(160 × 160 m · 단일 평면 · 다중 코어 교차 동선) + 억제기 파괴 → 스테이지 전환**
  (치수는 [ADR 0011](../Architecture/0011-authored-skeleton-and-arena-validator.md) E1 로 80×80 에서 갱신).
  예비 아레나는 같은 레벨의 셀 격자가 겹치지 않는 자리에 미리 배치해 두고, 전환 시 가시성·콜리전 토글 +
  플레이어 순간이동 + 마스크 교체로 바꾼다(서브레벨 미사용 — ADR 0010 D6 / ADR 0011 E2, 2026-08-17).
  아래 두 가지가 **폐기**됐다 — 읽는 사람이 현행으로 오해하지 않도록 명시한다:
  - **다중맵 심리스 폐기** (맵 4방향 문 → 인접 맵 스트림-in). U 연속필드 코드(`34b5eea`)는 **존치·미사용**이며 제거하지 않는다.
    아래 "맵 다양성"·"#3" 항목 4개는 **경위 보존용**이고 현행 설계가 아니다.
    계기 = `L_Map1_City`(450×495 m)가 플로우필드 셀 36,200/40,000(90.5%)을 써 **다중맵이 이미 물리적으로 불가능**했다.
  - **"절차적 생성 PCG 사용 안 함" 뒤집힘** → **골격 저작 + 내부 프롭·구조 절차**. 기각 근거였던 *"동적 생성 시
    Flow-Field 이점 소멸"* 은 **런타임 동적 생성**을 겨눈 것이고, 로드 시점 1회 생성 후 굽는 구조에는 해당하지 않는다.
    ⚠️ 더 나아가 0010은 마스크를 트레이스로 발견하지 않고 **생성기가 `FFPSRFlowFieldSurfaceData`를 직접 산출**한다.
  - 스테이지 전환 상세(억제기 = 파괴물 + 리워드 · 그레이스 창 · 예비 아레나) = ADR 0010 D6·D7.
- **고정 맵 / 고정 구조** (~~절차적 생성 PCG 사용 안 함~~ → 🔴 위 항목으로 갱신)
  - **맵 다양성 (기획 추가 2026-06-10 · 다중맵 심리스로 갱신 2026-07-03 · 🔴 폐기 2026-08-16)**: 단일 맵은 로그라이트 재플레이 피로 위험 → **복수의 authored 고정 맵**(각각 Flow-Field 사전계산). PCG 미사용 유지(성능·사전계산 근거 — 동적 생성 시 Flow-Field 이점 소멸).
    ...
    - **미션/스폰 = 플레이어 점유 맵 기반**: 파티가 여러 맵에 분산되면 **점유된 각 맵에서 독립적으로 미션·스폰이 출현**(상세 §2-8). 맵 간 적 예산 배분 = U 연속필드(구현·머지 완료 `34b5eea`)로 해소(전역 공유 캡, 아래 U 항목).
    - **#3 다중맵 아키텍처 = 원안 (2026-07-05, Codex 5R 컨설트 `Docs/Review/20260705-multimap-budget-regroup.md`)** — ⚠️**기구(per-map allocator·레지스트리)는 U 연속필드로 대체됨**(다음 항목; **명제·디렉터 결정은 U에 계승**): **명제 = 다중맵은 자유를 주되 본게임 압력·보상·burst는 "2명 이상 같은 전선"에서 선명하게 켜진다. 솔로 분산 = 정찰 모드. 하드 테더 없음.** (a) 분산 = 허용 + 암묵 유도(공유 프리즈·예산 thinning·per-map 미션·DBNO 근접부활 + 선택적 rally pad — 강제 아님), (b) 적 예산 = **전역 공유 캡**(맵 무관 호스트 전역, per-map 금지=붕괴; 잠정 전역 200, perf 미검증) → 단일 `FPSREnemySpawnSubsystem`을 map-aware allocator("2인+ 맵 > 솔로 맵" 가중)로·U7 플로우필드를 per-map 레지스트리로 확장, (c) 복제 = NetCull(1순위 미구현 레버) → RepGraph → level-streaming/WP Data Layer(`Performance.md §5`). **Tier 0 코어 = U 연속필드로 구현·머지**(다음 항목 · `Architecture.md §4-1`) / Tier 1(예산 게임필: burst reserve·silent recycle·**콘텐츠-aware allocator 정책**)·Tier 2(텔레포터·split 감지·은근한 비효율)=후속. **디렉터 결정(2026-07-05, 리포트 §✅)**: 솔로=정찰(약억제, per-map 미션이 이미 억제) · **분산 판타지=하이브리드**(짧은 정찰=자유/장기 완전분산=은근 비효율, 명시 확정) · 2인+=효율 좋음(강제 아님; **"2인+"=파티크기 아닌 뭉침 임계값 2·3·4 전부, 게임=1~4인**) · **그룹 버프 전면 폐기**(뭉침 유인=콘텐츠 배분만) · 텔포=장치 활성화·대상 지정·쿨타임(+도착 앵커/채널/shared 쿨타임) · 문=혼자선 어려운 체력 · 맵=언로드X·LOD컬·잔존(픽업 dormant/HISM 잔존·**적만** 빈맵 드레인). ⚠️ **allocator가 이 설계의 심장**: 그룹 버프가 0이라 "뭉치면 효율"(#2)은 고가치 콘텐츠(미션/보스/엘리트/연쇄)를 2인+ 그룹에 자연 집중시켜야만 성립(애매 시 1/1/1/1 저층파밍이 최적→붕괴).
    - **#3 U 대전환 — 연속 크로스도어 플로우필드 (2026-07-07, `/plan-consult` Codex 3R → `Docs/Review/20260707-plan-continuous-field-arch.md`)**: 위 2026-07-05 설계의 per-map 경계·전환추적자가 심리스 연속 추격과 근본 충돌 → **U(고정 3×3 단일 그리드)로 피벗**. **allocator GroupBonus 키 = `MapId` → `FrontId`**(열린문으로 연결된 전투 포켓). ⚠️신규 실패모드: all-open 후반 멀리 떨어진 1/1/1/1이 한 front로 오인 → 보너스에 **graph-distance/전투포켓/미션앵커 근접 가중**(연결=전투가능 gate, 보너스=실제 같은 전투권). **`FrontId`는 gameplay 소비 추상화로 유지**(단일배열 index를 gameplay에 누수 금지 = U↔P 회귀 비용 억제). **D1 확정(2026-07-07)**: 전투 슬롯 100~132m/변 → 200cm 셀 기준 U 유효(캡 40000셀 이내, near-cap; 재계산 예산 `Performance.md §5`). **구현 완료·main 머지 `34b5eea`**(FrontId allocator·origin connectivity 게이트·TopologyGeneration ack 라이브; 순서·제거목록 = 리포트 §2-4 · `Architecture.md §4-1`).
    - **코어 검증은 단일 맵(~~L_Sandbox~~ → **`L_Map1_City`**, 🔁 *정정 2026-08-13*: `L_Sandbox`는 실재하지 않는다 — 현행 유일 인게임 맵은 `L_Map1_City`(`dca19b4e`))** 유지, 다중맵 = 콘텐츠/P6+ 단계(Synty 파일럿 검증 후 착수 — `Roadmap.md §8`).
...
- **레벨업 시 전역 프리즈 (확정 2026-06-04, "예전 방식" 복귀 — 기존 "교전 중 프리즈 폐지"를 되돌림)**: 공유 풀이 레벨업 임계 도달 시 **그 즉시 게임이 전역 프리즈**된다 — **적·플레이어 모두 정지**(적은 제자리 동결, 스폰 정지, 플레이어 이동·발사 차단). 연결된 **전원이 본인 카드를 모두 고르면 재개**된다.
  ...
  - 🔑 **프리즈가 멈추는 것은 입력과 게임플레이 틱이지 애니메이션이 아니다** (명문화 2026-08-19). 종전 이 절이 애니메이션을 언급하지 않아 *"보스가 프리즈 중에도 정지하지 않는다 = §2-2 게이트 대칭 구멍"* 이라는 **오진이 실제로 보드 행으로 올라왔다**(PIE 재현 결과 = 아이들 애님만 재생 → 오진 확정). 세 주체가 전부 같은 규칙이다:
    | 주체 | 프리즈 중 멈추는 것 | 계속 도는 것 |
    |---|---|---|
    | 플레이어 | 입력 전부(이동·사격·장비·대시) — `AFPSRCharacter::IsRunFrozen()` 게이트 | AnimBP(아이들 포즈) |
    | 스웜 적 | 이동·공격 배치 패스 통째 + 스폰·드레인 — `FPSREnemySpawnSubsystem` early return 2곳 | **VAT 아이들 클립** — playrate 0은 *거리* LOD(`FPSRVATAnim::AnimFreezeRadiusSq`, `FPSREnemyBase.cpp`)에서만 걸린다 |
    | 보스 | (멈출 이동·공격이 아직 없다 — `AFPSRBossBase`는 정지 스캐폴드) | 스켈레탈 아이들 |
    - ⚠️ 위 *"적은 제자리 동결"* 은 **위치가 고정된다는 뜻이지 포즈가 정지한다는 뜻이 아니다.** 이 한 단어가 오진의 출발점이었다.
    - 애니메이션까지 멈추는 것(시간정지 느낌)은 **미채택 상태의 설계 선택지**이지 미구현 결함이 아니다. 바꾸려면 적 200-300의 애님을 일제히 정지시키는 비용과 VAT 렌더 경로(`Docs/Architecture/0007` CPD 채택)와의 정합을 함께 봐야 하므로, 바꾸고 싶으면 **별도 결정 행**으로 연다.
  - **설계 근거 / 향후 대안 (확정 2026-06-10)**: 프리즈 채택의 제1원리 = ① 빌드 선택이 전투 결과를 좌우하는 로그라이트에서 *선택의 무게*를 보장(실시간 선택은 스웜 압박에 묻힘) ② 서버권위 카드추첨·복제 동기화가 정지 상태에서 가장 단순·안전 ③ 4인 빌드 공유 순간 형성. 트레이드오프 = 잦은 레벨업 시 스톱-스타트 페이싱 → **완화책: 후반 레벨업 임계 XP를 가파르게 ↑(프리즈 빈도 억제), 보스전 프리즈 정책은 플레이테스트(§7-5) 후 재검토**. **향후 대안(후속 평가, 현 단계 미구현)**: 경미한 강화는 프리즈 없이 화면 2지선다(**Q/E** 비동기 선택)로 전환하고, 무게 큰 선택(무기/Fragment/다중레벨)만 프리즈 유지하는 **하이브리드**. 현 단계는 전역 프리즈로 진행.
...
- **구현 상태(U3/U4 스캐폴드, 2026-06-21 머지)**: `AFPSRBossBase : ACharacter`(StateTree/GAS 미장착·정지 box, 재부모화 불요 구조) + `UFPSRBossDefinitionDataAsset`(체력 주도) + 전용 `AFPSRBossSpawnPoint`. **`UFPSREnemyHealthComponent` 재사용**으로 전 무기경로(히트스캔/투사체/차지/근접/폭발)가 데미지·크릿·FF·약점(U3a) 적용(신규 데미지코드 0), 처치→`FPSRGameMode::NotifyBossDefeated`→`EndRun(Victory)`. 콘텐츠=BP_Boss(약점2)+DA(체력3000)+~~L_Sandbox~~ 스폰포인트+체력바 (🔁 *정정 2026-08-13*: 콘텐츠 3종은 `Content/Character/Boss/`에 실존하나 **맵이 실재하지 않는다** — `L_Sandbox`는 없고 현행은 `L_Map1_City`다. ⚠️ **`AFPSRBossSpawnPoint`가 `L_Map1_City`에 실제로 배치됐는지는 미확인** — (a′) 배치 완료 목록(`Roadmap.md` §7-6)은 PlayerStart·플로우필드 볼륨·적 스폰포인트 18·프롭만 열거하고 **보스 스폰포인트가 없다**. 보스 런 검증 전에 확인할 것). **실보스(이동·StateTree·GAS 스킬·2페이즈)는 장기 백로그** — 위 Base 구조(StateTree/GAS)는 그 목표 비전.
...
- 🔴 **첫 앵커는 `StageIndex 0` 의 항등 앵커(`+0, ×1.0, ×1.0`)로 시작하라.** 첫 앵커 아래는 flat clamp 이므로,
  첫 앵커를 `(3, +8, ×1.15, ×1.6)` 처럼 두면 **스테이지 0 이 이미 ×1.6 내구도로 시작하고 스테이지 0→3 의
  억제기 파괴 3회가 난이도를 전혀 올리지 않는다** — 이 축의 전제("억제기 파괴가 난이도를 올린다")가 앞
  세 스테이지에서 죽는다. 의도적으로 그렇게 저작할 수도 있어 밸리데이터는 **Warning** 으로만 짚는다.
- **인원수는 스테이지 시작에 1회 고정**하고 생존자가 아니라 **참가자**를 센다(DBNO 악용 차단). 근거 = ADR 0010 D6 정정 ③.

> ✅ **마릿수 축의 실효 천장 = 240(갱신 2026-08-31, M1 캡 상향).** `UFPSREnemySpawnSubsystem` 의 `GlobalAliveCap`(250) −
> `SeedReserve`(10) = **240**(컴파일 타임 상수). 스케줄의 `MaxAliveCount` 기본값도 **240**으로 함께 올라 정렬됐다 —
> 종전 "300 vs 192"처럼 스케줄값이 천장 위로 죽어 있던 구간이 **사라졌다.**

> **재계산**: 현행 저작 `AliveCountByLevel` = (1,40)·(20,90)·(30,140), `StageDifficulty` 마지막 앵커 =
> (10, +24, ×1.45). 레벨 30·스테이지 10 이면 (140+24)×1.45 = 237.8 → **238**로 천장(240) **아래**다(코드 `FMath::RoundToInt`, `FPSRRunDirectorSubsystem.cpp:281` — 반올림이지 내림이 아니다).
> 즉 **이제 곡선이 천장에 닿지 않는다**(종전엔 192에서 잘렸다) — 마릿수 축이 저레벨·초반 스테이지뿐 아니라
> **전 구간에서 실효를 갖는다**는 뜻이다. 억제기 내구도 축은 이 천장과 무관(불변).

> **캡 상향 + `MaxAliveCount` 드리프트 정리 = 완료**(이 행이 그 작업이다, 2026-08-31).
