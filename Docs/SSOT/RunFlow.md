...
- **고정 맵 / 고정 구조** (절차적 생성 PCG 사용 안 함)
  - **맵 다양성 (기획 추가 2026-06-10)**: 단일 맵은 로그라이트 재플레이 피로 위험 → **복수의 authored 고정 맵**(각각 Flow-Field 사전계산)을 목표로 하고 런 시작 시 선택/랜덤. PCG 미사용은 유지(성능·사전계산이 근거 — 동적 생성 시 Flow-Field 사전계산 이점 소멸). 맵 수·해금 방식은 콘텐츠 단계(P6+) 확정, **코어 검증은 단일 맵(L_Sandbox)** 로 진행.
