...
> 🔒 **내부 순서 강제**: (a′)를 먼저 끝내고 (b)를 측정한다. 이 순서는 프로즈가 아니라 **보드 `선행작업` 릴레이션으로 건다**((1)의 "순서 = 선행작업" 전제 — 안 걸면 전제가 거짓말이 된다).

- (a) **드리프트 정정 — 전수** ⚠️ *범위 정정 2026-08-11(레드팀): §7-3만 보면 부족하다.* 대상 = **§7-3 완료 표기 + §7-5 판정 기록 + §8 인벤토리 + 도메인 SSOT**. 이미 확인된 실물 불일치: §8·`Game.md §9`가 인용하는 **`L_Sandbox`가 실재하지 않고**(`Content/Maps` = L_Lobby·L_MainMenu·L_Transition·Map_CyberCity·TestWorld), `CombatWeaponCard.md §2-3`이 *"현행 코드는 v1 단일효과"* 라 적었으나 **코드는 v2**다(`UFPSRCardEffect` 실존, `ECardScope` 참조 0).
- (a′) **인게임 맵 확정** ⚠️ *누락 정정 2026-08-11(레드팀): 종전 이 항목이 정본에 없어 (b)를 무효화할 경로가 열려 있었다.* 맵을 갈면 **플로우필드 베이크·스폰포인트·미션존·성능 베이스라인이 전부 무효**가 된다 → **유지할 맵 위에서만 (b)를 측정한다.** ⚠️ **교체 여부·대상은 사용자 결정 대기**((5)-5) — §8 아트 스택은 `Synty Cyber City(맵1)`(2026-07-10 확정)이고 보드의 "Synthwave City Kit"은 **SSOT에 기록된 적 없는 결정**이다.
...
① `Performance.md §5`에 **측정 빌드 구성이 명기된** 적 300/500 실측 프레임값이 기입됨(추정치 아님) — 그 값을 측정한 맵 = (a′)로 확정된 맵
...
| 5 | **인게임 맵 확정** — 현행 `Map_CyberCity` 유지인가 교체인가. 보드의 "Synthwave City Kit"은 **SSOT에 기록된 적 없는 결정**이고 §8 아트 스택은 `Synty Cyber City(맵1)`(2026-07-10 확정)다. 교체면 §8도 함께 갱신 | **M0 (b) 착수 전** (레드팀 신설) |
...
| 환경/레벨 지오메트리 | ⚠️ **정정 2026-08-11(레드팀)**: 종전 "L_Sandbox 화이트박스"는 **실재하지 않는다** — `Content/Maps` = `L_MainMenu`·`L_Lobby`·`L_Transition`·**`Map_CyberCity`**(유일 인게임 맵)·`TestWorld`. 현행 = `Map_CyberCity` | **Synty POLYGON Sci-Fi Cyber City**(맵1 베이스; 2맵 Nature·3맵 Space=다중맵 단계). **전체 셀/툰 통일**(§1-C-9). 다중맵 심리스(`RunFlow.md §2-1`). ⚠️ 맵 교체 여부 = §7-6 (5)-5 사용자 결정 대기 |
