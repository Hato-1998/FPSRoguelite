...
- (a′) **인게임 맵 확정** ⚠️ *누락 정정 2026-08-11(레드팀): 종전 이 항목이 정본에 없어 (b)를 무효화할 경로가 열려 있었다.* 맵을 갈면 **플로우필드 베이크·스폰포인트·미션존·성능 베이스라인이 전부 무효**가 된다 → **유지할 맵 위에서만 (b)를 측정한다.** **교체는 이미 결정됐다** — `Map_CyberCity` 폐기 → Fab **Synthwave City Kit** 데모맵 기반 재구축(사용자 결정 2026-08-11; §8에 전파 완료). ~~⚠️ **블로커 = 사용자의 Fab 에셋 임포트**라 (b)는 그때까지 열리지 않는다.~~
  ...
  - 🔴 **재개 2026-08-16 — (a′) 다시 열렸다 (사용자 판정 · [ADR 0010](../Architecture/0010-arena-topology-and-stage-transition.md)).**
    `L_Map1_City`가 **플레이 감각에서 기각**됐다 — ① 너무 넓다(450×495 m, 압박·밀도 없음) ② 동선에 선택지가 없다 ③ 룩이 안 맞는다.
    새 결정 = **160 × 160 m 단일 평면 아레나 · 다중 코어 교차 동선 · 골격 저작 + 내부 절차 · 억제기 파괴 → 스테이지 전환 · 회로 기판 아트**(치수는 [ADR 0011](../Architecture/0011-authored-skeleton-and-arena-validator.md) E1 로 80×80 에서 갱신).
    **(b) 는 다시 닫혔다** — 이 항목 본문이 스스로 적어 둔 *"맵을 갈면 플로우필드 베이크·스폰포인트·미션존·성능 베이스라인이
    전부 무효"* 가 그대로 발동했다. **(a″) VAT-1 실측 절대값도 `L_Map1_City` 위에서 잰 것이라 재측정 대상**이다
    (⚠️ A/B 대조 결론 = CPD 채택([ADR 0007](../Architecture/0007-enemy-swarm-render-path-cpd.md))**은 유효** — 같은 맵에서 양쪽을 쟀으므로 상대 비교는 살아남는다).
    **(b) 측정 시나리오에 추가할 것 = 예비 아레나 상주 비용**(렌더·콜리전 off 상태의 프리미티브 등록·컬링·메모리, 0010 D6).
    아래 ✅ 블록과 실측 지형 수치는 **경위 보존용**이며 현행이 아니다.
...
① ✅ **충족 2026-08-31** — `Performance.md §5` 「M0 공식 베이스라인 실측」 블록(패키지 `26_8_30_BuildTest_1`, Development). 원문: `Performance.md §5`에 **측정 빌드 구성이 명기된** 적 300/500 실측 프레임값이 기입됨(추정치 아님). 그 값은 **(a′)로 확정된 맵 위에서, (a″)로 확정된 렌더 경로가 적용된 상태**로 잰 것이어야 한다 — 즉 **우리가 유지할 아키텍처의 수치**여야 한다.
  - 🔒 **판정 완화 명기 (사용자 결정 2026-08-31)** — 실측의 **요청**은 300/500 이 맞고(`delivered 300/300`·`500/500`), 스폰 직후
    **정착 생존은 259/428** 이다. 60m 스폰 링의 일부가 아레나 바닥 밖에 걸려 그 자리 적이 공중에 태어나 낙하·회수되기 때문이다
    (상세·근거 = `Performance.md §5`). 사용자는 **이 값으로 EC ① 을 마감**하기로 결정했다 — 판정 여유가 3.8~7.3배라 13.7% 차이로
    결론이 뒤집히지 않는다. **대가(정직 기록)**: 이 EC 는 이제 문자 그대로의 «적 300/500 에서 쟀다」가 아니라 «요청 300/500 ·
    정착 259/428 에서 쟀고 그 차이를 문서화했다」로 충족된 것이다. 링을 유효 바닥 위로만 배치하는 수정은 별도 행으로 이월했다.
...
> - **환경** ~~Synty Cyber City(맵1)~~ → 🔴 **회로 기판(맵1, 재결정 2026-08-16 · [ADR 0010](../Architecture/0010-arena-topology-and-stage-transition.md) D8)** — 대형 IC·히트싱크=클러스터 / 기판 트레이스=**통로 그 자체** / 비아·정션=교차점 / 전해 커패시터=엄폐물 / 전원부=억제기. **전체 셀/툰 통일(§1-C-9)·로우폴리 제1원리는 불변** · **무기** Synty **Military Pack 모듈 백본 + 사이버 리스킨**(Infima 교체) · **캐릭터** 애니 셀 'Anime Girl Blu' 리스킨(플레이어/팀원) · **적 스웜** 별도 저코스트 VAT(애니 리스킨 금지) · **FP 팔** Blu 팔 추출 + **PWAS** 절차 애니 · **UI** Synty **Sci-Fi Soldier HUD** · **VFX** Synty Particle FX + Epic Niagara(무료) · **오디오** Synty 밖(Sonniss·Kenney·Fab).
