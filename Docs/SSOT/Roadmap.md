...
### 7-5. 코어 재미 게이트 (확정 2026-06-10)
로그라이트의 생사 = **30초 루프(스웜 사격 + 카드 선택)가 재밌는가**. 기능 단위 로드맵(§7-3)만으로는 이 판정이 누락되므로 **명시적 게이트**를 둔다.
- **시점**: P4-C 무기 콘텐츠(6종) 완료 직후 — **§5 성능 검증과 동시**(무기 6종 PIE 가능 시점). 콘텐츠 양산(P4 후속·P5)보다 **루프 재미가 선행**.
- **판정 항목**: ① 스웜 사격 손맛(타격감·반동·학살 쾌감, §2-4-2) ② 카드 선택의 의미(빌드가 달라지는 체감, §2-3 시너지) ③ 프리즈 페이싱 수용성(§2-2 — 비동기 Q/E 전환 필요성 판단) ④ 1인칭 사각 위협 체감(§2-14 사각 오디오) ⑤ 적 500 체감 성능(§5).
- **불합격 시**: 다음 콘텐츠 진행 전 **루프 재설계 우선**. 항목별로 §2-2(프리즈)/§2-4(사격)/§5(성능) 재검토 트리거.
