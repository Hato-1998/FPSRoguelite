...
| 코드 | 마일스톤 | 성격 |
|---|---|---|
| **M0** ✅ | 기준선 정정 — **Exit 2026-08-31** | ⚠️ 기반 (유일한 비-수직슬라이스, 아래 예외 근거) |
...
> 🏁 **M0 EXIT — 선언 2026-08-31 (사용자).** EC ①~④ 전부 충족. 이 선으로 **M1 착수가 열린다.**
> - 잔여 작업은 **하나도 M0 에 남기지 않았다** — 전량 M1/M2/M3/M5 로 재배정했다(배정표 = `Docs/WorkLog.md` 최상단 「M0 Exit」 항목).
> - 재배정은 **우선순위를 바꾸지 않았다**(승격·강등은 사용자 권한, §6-9 (5)) — 옮긴 것은 `마일스톤` 뿐이다.
> - ⚠️ **EC ① 은 완화된 판정으로 닫혔다**(아래 ① 의 🔒 블록). 「적 300 에서 쟀다」로 인용하면 안 되고,
>   **M2 EC ③ 재측정 전에** 행 「바닥 없는 자리에 적을 스폰한다」를 닫아야 그 완화가 해소된다.
...
- ~~맵 2·3(Nature / Space)~~ → 🔴 **맵 테마 체인 재설계 (사용자 결정 2026-09-10)**: 맵은 **고전 아케이드 리스펙트**로 간다 — **맵1 = 팩맨**([ADR 0016](../Architecture/0016-art-direction-retro-arcade-pixel.md) D9, 확정) · **맵2 = 배틀시티**. **Nature / Space 트랙은 폐기**했다(보드 두 행도 `폐기`로 닫음). 배틀시티의 벽돌 벽은 **장식**이며 통행 위상을 바꾸지 않는다 → 플로우필드 **로드 시 1회 베이크** 전제 무손상(ADR 0012 불변식 1). IP 취급 = **미로 형태·규칙·디자인 모티브까지, 모방은 하지 않는다**(캐릭터 실루엣·명칭·로고 차용 금지). U 연속필드(`34b5eea`)는 다중맵 폐기(ADR 0010) 이후 존치·미사용이라 이 항목과 무관하다.
- 무기 · 카드 · 미션 · 보스 물량
...
- ⚠️ **집행 가능성 = 조회에 실려야 성립한다.** 세션이 행을 고르는 경로에 `마일스톤`이 실리지 않으면 이 규칙은 **집행 수단이 없다** — 그래서 §6-9 (2) **③ 대기열 조회**의 SELECT에 `마일스톤`·`크기`를 넣고, 그 조회를 아예 현재 마일스톤으로 `WHERE` 좁힌다(개정 2026-09-09: 종전엔 세션 시작 전수 조회에 열을 얹었는데, 그 조회가 100행에서 잘리면서 집행 근거가 조용히 빠지는 경로가 있었다).
...
> 🔴 **정정 2026-09-10 (사용자 결정)** — 이 절 곳곳의 *"2맵 Nature · 3맵 Space 계획은 불변"* 서술은 **더는 유효하지 않다.** 맵 테마 체인이 **고전 아케이드 리스펙트**(맵1 팩맨 · 맵2 배틀시티)로 재설계됐고 Nature/Space 트랙은 폐기됐다. 상세 = §7-6 M4 항목 · [ADR 0016](../Architecture/0016-art-direction-retro-arcade-pixel.md) D9.
...
| FP팔/3P 바디 메시 | ⚠️ *정정 2026-08-13*: **Infima는 없다**(`Content/`에 Infima 폴더 0). 실물 = FP팔 `Content/Character/FPArms/SK_NeonV_FPArms`(+`ABP_FPArms`/`ABP_FP_Base`) · **3P 바디 = `/Game/Characters/Blu/…/Blu_` + `ABP_Blu_Body`**. BroBot은 이제 **적 스웜 VAT 메시**로만 남았다.<br>🪤 `BP_FPSRPlayer`가 `SK_LPAMG_Arms_Base_Smooth`도 참조해 **팔 계보가 3중**(NeonV/Blu/LPAMG)이라 문서만으로는 어느 것이 활성인지 판별 불가 | ✅ **3P 바디 = 완료**('Anime Girl Blu' 리스킨이 이미 플레이어에 적용됨 — 이 칸이 "전환 계획"이라 적어 둔 것이 이미 실물이다). **잔여 = FP 팔 Blu 추출 + PWAS 절차 애니**(§1-C-9) 🔄 **2026-09-04 ADR 0015: FP 팔은 그리지 않는다**(숨긴 모션 드라이버). "FP 팔 Blu 추출 + PWAS 절차 애니" 잔여 항목은 **소멸**, 대신 무기 §3-5 머티리얼이 1P 비주얼 전부 |
