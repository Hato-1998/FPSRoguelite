...
- 🔴 **아래 다중맵 항목은 폐기됐다 (사용자 확정 2026-08-16 — [ADR 0010](../Architecture/0010-arena-topology-and-stage-transition.md))** — **코드는 존치·미사용**이고 제거하지 않는다(순이익 없이 회귀 위험만 있다). 아래는 **경위 보존용**이며 현행 설계가 아니다. 다만 **재사용되는 부품 셋은 살아 있다**: ① 문 파괴 시 `blocked→open` **stamp + 단일 `RunBFS` + generation bump** 패턴 → 0010 D7의 파괴 가능 프롭이 이것의 일반화다 ② **origin-aware connectivity 전투게이트**(`CanAffectTarget`) ③ `TopologyGeneration` ack(late-join). 폐기 계기 = `L_Map1_City`(450×495m)가 셀 36,200/40,000을 써 **다중맵이 이미 물리적으로 불가능**했다는 것이 논의 중 확인됐다.
- ~~**다중맵(#3) — U 연속필드 단일 그리드**~~ (구현 완료·`--no-ff` main 머지 `34b5eea`, 설계 `Docs/Review/20260707-plan-continuous-field-arch.md` · 🔴 폐기 2026-08-16): 단일 `UFPSREnemySpawnSubsystem` + **FrontId occupancy allocator**(전역 공유 예산·전선별 배분·"2인+ 전선 > 솔로" 가중, `RunFlow §2-1`), 길찾기 = **고정 3×3 world extent 프리사이즈 단일 `UFPSRFlowFieldComputer`**(맵 stream-in 시 그 슬롯 셀 구간만 shared grid에 증분 atomic bake=경계 양쪽 edge 클리어; 미로드 슬롯=blocked). **문 부수면 door cells `blocked→open` stamp**(문 leaf=`ECC_FPSRPlayerPawn`·blocker=`WorldDynamic`이라 `WorldStatic` bake 미포집→명시 stamp) **+ 단일 `RunBFS` + generation bump** → 적이 O(1) 샘플로 열린 문 넘어 seamless 추격. **origin-aware connectivity 전투게이트**(`FPSRCombatStatics::CanAffectTarget`=instigator 원점셀↔타겟셀 open-grid 연결, explosion=`Center` 원점). late-join = `GameState.TopologyGeneration` ack + freeze pre-unfreeze RunBFS + reset baseline. Tier-0 복제 = 대칭 거리컬 교전버블 NetCull(Option A); 진짜 공간 relevancy=RepGraph 후속. `UFPSRRunDirectorSubsystem`은 단일·런클럭 전역(미션/스폰만 대상슬롯 파라미터화). 슬롯 100~132m/변(near-cap, D1). 재계산 예산=`Performance.md §5`.
