...
### D12. OBJ 임포트가 **Y 축을 부호 반전**한다 — 크기만 보면 절대 못 잡는다 (2026-09-04, 하드서피스 라이플)

파이썬이 직접 쓴 OBJ 를 헤드리스로 임포트하고 `size_cm` 으로 검증해 두 번 "통과"시켰다. 세 번째에
**바운드 min/max** 를 찍어 보니 엔진 기대 `Y −8.0..22.6` 이 `Y −22.6..8.0` 으로 들어와 있었다 — X·Z 는 그대로.
**UE OBJ 임포터가 오른손→왼손 변환으로 Y 를 뒤집는다.** 크기(max−min)는 부호에 둔감해서 **두 번 다 정상으로 보였다.**

결과 = 메시는 −Y 를 보는데, 파이썬으로 직접 찍은 소켓(임포터를 안 거침)은 +Y 앞에 있어 **총열 소켓이 총 뒤 40cm 에 떠 있었다.**
같은 이유로 좌우 비대칭 디테일(탄피 배출구)도 반대편에 가 있었다 — 첫 임포트부터 그랬는데 아무도 못 봤다.

→ **임포트 검증은 `size` 가 아니라 `get_bounding_box().min/max` 로**, 그리고 **기대값을 먼저 적어 두고 대조**할 것
(기대값 없이 숫자를 보면 뒤집힌 것도 그럴듯해 보인다).
→ 생성기 쪽 정석 = 정점을 쓸 때 **Y 를 미리 뒤집고**(임포터가 되돌린다), 그 사상이 det −1 이므로 **삼각 인덱스도 같이 뒤집어**
파일 자체가 정상 오른손 OBJ(바깥 CCW)가 되게 한다. 소켓/manifest 는 임포터를 안 거치니 엔진 좌표 그대로.
...
### G12. **참조 0건**을 "결함이 있다"의 증거로 쓰지 말 것 (2026-08-19, 보스 프리즈)
*"`Boss/FPSRBossBase.cpp` 에 pause 관련 참조가 0건이다 → 카드 프리즈 중에도 보스가 계속 행동한다"* 로 결론내
**하이 우선순위 보드 행**이 열렸다. 재현 절차도 PIE 관찰 기록도 없이 **grep 결과만으로** 증상을 서술한 것이다.

**실제**: 같은 클래스의 **헤더**가 *"movement is disabled … no per-tick logic — so there is nothing for the global
level-up freeze to gate here"* 라고 적고 있었고 `PrimaryActorTick.bCanEverTick=false` 였다. 즉 **멈출 행동 자체가
없어서** 가드가 없는 것이지, 가드를 빠뜨린 게 아니다. `.cpp` 만 grep하고 `.h` 를 안 읽으면 이 문장이 안 보인다.

**PIE 재현 결과 = 아이들 애니메이션만 재생.** 그리고 그건 보스만의 현상이 아니다 — 프리즈는 **입력·게임플레이 틱
게이트**라(§2-2) 플레이어 AnimBP도, 근처 스웜 적의 VAT 아이들 클립도 똑같이 계속 돈다(스웜의 playrate 0은 *거리*
LOD `FPSRVATAnim::AnimFreezeRadiusSq` 에서만 걸린다). **비대칭이 아니었다.**

→ 세 가지를 갈라 쓸 것. **①"가드가 없다"(코드 사실) ②"그래서 X가 일어난다"(행동 주장) ③"실제로 X가 일어났다"
(관찰)** — ①에서 ②로 건너뛰면 없는 버그를 하이로 올린다. 행동 주장을 쓸 거면 **재현 절차와 관찰을 같이 적고,
없으면 "미검증 추론"이라고 명시**한다. 그리고 클래스의 계약은 `.h` 주석에 사는 경우가 많다 — `.cpp` grep 0건은
...
🚨 **토큰이 없어 API 를 못 부르는 상태에서는 이 버그가 안 잡힌다.** 구문·BOM·무토큰 분기가 전부 통과해도 실제 호출은 100% 실패한다. 검증 수단이 없으면 **로컬 `HttpListener` 로 모의 서버를 띄워 바이트를 받아 보라** — 이 항목이 그렇게 잡혔다. (같은 실패형 = G7.)

---

## I. 전투 · 콜리전

### I1. 총알이 파괴물을 그냥 통과한다 (막히지도 않고 데미지도 안 들어간다)
**증상**: 억제기·문을 쏴도 안 부서지고, **그 반대편에 있는 적이 맞는다.** 메시는 기본 Cube 라 콜리전은 분명히 있다.

**먼저 콜리전을 의심하지 말 것.** 에디터 월드에서 그 액터를 관통하는 Visibility 라인트레이스를 쏴 보면
`bBlockingHit=True` 로 정상 차단된다(`SystemLibrary.line_trace_multi` + `HitResult.export_text()` —
`FHitResult` 필드는 Python 에서 protected 라 `export_text` 로 읽는다). **콜리전은 멀쩡하다.**

**원인**: 파괴물이 **데미지를 받으려고 pawn 오브젝트 채널을 빌려 쓴 대가**로 투사체에 투명해진 것.
`AFPSRArenaDestructible`/`AFPSRDoor` 의 메시는 오브젝트 타입이 `ECC_FPSRPlayerPawn` 이고
`SetGenerateOverlapEvents(false)` 다. 투사체는 이 조합에 대해 두 경로가 **동시에** 불발한다:

| 경로 | 왜 안 되나 |
|---|---|
| 블로킹 히트 | `AFPSRProjectile` 이 그 채널에 **Overlap** 응답만 준다 → `OnComponentHit` 안 뜸 |
| 오버랩 이벤트 | 엔진은 **양쪽 컴포넌트 모두** 플래그가 있어야 보낸다 (`PrimitiveComponent.cpp:2972`) → 파괴물이 꺼 놨으니 안 뜸 |

**히트스캔이던 시절엔 멀쩡했다** — 오브젝트 타입 라인트레이스는 씬 쿼리라 오버랩 이벤트 플래그를 보지 않는다.
**무기를 히트스캔→투사체로 바꾸면 이 종류의 결함이 조용히 생긴다.** 해결 = 파괴물을 전용 오브젝트 채널
(`ECC_FPSRDestructible`)로 옮기고 투사체가 그 채널을 **Block** 하게 한다(`d49998f2`).

### I2. 오브젝트 채널을 옮겼더니 다른 데서 관통이 재발한다
**원인**: 오브젝트 **쿼리**는 질의한 채널만 찾는다. 파괴물의 오브젝트 타입을 바꾸면
그 액터를 찾아야 하는 **모든 쿼리**가 동시에 눈이 먼다 — 무기 데미지 경로뿐 아니라
**적 원거리 LOS**(`FPSRRangedEnemyBase::HasLineOfSight`)까지. 빠뜨리면 적이 문 너머로 쏜다.
→ 채널을 옮길 땐 `grep` 으로 그 채널의 **모든 질의 지점**을 먼저 전수 조사할 것.

### I3. 파괴물이 벽인데 벽 취급을 못 받아 뒤쪽 적까지 데미지가 샌다
**원인**: 히트스캔·차지레이저는 벽 거리(`WallDist`)를 구할 때 **폰 쿼리에 걸린 액터를 전부 무시 목록에
넣는다**("폰이 엄폐물 행세를 못 하게" 하려는 것). 파괴물도 그 쿼리에 걸리므로 같이 무시되고,
컷오프가 파괴물 너머로 흘러 뒤쪽 적이 맞는다. → 무시 목록은 **진짜 폰만**
(`FPSRCombat::IsDestructibleGeometry` 로 제외).

⚠️ 함께: 파괴물은 **벽이면서 표적**이라 자기 항목 거리가 정확히 `WallDist` 다.
`Entry.Distance > WallDist` 를 맨 `>` 로 두면 float 잡음에 **자기 데미지가 조용히 누락**되고,
증상은 "안 부서진다"로만 보인다. → `+ KINDA_SMALL_NUMBER` 톨러런스.

### I4. C++ 에서 콜리전을 고쳤는데 BP 인스턴스에 안 먹는 것 같다
네이티브 컴포넌트라도 **BP 가 `BodyInstance` 를 오버라이드로 저장했으면 C++ 생성자 변경이 안 먹는다**
(디테일 패널에서 Collision 을 한 번이라도 건드리면 저장된다). `profile=Custom` 표시만으로는
...
> 등록 2026-09-09 (노션 워크스페이스 재편 세션에서 전부 실제로 밟은 것). 구조·역할 분담 = `Docs/SSOT/Workflow.md` §6-9 (9).
...
**해결** — 둘 중 하나. 정본 = `Docs/SSOT/Workflow.md` §6-9 (2).
