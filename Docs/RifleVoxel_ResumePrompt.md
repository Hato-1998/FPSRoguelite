...
§2 는 넷이 확정됐다. 남은 건 2-1(복셀 칸 크기) 하나이고 전장 실측이 선행이다.
§3-0 부터 시작해 순서대로. C++ 가드 확장(§3-2)은 플랜 게이트 대상이다.
```
...
> **2026-09-03 현재 — 다섯 중 넷이 확정됐다.**
> ✅ 2-2 정면 축 = **+X** · ✅ 2-3 색 = **머티리얼 슬롯(B안)** · ✅ 2-4 = **파츠 분리 + C++ 가드 확장** ·
> ✅ 2-5 = **파일럿(라이플 1종)**
> 🔴 **남은 것은 2-1(복셀 칸 크기) 하나**이고, 그건 **라이플 전장 실측이 선행**이다.
> 확정된 넷은 근거까지 아래에 적어 뒀다 — 다시 논의하지 말고, 뒤집을 이유가 생기면 근거부터 반박할 것.
...
### 3-0. 남은 결정 1건 (에디터 켜짐 — 실측만)
§2 는 **넷이 확정**됐고 **2-1(복셀 칸 크기)** 만 남았다. 그 결정의 입력값이 실측이다:

1. **라이플 전장 실측**(§2-1 코드) — 몸통만이 아니라 **조립된 전장**으로.
2. 사용자에게 §2-1 표(칸 크기 ↔ 칸 수 ↔ 인상)를 실제 숫자로 채워 제시하고 **칸 크기를 받는다.**
3. 확정되면 **`Docs/SSOT/ArtDirection.md` 에 전역 복셀 격자 규격으로 올린다** —
   적·환경 복셀도 같은 격자를 써야 하므로 무기만의 값이 아니다.

**그 다음, 모델링 시작 전에 하나 더**: **Blockbench 머티리얼 그룹 왕복 테스트**(§2-3 사다리) —
2색 큐브 2개짜리 최소 모델로. 전체를 만든 뒤에 알면 재저작이다.
> 파츠 분리(§2-4)가 확정됐으므로 이 테스트의 **위험은 크게 낮아졌다** — 파츠마다 파일이 갈리면
> 파일 하나에 슬롯 1개씩만 있어도 충분하다(사다리 3번이 사실상 기본 구조가 된다).
> 테스트의 목적은 이제 "**한 파츠 안에서** 발광부를 따로 뽑을 수 있는가"로 좁혀진다.
