...
5. 🔁 **정정(2026-09-03): 모델링도 Claude 가 한다.** 이 문서는 처음에 *"Blockbench 저작은 사용자 작업"* 이라고
   적었는데 그건 **물려받은 가정이었지 제약이 아니었다** — `Scripts/gen_enemy_proto_meshes.py` 가 이미 순수
   파이썬으로 OBJ 를 직접 써서 적 프로토를 만들고 있었다. 복셀은 이산 격자라 자동 생성에 특히 유리하다.
   → **`Scripts/gen_rifle_voxel.py` 로 파츠 7종을 생성한다.** 형태 수정 = 그 파일 상단 박스 목록 몇 줄.
...
#### ✅ 검증 완료 (2026-09-03) — 이 항목의 위험은 사라졌다
종전 우려는 *"Blockbench 가 머티리얼 그룹을 나눠 내보낼 수 있는가"* 였다. **Claude 가 OBJ 를 직접 쓰게
되면서 그 질문 자체가 없어졌다** — `usemtl` 을 우리가 제어한다.

**끝단까지 실측 확인**: 임포트 후 UE 메시 섹션 수가 슬롯 수와 정확히 일치했다 —
`Body` 3섹션(Body/Grip/Emissive) · `Mag` 2 · `Reddot` 2 · 나머지 1. `slots_ok=True`.
즉 **텍스처 0장으로 파츠 안에서 색 구역을 나누는 것이 성립한다.**
