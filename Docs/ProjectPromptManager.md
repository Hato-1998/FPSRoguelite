...
> 이 문서는 *역할 정의*다. **라이브 현황·우선순위·의존성(DAG)은 Notion PM 보드**(`Docs/SSOT/Workflow.md` §6-9, 2026-08-07 전면 이관), 보류 로드맵 아카이브는 `Docs/TaskPrompts_Master.md`, 완료 상세는 `git log`.
...
- 통과 시: **보드 행 상태=완료 + `완료커밋` 해시 기입 + 로그 append**(§6-9). 후행 차단 해제 확인. 필요 시 메모리 갱신. (상세는 git log·`Docs/WorkLog.md`)
- 실패/미검증 시: 무엇이 부족한지(PIE 미확인/빌드 실패/머지 안 됨) 보고하고 마킹 보류.

### 책임 C — 변경 감지 → 의존성 재계산 → 프롬프트 재수정
- **라이브 DAG = 보드의 `선행작업`/`후행작업` 관계**(§6-9). 작업이 완료·추가·삭제·재설계되면 보드 관계를 재평가한다: 후속 유닛의 선행조건·파일 핫스팟(`주요경로`)·순서가 바뀌었는가?
...
- **순차 진행**(병렬 폐지 2026-06-15): 한 유닛씩 — `main`에서 구현 → 빌드+스모크 → 검토 → (코어면 §6-6-1 레드팀 게이트) → `origin/main` 푸시 → 다음. 브랜치는 파지 않는다(트렁크 기반, `Workflow.md` §6-7, 2026-09-05). (§B-3)
- **푸시 전 검토**: 완료 = 푸시가 아니라 "빌드+스모크 통과 후 사용자 검토". 미검증 푸시 금지.
- **단일 진실 출처 분리**: **PM 보드=라이브 현황·우선순위·DAG**(§6-9) / `TaskPrompts_Master.md`=보류 로드맵 아카이브(재개 시 보드로 유닛 인입) / `git log`=완료 상세 / 긴 경위=`Docs/WorkLog.md`. 같은 내용을 여러 곳에 중복 금지. 완료 가이드 등 쓰임 끝난 문서는 정리(원본은 git 히스토리 보존).
- **HIGH_RISK는 승인 후**: 파일 삭제·push·worktree 변경은 사용자 승인 후 실행(CLAUDE.md).
- **문서만 올리려다 남의 미검증 코드를 함께 밀지 말 것**: 트렁크 기반에서는 이게 예외가 아니라 **기본 토폴로지**다 — `origin/main..HEAD` 에 다른 세션의 미검증 커밋이 쌓여 있으면 문서 1줄 푸시가 그 코드까지 민다. 푸시 전 `git log --oneline origin/main..HEAD` 로 범위 확인(§6-7 5단계).
- **플랜↔목표 정합**: 작성한 프롬프트가 Game.md/SSOT의 설계 목표·장르 3원칙(§1)과 어긋나지 않는지 점검(필요 시 [[plan-codex-comparison-gate]]).
- **모델 정책**(§6-5, 개정 2026-08-26): 유닛을 먼저 2갈래로 나눈다 — **코어·구조설계·리팩토링** = `Opus 조사·플랜 → Fable 플랜 게이트(G1) → 사용자+Opus 승인 → Sonnet 구현 → Opus 검증 → Fable 머지 게이트(G2)`(명세는 `Docs/Specs/`에 커밋, §6-5-2. Fable 은 주도자가 아니라 2게이트 검증자다) / **그 외** = 구현 Sonnet 위임(현 최신 Sonnet 5) · 검증(빌드·스모크·diff) Opus 직접. 검증·판정의 하위 모델 위임은 어느 쪽도 금지. 프롬프트를 쓸 때 유닛마다 **어느 갈래인지 명시**한다.
...
- ⚠️ **적대 검증 호출은 조건부**(ConsultLoop §0-1): 코어·리팩토링·설계/구조(§6-5-2 `T1~T5`) 유닛에만 — **머지 레드팀 게이트**(내부 Fable, §6-6-1)와 **Codex 토론 채널**(`/consult`·`/plan-consult`) 양쪽 다. 콘텐츠·수치·BP배선·에셋 유닛은 둘 다 없이 진행하며, 프롬프트에 그 사실을 명시한다.
- 진행: **PM 보드가 현재 순서·상태의 SSOT**(§6-9 — `PROGRESS.md`는 포인터만 남음, 2026-08-07 이관).
- 검증/빌드 상세: `Docs/SSOT/Workflow.md` §6(레드팀 게이트 = §6-6-1).
- ⚠️ 이 절의 구체 유닛 상태는 빠르게 stale된다 — **항상 git log + 보드 조회로 실확인** 후 말한다.
