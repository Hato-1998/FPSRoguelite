...
- 유닛 프롬프트 골격(§C 공통): ① 읽을 SSOT(Game.md §0-1 라우팅) ② phase 브랜치 분기(§6-7) ③ 플랜모드 우선·모델정책(구현 Haiku/검증 Opus) ④ 산출물·콘텐츠 보류분 ⑤ **함정/주의(코드에서 확인된 gotcha)** ⑥ 검증 절차 ⑦ 완료 시 처리.
...
- **모델 정책**: 구현=Haiku 위임 / 검증(빌드·스모크·diff·Codex)=Opus 직접(§6-5).
