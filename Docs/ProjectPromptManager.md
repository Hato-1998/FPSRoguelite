...
- **모델 정책**(§6-5): 유닛을 먼저 2갈래로 나눈다 — **코어·구조설계·리팩토링** = 플랜·지시·검증 모두 **Fable**(구현만 Sonnet 위임, 명세는 `Docs/Specs/`에 커밋, §6-5-2) / **그 외** = 구현 Sonnet 위임(현 최신 Sonnet 5) · 검증(빌드·스모크·diff·Codex) Opus 직접. 검증·판정의 하위 모델 위임은 어느 쪽도 금지. 프롬프트를 쓸 때 유닛마다 **어느 갈래인지 명시**한다.
