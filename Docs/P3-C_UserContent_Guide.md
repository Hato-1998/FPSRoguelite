...
- GE가 건드릴 수 있는 **글로벌 속성**(현재 구현된 것):
  ...
  | `FPSRCombatSet` | `Luck` | 0 | **광역 행운** — 카드 추첨에서 상위 등급 확률↑(향후 드랍품질·희귀스폰 등도 담당) |

  > 새로운 스탯 축(이속/픽업반경/XP획득 등)은 아직 AttributeSet에 없다 → 그 카드를 만들려면 C++ 확장 필요(§2-3).
  > 지금은 위 6개 속성 범위 안에서 샘플 카드를 만든다. (RarityBonus는 Luck으로 통합·폐지 — 2026-06-02)

- 콘텐츠 폴더 권장 구조:
  ```
  Content/Cards/
  ├── Character/        ← Character-scope 카드 DataAsset + 그 GE (P3-C에서 실제 추첨/적용)
  │   └── GE/
  └── Weapon/           ← 무기 전용 카드 DataAsset (P4 작동). 풀이 아니라 각 무기 DA의 WeaponCards에서 참조
      └── GE/
  ```
  - **Character 카드** = 풀 1개(`DA_CardPool_Character`)에 담아 GameMode에 할당 → 추첨됨.
  - **Weapon 카드** = 별도 풀을 만들지 말 것. 각 `UFPSRWeaponDataAsset.WeaponCards`에서 참조(보유 시 동적 합류, §2-4). P3-C에선 Scope=Character만 추첨되므로 무기 카드는 **P4에 작동**.
  - 폴더 이동은 **에디터에서 이동/이름변경**으로(디스크 직접 이동 금지 — 참조 깨짐).
...
| **Description** | 설명문(멀티라인) | `치명타 확률 증가` |
| **Scope** | **`Character`** 로 둘 것 (P3-C는 Character만 적용) | `Character` |
| **Applied Effect** | 1단계에서 만든 GE 클래스 선택 | `GE_Card_MaxHealth` |
| **Weight** | 카드 전체 상대 추첨 가중치 배율(기본 1.0) | `1.0` |
| **Rarity Tiers** | **이 카드가 나올 등급 + 등급별 수치 배열.** 항목당 `Rarity` + `Magnitude`. **모든 등급에서 나오게 하려면 4개**, 한 등급만이면 1개 | (아래 예) |
| **Card Family** | (선택) GameplayTag. 같은 family 카드는 한 번에 1장만 제안. **비우면 Applied Effect GE 클래스로 자동 그룹핑** | (보통 비움) |

> ⚠️ **`RarityTiers`가 비면 그 카드는 추첨에서 빠진다**(에디터에서 에러 표시 + 런타임 경고 로그). 반드시 1개 이상 넣을 것.

### 샘플 — "최대체력" 카드 **1개**가 모든 등급에서 (GE 1개 + 카드 1개)

`Card_MaxHealth` (단일 에셋):
- Scope = `Character`, Applied Effect = `GE_Card_MaxHealth`(방식 A, SetByCaller), Card Family = (비움)
- **RarityTiers**:
...
→ 이 **1개 에셋**이 추첨 때마다 등급을 굴려 그 수치로 등장. 한 추첨에 이 카드는 **최대 1장**(같은 카드의 다른 티어 동시 등장 없음).

다른 샘플 카드들(각 1 에셋):

| 카드 DataAsset | Applied Effect | RarityTiers (Rarity→Magnitude) |
|---|---|---|
| `Card_MaxHealth` | `GE_Card_MaxHealth` | Common 15 / Rare 35 / Epic 60 / Legendary 100 |
| `Card_CritChance` | `GE_Card_CritChance` | Common 0.03 / Rare 0.05 / Epic 0.08 |
| `Card_Damage` | `GE_Card_Damage` | Rare 0.15 / Epic 0.25 / Legendary 0.40 |
| `Card_Luck` | `GE_Card_Luck` | Epic 1 / Legendary 2 |

> 한 등급에만 나오는 카드는 RarityTiers에 그 등급 1개만 넣으면 된다.
> 다른 GE/속성인데 같은 계열로 묶어 상호배타시키고 싶을 때만 `Card Family` 태그를 직접 지정.
> 최소 4~6종(카드)·각 2티어 이상 권장.
...
### 생성 절차
1. `Content/Cards/` → 우클릭 → **Miscellaneous → Data Asset** → 클래스 **`FPSRCardPoolDataAsset`** → 이름 `DA_CardPool_Character` (**Character 카드 전용 풀 1개**. weapon 풀은 P3-C에서 만들지 않음 — 위 §0 참조)
2. 더블클릭 → 필드 입력
...
- **추첨 공식**(참고): `유효가중치 = Card.Weight × (등급 기본가중치) × (1 + Luck×LuckScale × 등급tier)`
  - 등급tier: Common=0, Rare=1, Epic=2, Legendary=3 → Luck이 0이면 등급 기본가중치만 작용,
...
2. 디테일 패널 → 카테고리 **`FPSR | Cards`** → **`Card Pool`** 프로퍼티에 `DA_CardPool_Character` 할당
3. 컴파일 · 저장
...
| 2 | `FPSR.DrawCards 3` | `[Card] [0] Card_xxx (Rare, mag 35.00)` … 3줄(등급·수치는 가중 랜덤, 같은 카드 중복 없음) |
...
| 카드가 0장 추첨됨 | 풀 Cards가 비었거나, 전부 `Scope≠Character`, 또는 **카드의 `RarityTiers`가 빔**(로그에 `has no RarityTiers — skipped` 경고) |
...
| 특정 등급만 안 나옴 | 그 등급 카드가 풀에 없음 / 등급 기본가중치가 너무 낮음(Legendary 0.05) — Luck 카드 적용 후 재추첨해 비교 |

---

## 작업 체크리스트

- [ ] 1단계: 속성당 GE 1개 생성 (`Content/Cards/Character/GE/`, Duration=Instant, **방식 A=SetByCaller `SetByCaller.CardMagnitude`**)
- [ ] 2단계: 카드 DataAsset 생성 (`Scope=Character`, Applied Effect 연결, **`RarityTiers`에 등급별 수치** 1개 이상)
- [ ] 3단계: `DA_CardPool_Character` 생성 + Cards 배열에 Character 카드들 추가 (weapon 풀은 P3-C에서 안 만듦)
- [ ] 4단계: `BP_FPSRGameMode.Card Pool = DA_CardPool_Character` + 컴파일
