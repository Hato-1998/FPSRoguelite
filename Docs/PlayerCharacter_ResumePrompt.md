...
**다음 작업 (순서대로)**
1. 🔴 **사용자 판정**(§6 체크리스트) — 비교 시트를 놓고 3장 승인/반려. **아래 「판단 대기」 2건이 여기서 갈린다.**
2. **Meshy A/B 실행**(사용자 계정) → 결과 2개 비교 → `MeshyPrompt_v1.txt` §5 로 판정.
3. **복셀화 트랙 A/B′ 결정**(§3). 판정 기준이 구체화됐다 — **복셀 스킬이 「세로 43블록」(161.25 ÷ 3.75)을
   지정할 수 있는가**. 못 하면 A 탈락, B′(Blender Remesh Blocks)로 확정.
