...
2. **§7 크롭** → `PlayerCharArcade_front/side/back.png`(높이 = 캔버스 88%, 배경 = 패널 평균색).
3. **Meshy 프롬프트 토론**(사용자 지시로 계속 범위 밖) → Meshy Image-to-3D.
4. **복셀화 트랙 A/B′ 결정**(§3). 드론 행의 A/B 테스트 결과가 선행 답.
