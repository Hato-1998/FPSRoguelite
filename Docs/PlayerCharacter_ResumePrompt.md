...
**어디까지 왔나** — SD(Forge) 경로로 **정면 v19 까지** 나왔다. 제미나이 경로(v4)도 살아 있다.
산출물 = `Docs/Handoff/PlayerChar_Arcade/sd_out/v19_fullclosed_w045_seed1234567_112834.png`
...
**다음 작업 (순서대로)**
1. **측면·후면 재생성** — 현재 3장이 서로 다른 세대다(정면 v19 / 측면 v8b / 후면 v8b).
   같은 프롬프트·같은 시드로 다시 뽑아야 세 뷰가 같은 캐릭터가 된다.
   `control/gen_sd_cn.py <depth_ene_side.png> tag 0.45 1234567 832 side` (back 동일)
   ⚠️ 측면 블록아웃(`make_depth.py side`)이 **롱 재킷 변경을 아직 반영 못 했다** — side 분기의
      재킷 폴리곤을 확인할 것. 그리고 이전 측면 결과는 정측면이 아니라 **3/4 뷰**였다
      (뷰 절 가중치를 1.3 → 1.5 로 올리거나 네거티브에 `front view` 추가로 재시도).
2. **Meshy 프롬프트 토론**(사용자 지시로 이번 범위 밖이었다) → 3장 크롭 → Meshy Image-to-3D.
3. **복셀화 트랙 A/B′ 결정**(§3). 드론 행의 A/B 테스트 결과가 선행 답.

**미해결 / 판단 대기**
- 등신 ~5.8 (목표 5.0). depth 로 6.5 → 5.8 까지 왔고 태그로는 더 안 내려간다.
- 머리 폭 > 어깨 폭 ❌ — 오버사이즈 재킷과 구조적으로 상충. **포기할지 사용자 결정 필요.**
