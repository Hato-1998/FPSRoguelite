...
삼면도 3장·§7 크롭·Meshy 1차 생성은 끝났다(§0-A). 이번 단계 = **`Docs/Handoff/PlayerChar_Arcade/MeshyPrompt_v1.txt` §10 Phase 1～5**
(다운로드 → A/B 판정 → 비례 실측 → 리깅 → 애니). 그 문서의 §7·§9·§10-6 함정을 먼저 읽어라.
```
...
**다음 작업 = `MeshyPrompt_v1.txt` §10 (Phase 1~5)**
1. **Phase 1 다운로드** — 무텍스처 지오메트리 FBX+GLB → `Docs/Handoff/PlayerChar_Arcade/meshy_out/`
2. **Phase 2 A/B 판정** — A 런(정면 1장, 텍스처 OFF) 생성·다운로드 → B 와 구조 비교(§5)
3. **Phase 3 비례 실측** — ADR 0002 기준(두 손 간격 vs 팔 여유)을 ENE 에 적용 → §2 스펙표 실측 갱신
4. **Phase 4 리깅** — 기본값 = **Blender 웨이트 전이**(Meshy 오토리그는 실험용으로만)
5. **Phase 5 애니** — **결손이 실제로 있는지 먼저 확인**(§10-0: UE 안 3P 커스텀 애니는 단 2개이고
   로코모션·무기는 마네킹 팩 426개로 이미 채워져 있다). 없으면 범위 밖으로 닫는다.

🔴 **여전히 사용자 판정 대기** — 삼면도 3장(§6 체크리스트) 승인/반려 · 머리 폭 > 어깨 폭 포기 여부 ·
등신 6.2(목표 5.0). 뒤 두 건은 오버사이즈 재킷과 구조적으로 상충해 한쪽을 버려야 한다.

**미해결 / 판단 대기**
- 🔴 **머리 폭 > 어깨 폭**을 포기할지 — 오버사이즈 재킷과 구조적으로 상충한다. 세 장 모두 재킷
  어깨가 머리보다 확실히 넓다. 이 항목을 살리려면 **재킷을 포기**해야 하므로 둘 중 하나를 버려야 한다.
- 🔴 **등신 ~6.2**(목표 5.0). depth 로 6.5 → 5.8 까지 갔다가 롱 재킷이 몸통을 늘려 다시 올라갔다.
  태그로는 더 안 내려간다 — 남은 수단은 블록아웃의 머리를 더 키우는 것뿐인데 위 항목과 같은 충돌이다.
- **롱 재킷의 스키닝 비용** — 재킷이 골반 관절을 지나 다리를 들 때 접혀야 한다. 복셀 강체감과
  충돌한다. 열린 크롭 안(`control/make_depth_open.py`, 결과 v17)이 이 문제가 없으므로
