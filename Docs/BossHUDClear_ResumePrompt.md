...
> ## ⛔ 폐기됨 (2026-07-15 · 작업 완료) — 착수용으로 쓰지 말 것
> **이 작업은 끝났다.** `phase/boss-hud-clear` → main `--no-ff` 머지 완료(구현 `74e63137`).
> **이 문서가 가리킨 clear 누락 = 재현 불가(잠재 계약 갭)** — 계약 정합 + 보스 2페이즈/재런 대비로 고쳤다.
>
> ## ⚠️ 그런데 이 문서의 프레임이 진짜 버그를 가렸다
> 이 문서는 문제를 **"보스가 죽은 뒤 남는 stale 포인터"** 로 규정했고, 그 프레임을 그대로 따라간 조사는
> **"보스가 생기기 전"을 아예 묻지 않았다.** 실제 유저 버그는 **정반대 끝**에 있었다 —
> **런 시작(보스 없음)부터 BOSS 바가 꽉 찬 채로 표시**됨(사용자 스크린샷으로 발각).
> 원인은 `WBP_BossHUDBar`의 **Construct 초기 동기화가 `IsValid` 뒤에 갇혀 있어 빈 상태에서 호출이 안 된 것**
> (콘텐츠 문제, 이 문서의 C++ clear와 **무관**). 상세 = `PROGRESS.md` **§④-2**.
> **교훈: 재개 문서의 프레임을 무비판적으로 물려받지 말 것. 반대 끝도 물을 것.**
>
> **⚠️ 이 문서 §2의 핵심 전제는 틀렸다.** "보스가 파괴되면 GC가 포인터를 null로 만드는데 알림이 안 감"이라고 썼지만,
> **보스는 파괴되지 않는다**(`FPSRBossBase.cpp:155`가 결과 연출용으로 의도적 존치). 실제 상태는 GC-null이 아니라
> **체력 0인 유효한 보스를 가리키는 멀쩡한 포인터**다. §3이 요구한 재현 판정의 답은 **"관측 불가"**이며,
> 근거 3개(트래블 2홉에서 GameState 파괴 / `StartRun` 단일 호출처 + `bRunActive` 일방 래치로 재런 도달 불가 /
> 유일 구간은 의도된 결과 연출)는 **`PROGRESS.md` §④**에 정리했다.
>
> **최신 사실관계 = `PROGRESS.md` §④.** 아래 본문은 조사 이력으로만 남긴다(§1 표의 grep 사실들은 유효,
> §2 전제와 §4 후보 평가는 무효 — 실제 채택은 후보 A + 액터 파괴, 순서 제약 있음).
...
- 머지게이트: 레드팀 리뷰 — 절차·전달 범위는 `Workflow.md` §6-6-1을 따른다. 코어/구조 변경일 때만
