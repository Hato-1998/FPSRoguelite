...
- ✅ **통로 폭 = 10m 로 픽스**(사용자 결정 2026-09-03). 재검토 안 한다.
  적 캡슐 80×180cm 이므로 적이 통로 폭의 **10%** 다(팩맨 유령은 ~90%). 사용자가 화면으로 확인하고
  이 비율을 수용했다. 아트 쪽 대응은 이미 끝났다 — 적이 검정에 묻히지 않도록 실루엣 림 + 하드 엣지를
  넣었다(e122595a·90daaa81).

  🔴 **이 픽스가 ADR 두 곳과의 어긋남을 "실험"에서 "상시"로 바꾼다. 반드시 문서화할 것:**
  | ADR | 문서상 | 실제 |
  |---|---|---|
  | 0012 규모 상한 | 아레나 160×160m | **280×310m** |
  | 0011 E1 / 0012 | 셀 100cm (25,600셀) | **셀 200cm (21,700셀)** |

  셀을 200cm 로 올린 것은 편법이 아니라 통로가 넓어진 만큼의 정합이다(통로당 5셀, 적 반경 40cm 대비
  충분). 파라미터는 맵1 전용 `DA_ArenaParams_Map1_Maze` 로 분리해 두었으므로 L_Map_2·L_Map_Boss 는
  100cm/160×160 그대로다 — **공유 자산을 고치지 않았다는 점이 이 결정을 되돌리기 쉽게 만든다.**
  → `ArtDirection.md` 확장 작업(아래)에서 **ADR 0012 에 `ℹ️ 정정` 인라인 노트**로 남길 것.
  안 남기면 다음 사람이 ADR 만 읽고 아레나를 160m 로 알게 된다.
