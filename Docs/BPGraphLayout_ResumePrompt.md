...
자작 BP 69개 중 연결선이 있는 33개 그래프를 정리했다(커밋 `ef11412e`~`80c9800a`).
합계 **교차 237→98 · 노드 겹침 97→0 · 역방향 선 61→8**, 배선 되돌림 0건.
**다시 돌릴 땐 이 문서 대신 `Scripts/bp_graph_layout.py`를 쓴다**(멱등 — 이미 정리된 건 안 건드린다).
