...
>
> ⚠️ **현황 재프레임 (2026-07-13, 문서 정리 패스)**: 아래 **§B DAG(V0~U20) · §C 유닛 실행 프롬프트는 대부분 ✅ 완료된 역사 기록**이다 — P0~P8 전 구간 + 2차 트랙(U5~U20) + 다중맵 U 연속필드 아크(P-0~P-H) + 반동 CrystalRecoil + 무기 전면개편 + FPSR Data Editor가 모두 main 머지됨. **진행/다음 작업의 진실원본 = `PROGRESS.md`(현재 상태·완료 이력) + §E ConsultLoop 인입표**. 다음 프론티어 = Synty 아트 파일럿(`Docs/SyntyArtPilot_ResumePrompt.md`). **📌 후속 PM 패스 대기 = 완료 §B/§C 트림 + 현재 P8/다중맵/Synty/meta 아크 유닛화(DAG 재앵커)** — 이번 문서정리 범위 밖(역사 프롬프트 내부의 이동/삭제 문서 링크는 잔존 가능; 살아있는 §E·장기백로그 포인터만 교정).
...
| ✅U1 | 재미 게이트(§7-5)+성능 검증(§5) — **합격(사용자 판정 2026-06-30)**: 조건부합격의 MP 차단조건 해소(Phase 1A/1B 머지 `e38dfbe` + PIE 2-client 통과) + G1①~④ 합격(프리즈=B안 baseline-only, ⑤=G2 보류) + V2 소스 재검증 confirmed. **⚠️ §5 적500 정량 측정은 미실시(보류)** → 하드캡 잠정값 유지, 콘텐츠밸런싱/U14 perf 패스로 이월. 시트 `Docs/Archive/gates/U1_GateSheet.md` §D. **→ 2차 트랙 해금** | 검증(사용자 PIE) | V0, V1, W1 | 2차 트랙 전체 | (검증·문서) |
...
| 🔶U12 | UI/필 후속(크로스헤어·히트마커·카드 아이콘·무기별 팔 AnimBP) — **크로스헤어/히트마커=✅ 진실 크로스헤어 재구축 완료·main 머지(2026-07-03, U17 동반)**: 런HUD `ComputeSpreadUV` 진실투영(무기 실분산 콘 base+블룸×ADS→FOV/뷰포트/DPI 화면투영, 탄이 안에 착탄)·발사체 블룸(`ComputeSpreadDegrees`)·`CrosshairStyle` DataAsset(무기별 스타일 폴리모픽)·4종 절차적 SDF(Cross/Ring/BoxDots/Dot)·색/두께 유저설정. **잔여**: 카드 아이콘, 무기별 팔 AnimBP(→U15 흡수, 중복금지) | C++/콘텐츠 | U1 게이트 | — | `phase/p4d-ui-followups`(크로스헤어분 머지됨) |
| 🔶U15 | 1P 무기 애니메이션 시스템(모듈러 부품+노리쇠+팔↔무기 이중 스켈 동기)+1P 팔·손 애님 강화 — **코드 완료(통합 애니 패스 `phase/p6-animation-pass`, 도메인 A)**: DA `ReloadMontage`+`WeaponParts1P`(부품 struct/배열)·`OnRep_Reloading`(bReloading RepNotify, 오너 1P 팔 몽타주 재생속도=몽타주/ReloadTime 정합·프리즈 게이트)·`RefreshWeaponPartComponents`(스켈 무기 런타임 부품 부착). **콘텐츠(무기/팔 AnimBP·몽타주·DA 채우기)=사용자**([AnimationPass_ContentGuide.md](AnimationPass_ContentGuide.md) §A). 그립=`ArmsAnimInstanceClass` 재사용(BlendByEnum 중복금지) | C++✅/콘텐츠 | U1 (조건부 우선순위) | — | ~~`phase/p6-animation-pass`~~ |
...
| ✅U17 | 플레이어 설정 시스템(크로스헤어 **색/두께**) + 메인메뉴 Settings 진입 — **완료·main `--no-ff` 머지(2026-07-03)**: 크기설정 폐기→GUS `CrosshairColor`+`CrosshairThickness`(0.5~2.0)+`OnCrosshairSettingsChanged` 델리게이트 + 설정오버레이(두께슬라이더·색프리셋5 UButton, GUS 직접구동) + 런HUD 실시간반영(FillColor/Thickness, Construct구독/Destruct해제). 빌드+Smoke 4/4+사용자 PIE(메뉴/인게임 실시간·세션영속·기본무회귀·마스터볼륨 무회귀) 통과. **동반=U12 진실 크로스헤어 시스템 재구축**(위 U12 행 참조). Codex 머지게이트=P2 1건(발사체 블룸 원격 클라 비서버권위=히트스캔 parity·회귀아님) 문서화+후속이월(백로그 §B) | C++/콘텐츠 | V3(크로스헤어 위젯) / 소프트 U1 게이트 | (감도·FOV·오디오 설정 확장) | ~~`phase/settings-system`~~ 머지됨 |
| 🔶U19 | 3P 팀원 캐릭터 애니메이션 (협동 가시성) — **코드 완료(통합 애니 패스 `phase/p6-animation-pass`, 도메인 B)**: DA 3P visual 블록(WeaponMesh3P·소켓·FireMontage3P·ReloadMontage3P)·`WeaponMesh3P` 컴포넌트(GetMesh() 부착·SetOwnerNoSee)·`MulticastFireCosmetics` 원격 3P 발사 몽타주·`OnRep_Reloading` 원격 3P 분기·`IsReloading()` 노출. 신규 복제 0(GetBaseAimRotation·복제 Velocity 재사용). **콘텐츠(BroBot AnimBP 발사/재장전 슬롯·몽타주·DA 3P필드)=사용자**([가이드](AnimationPass_ContentGuide.md) §B). PIE 라운드2/3 후속(3P 가시성/관전 오버랩)=PIE서 판정 | 코드✅/콘텐츠 | U9(관전 SetViewTarget)·U15(무기애님 톤) | — | ~~`phase/p6-animation-pass`~~ |
| 🔶U20 | 적 애니메이션(경량 스웜 · 기존 VAT 파이프라인 확장) — **코드 골격 완료(통합 애니 패스 `phase/p6-animation-pass`, 도메인 C, Stage 0-1)**: `EFPSRAnimState`+폴리모픽 `UFPSREnemyAnimProfile`(_VAT concrete)·`AFPSREnemyBase` 드라이버(권위=서버 배치패스/클라=PostNetReceive, 거리LOD·근접Attack휴리스틱)·`OnRep_bDead`+OnDeathCosmetic·**AnimProfile null=휴면(무회귀)**. 보스=`AFPSRBossBase` 스켈 시임(PlayBossMontage+DeathMontage). ⚠️**Stage 2(MF 파라미터 에디터확인)+Stage 3(idle/attack/death VAT베이크·프로파일할당)=사용자**([가이드](AnimationPass_ContentGuide.md) §C). GMS 무접촉(리스너0→U13유지). 엘리트 Siege=VAT유지·보스만 스켈 | C++✅/콘텐츠·에디터 | U1 게이트, U7(적 이동 확정) | U14(§8 폴리시·**정량 perf**) / U13(GMS) | ~~`phase/p6-animation-pass`~~ |

### 장기 백로그 (유닛화 보류 — 기록만)
- **다중맵 심리스 (컨셉 피벗 2026-07-03 · SSOT 반영 2026-07-05)**(`RunFlow §2-1` · `Concept §1-C-9` · `Roadmap §8`): 맵 4방향 거대문 부수기 → 심리스 다음 맵, 맵점유 미션/스폰, 통일 로우폴리 Synty Path A(Sci-Fi City/Nature/Sci-Fi Space). **#3 다중맵 아키텍처 = 설계 수렴(Codex 5R 컨설트 `Docs/Review/20260705-multimap-budget-regroup.md`, §E G1-G5)**: 전역 공유 캡 map-aware allocator · per-map 플로우필드 · NetCull→RepGraph→스트리밍 · 명제 "솔로=정찰/2인+=본게임". ⚠️**per-map 설계는 U 연속필드로 대체·구현·머지 완료**(`34b5eea`, `Architecture §4-1`) / Tier 1(예산 게임필)·Tier 2(rally·인센티브)=후속. **에셋 전체교체(Synty 파일럿·환경 3맵·적 Paragon→Synty) = `Docs/AssetReplacement_Synty_ResumePrompt.md`**(별도 세션; Tier 0는 화이트박스 2맵 독립, Synty 맵은 나중 합류). 획득=SyntyPass, 추천=Artifact 구매플랜.
...
- **서버권위 블룸(원격 클라)**: `CurrentBloom`이 owning-client-local(비복제)라 서버에서 원격 클라의 발사체 스폰/히트스캔 트레이스가 bloom=0(base spread) — HUD는 블룸 콘 표시(코스메틱, PvE 코옵서 익스플로잇 아님, 리슨호스트/싱글은 정확). 정식 수정=서버권위/복제 블룸을 **히트스캔+발사체 공동** 적용(서버가 샷 수락 시 증가 or 복제). Codex U12 머지게이트 P2 이월(2026-07-03), 인코드 주석=`FPSRGA_WeaponFire_Projectile.cpp:91`.
- **개발 툴 백로그**: [`Docs/ToolingBacklog.md`](ToolingBacklog.md)(living, 상태추적 📋/🔨/✅) — **로드맵 전 구간 × 툴 매트릭스**(10-에이전트 워크플로 2026-07-05 재작성): 65툴을 phase(CORE→COOP→MM-T0→MM-T1/2→SYNTY→P6-META→BALANCE→P7-SHIP→LIVEOPS)에 배치 + seam(조기빌드 상속) 9 + 관통 테마 5. 유형태그(저작/뷰어/검증/자동화/계측)·[가정] 표기. 착수 시 별도 phase 유닛(§B-3).
...
```
[작업] 통합 애니메이션 패스 — 1P 무기(도메인 A) + 3P 팀원 캐릭터(도메인 B) + 적(도메인 C, VAT) 세 도메인의 애니메이션을 한 패스로 배선한다. A/B=플레이어측 경량 스켈레탈(≤4 액터, 비용 무관), C=적 perf-critical(200-300, 캡500) VAT 확장 — ⚠️기법·복제 프로파일이 도메인별로 다름(아래 통합 함정 필독). 사용자 결정(2026-07-03): 1P 무기+적+3P 아군 애님 동시 진행.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 공통 — 먼저 읽을 것 / 브랜치·모델 정책 / 브랜치 전략
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[먼저 읽을 것]
- Game.md(§0-1 라우팅 가이드)로 각 도메인 SSOT만 선별 + PROGRESS.md 최신 핸드오프.
- 도메인 A(1P): Docs/V0_WeaponVisual_UserContent_Guide.md(V0 베이스/스켈 경로·모듈러 결정), Docs/SSOT/PlayerFeel.md §2-9(Separated Arms)·§2-13(관전)·§2-14(HUD), Docs/SSOT/CombatWeaponCard.md §2-4-2(사격감).
- 도메인 B(3P): PROGRESS PIE 라운드2/3(원격 아군 SetViewTarget 3P 바디 가시성 미평가 → 흉하면 v2 관전카메라) + 캐릭터 애셋 STEP4(BroBot 플레이어 3P 배선 완료·PIE 미검증), Docs/SSOT/PlayerFeel.md §2-13(DBNO 관전·협동 가시성), Docs/SSOT/Concept.md §1-C(협동 판타지).
- 도메인 C(적): Docs/Archive/guides/AssetWork_CharEnv_Resume.md(VAT 베이크 레시피·DataAsset 필드·UAnimToTextureBPLibrary/UAnimToTextureDataAsset API=기존 VAT 계약 SSOT), Docs/SSOT/Performance.md §5/§5-1(Significance S0~S3)/§5-2(플로우필드), Docs/SSOT/Enemy.md §2-6(스웜 적 렌더=인스턴싱/VAT+거리LOD), Docs/SSOT/Roadmap.md §8(큐브→VAT 교체 착수됨).
- 공통: Docs/SSOT/Workflow.md §6(프로덕션·검증)·§6-7(브랜치/머지). TaskPrompts_Master.md §C의 U15/U19/U20 원안(이 통합의 출처) + U13 §C의 U8 GMS 핫패스 후속 경고(원문은 이 수정을 U13 산출물에 배정) 재확인.
- 메모리: [[vibeue-buildgraph-pie-worldleak]](BP 편집 후 같은 세션 PIE=World Leak 크래시→편집 후 에디터 재시작 Don't Save), [[seamless-travel-input-loadout]](2-client PIE net.AllowPIESeamlessTravel=1), [[haiku-delegation-security-wiring]].

[브랜치 / 모델 정책]
- main에서 phase/p6-animation-pass 분기(§6-7), 검증 후 --no-ff 머지.
- 플랜 우선 → 승인 후 구현=Sonnet 위임 / 검증=Opus 직접.
- ⚠️ 위임 카브아웃(Opus 직접): (B) 원격 가시 3P 발사/재장전 애님 트리거 배선 = 서버권위 표면. (C) 적 애니 상태 소스·트리거 신호 결정(신규 복제 프로퍼티/RPC 추가 여부) = 복제 계약 표면. **(공통) GMS 통복사 교체(재진입 가드 in-place/스크래치 버퍼)=핫패스 힙할당 계약 표면 → Opus 직접.** A는 서버권위 표면 없음(bReloading 이미 복제·소비만)이라 구현 Sonnet 가능하되 재장전 트리거 훅(OnRep vs 폴링)은 Opus 검증. 3P 메시 컴포넌트 생성·DA 필드 추가·AnimBP 그래프 스캐폴딩은 Sonnet 위임 가능.

[브랜치 전략 — 단일 브랜치 순차 스테이지]
- 단일 브랜치에서 A→B→C 순차 스테이지, 각 도메인 끝에 독립 빌드+PIE 체크포인트(스테이지 경계=안전 롤백 지점). 세 도메인 통과 후 통합 --no-ff 머지.
- 순서 근거: A가 WeaponDataAsset 1P visual 블록·PlayWeaponFireCosmetics 훅을 확립 → B가 그 대칭(WeaponMesh3P/FireMontage3P·MulticastFireCosmetics 원격분기)을 얹음 → C는 코드 표면 완전 분리(FPSREnemyBase)라 독립 마지막.
- C perf 실측이 장기 블로킹되면(U14 이월) 그 시점에 C만 phase/p7-enemy-anim로 분리 재판단.
- ⚠️ GMS 통복사 수정은 U13(phase/p7-vfx-audio)과 공유 breach — 아래 통합 함정 ③ 소유권 규칙 따를 것.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 도메인 A — 1P 무기 애니메이션 (순수 로컬 코스메틱)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
V0가 스켈레탈 무기 경로를 코드에 완비 → A = 재작성 아님. DA 채우기 + 재장전 훅 신규 + 모듈러 부품 부착.

[이미 존재 — 재사용, 재생성 절대 금지 (실측)]
- AFPSRCharacter::RefreshFirstPersonWeaponVisual()[FPSRCharacter.cpp:932]: SkelMesh 우선/StaticMesh 폴백 → WeaponMesh1P->SetSkeletalMeshAsset(:976, null 클리어=:958)+AttachToComponent → ArmsAnimInstanceClass 적용(bArmsAnimOverridden 가드) → Cached 발사 코스메틱 캐싱 → EquipMontage 재생. **재작성 금지 — 모듈러 부품 부착·오프셋만 확장.**
- WeaponMesh1P:USkeletalMeshComponent[생성자 82-86(SetOnlyOwnerSee(true):83), h 선언] — FirstPersonArms(WeaponAttachSocketName)에 SetupAttachment·SetOnlyOwnerSee(true)·NoCollision. **컴포넌트 신설 금지.** DA WeaponMesh1P 채우면 코드 자동 사용(현 V0 8종은 WeaponMeshStatic1P만 채워 정적 경로).
- WeaponMeshStatic1P:UStaticMeshComponent[생성자 88-90(SetOnlyOwnerSee(true):90)] — 칼(정적) 폴백.
- PlayWeaponFireCosmetics()[FPSRCharacter.cpp:1036]: IsLocallyControlled early-return 후 CachedFireMontage를 팔 AnimInstance에 Montage_Play + 머즐/사운드. **노리쇠(무기 AnimBP) 몽타주·팔↔무기 동기 지점=여기(단일 훅). 이 함수는 로컬 오너 전용 — 원격 3P 발사는 도메인 B의 MulticastFireCosmetics(:1067)이지 여기가 아님.**
- UFPSRWeaponDataAsset '1P visual' 블록[FPSRWeaponDataAsset.h ~72-124]: WeaponMesh1P/WeaponMeshStatic1P/ArmsAnimInstanceClass/WeaponAttachSocket/MuzzleSocket/EquipMontage/FireMontage/MuzzleFlash/FireSound 전부 존재(Soft ref, null=무회귀). **필드 재추가 금지.**
- 재장전 서버 로직: UFPSRWeaponInventoryComponent::StartReload/FinishReload 서버권위·IsRunPaused 게이트·ReloadTime 타이머·프리즈 시 Pause/UnPause. UFPSRWeaponInstance::bReloading[DOREPLIFETIME cpp:23, SetReloading:135 MARK_PROPERTY_DIRTY_FROM_NAME]. **RepNotify/OnRep 콜백 없음 — 순수 복제 프로퍼티(실측 확인). OnRep 채택 시 UFUNCTION 신규 필요.**
- 콘텐츠 자원 완비(제작 아닌 배선): Infima PCH 팔 애님(A_FP_PCH_<CODE>_Fire/Reload/Reload_Empty/Idle_Loop/Sprint_F_Loop/Jump_*, 스켈=SKEL_LPAMG_Character), WEP 무기 애님(A_FP_WEP_<CODE>_Fire/Reload/Reload_Empty=노리쇠·탄창, 무기스켈=SKEL_LPAMG_<CODE>, SK_LPAMG_<CODE> 리시버), 모듈러 부품(SM_LPAMG_<CODE>_{Forestock/Ironsight/Magazine}), 1P 팔(SK_LPAMG_Arms_Gloves). **팩 코드=AG14W/MAK12/SP60/LRAF9/X13/HVG7/RC425/MR22(리터럴 <W> 아님, git ls-files 확인).**

[남은 것 — A 실작업]
- (C++) 재장전 몽타주 훅 신규: bReloading 소비 경로(OnRep_Reloading 신규 vs 팔 AnimBP IsReloading() 폴링). OnRep=서버확정 시점 정확·대역폭 최소, 폴링=도메인 B 3P와 코드 일관(B는 폴링 채택) — 플랜에서 근거 명시. bReloading은 RepNotify 아님 실측이라 OnRep 채택 시 UFUNCTION 신규 필요. 재장전 몽타주 길이=Stats.ReloadTime 정합(PlayRate 스케일).
- (C++) 모듈러 부품 동적 부착(RefreshFirstPersonWeaponVisual 확장): DA 신규 TArray<FFPSRWeaponPartAttachment{TSoftObjectPtr<UStaticMesh> Part; FName Socket; FTransform Offset;}> → 스켈 무기 장착 시 WeaponMesh1P 소켓에 부품 UStaticMeshComponent 부착(가변개수→런타임 생성/풀). null-safe, 정적(칼)·부품無 스킵. 비대해지면 헬퍼 분리.
- (C++, 선택) 무기별 부착 오프셋 DA 필드(V0 후속 ③=A 흡수).
- (콘텐츠) 총기 DA를 WeaponMeshStatic1P(Preview)→WeaponMesh1P(SK_LPAMG_<CODE>) 전환 + ArmsAnimInstanceClass(무기별 팔 AnimBP) + FireMontage/EquipMontage/ReloadMontage(신규) + 모듈러 부품목록. 1종 먼저 PIE 후 확대(롤백 안전).
- (콘텐츠) 무기 AnimBP(스켈 SKEL_LPAMG_<CODE>): idle+발사(WEP_Fire=노리쇠)+재장전(WEP_Reload/Reload_Empty). 팔 AnimBP(스켈 SKEL_LPAMG_Character): 로코모션(Idle_Loop/Sprint_F_Loop/Jump BlendSpace)+발사/재장전 몽타주 슬롯+무기별 그립. **무기별 그립=BlendByEnum(EFPSRWeaponArchetype, GetArchetype() 존재) vs ArmsAnimInstanceClass 무기별(코드 이미 지원) — DESIGN-FIRST로 ArmsAnimInstanceClass 재사용 우선.** 손 IK는 팩 PCH 애님이 무기별 손위치 포함이면 생략(PIE 확인 후, 과설계 지양).
- BP_FPSRPlayer FirstPersonArms 메시=SK_LPAMG_Arms_Gloves 확인(C++가 WeaponMesh1P/Static 자동 관리 — BP에서 건드리지 말 것).

[A 조사] FPSRCharacter.cpp:932-1034(Refresh 전문, null 클리어 958·SetSkeletalMeshAsset 976)·1036-1066(PlayWeaponFireCosmetics)·1067~(Multicast=도메인 B 경계) / FPSRCharacter.h(컴포넌트·Cached*/ActiveWeaponMesh/bArmsAnimOverridden) / FPSRWeaponDataAsset.h ~72-124 / FPSRWeaponInventoryComponent(StartReload/FinishReload·IsRunPaused 게이트) / FPSRWeaponInstance.cpp:23,135. git ls-files "Content/Assets/LowPolyAnimatedModernGuns/*"로 SK/SKEL/SM 실재 확인. ⚠️ 착수 전 심볼로 재grep(라인 재드리프트 가능).

[A 플랜 결정] ① 재장전 트리거: OnRep_Reloading vs 폴링(B 동형) — 대역폭·정확성·B 일관성 근거. ② 노리쇠 재생: 무기 몽타주 vs 무기 AnimBP 상태머신, 팔 몽타주와 프레임 동기(단일 훅 PlayWeaponFireCosmetics 확정). ③ 모듈러 부품 데이터 구조(배열+소켓+오프셋 struct 권장, 런타임 컴포넌트 생성=고정풀 vs 런타임). ④ 손 IK 필요성(PIE 후 판단).

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 도메인 B — 3P 팀원 캐릭터 애니메이션 (협동 가시성 · 복제 carve-out=Opus 직접)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
4인 협동에서 다른 플레이어 3P 바디가 서로에게 보이고 이동/발사/재장전 애님 재생. 자기 자신은 1P 팔만.

[이미 존재 / 앵커 — grep 확인, 지어내지 말 것]
- FPSRCharacter.cpp:95-97 GetMesh()→BodyMesh + SetOwnerNoSee(true) 이미 존재(실측 95-97). 3P 무기 컴포넌트는 부재(1P만) → WeaponMesh3P 신규 정당.
- MulticastFireCosmetics_Implementation = **FPSRCharacter.cpp:1067**(U19 원안의 1054/1059는 stale 드리프트). 현 멀티캐스트 실측 구조: (1) IsLocallyControlled early-return(1072-1075) — 로컬 오너·리슨서버 호스트 스킵 (2) Weapon=WeaponInventory->GetCurrentWeapon() null 가드 (3) `if (LocalPC && LocalPC->GetViewTarget()==this)` 관전자 전용 1P muzzle/montage 블록(1091~). **3P 바디 발사 몽타주는 early-return 뒤·이 뷰타깃 게이트 밖(모든 원격 프록시 fall-through)에서 GetMesh() AnimInstance에 FireMontage3P 재생 — 자기 3P는 SetOwnerNoSee라 안 보이므로 IsLocallyControlled 스킵 유지가 곧 원격 전용.** ⚠️ 착수 전 함수 전문(1067~끝) 재확인해 삽입 지점 확정.
- FPSRWeaponFragment.cpp:28 Char->MulticastFireCosmetics()(서버 발사확정 훅, 클라 위조 불가).
- RefreshFirstPersonWeaponVisual(FPSRCharacter.cpp:932)은 all-clients 실행(efdb07f로 !IsLocallyControlled 게이트 제거됨) → 3P 무기 메시 배선 대칭 추가 지점.
- FPSRWeaponInstance bReloading(Replicated, RepNotify 아님 실측 cpp:23/135) — 3P AnimBP 폴링 신호(RPC 불요).
- FPSRCharacter CalcCamera + GetBaseAimRotation/RemoteViewPitch16 — 3P 상하 aim offset 소스(정밀 pitch 별도 복제 불요).
- FPSRWeaponDataAsset.h ~72-124 '1P visual' 블록 — 3P 필드 대칭 위치.
- PROGRESS 캐릭터 애셋 STEP4: BP_FPSRPlayer.CharacterMesh0 ← SK_BCC_01_BroBot + BCC_01_BroBot_AnimBlueprint(정적 배선 완료). BroBot 로코모션 자원(Idle/Walk/Run/Loop/Jump/Jump_Start/IdleRun_Blendspace) 존재, **발사/재장전 몽타주는 부재.**

[B 산출물 — C++ (복제/서버권위 트리거=Opus 직접)]
1. 3P 바디 가시성 정합: GetMesh() 이미 SetOwnerNoSee(true)만 설정(cpp:95-97). 신규 3P 바디 컴포넌트 불요. 그림자/Tick 옵션 점검(불요 시 생략, 하드코딩 지양).
2. 3P 발사/재장전 애님 트리거(서버권위, Opus 직접): (발사) MulticastFireCosmetics_Implementation(:1067)의 early-return 뒤·뷰타깃 게이트 밖 원격 fall-through 분기에서 GetMesh() AnimInstance에 FireMontage3P 재생(WeaponDataAsset 신규 소프트 필드 로드). (재장전) 전용 멀티캐스트 부재 → 3P AnimBP가 복제 bReloading을 매 프레임 폴링, false→true 엣지에서 ReloadMontage3P 재생(별도 RPC 불요=최소 대역폭). 몽타주 길이-재장전시간 정합=콘텐츠 튜닝.
3. WeaponDataAsset 3P 필드(데이터주도, 경로 하드코딩 금지): 1P visual 블록 대칭으로 TSoftObjectPtr<USkeletalMesh> WeaponMesh3P, TSoftObjectPtr<UAnimMontage> FireMontage3P/ReloadMontage3P, FName WeaponAttachSocket3P(3P 손 소켓). 전부 null=애님 없음(null-safe).
4. RefreshFirstPersonWeaponVisual에 3P 무기 메시 배선 대칭 추가(3P 손 소켓에 WeaponMesh3P attach). 신규 USkeletalMeshComponent WeaponMesh3P를 생성자에서 GetMesh()에 SetupAttachment + **SetOwnerNoSee(true)**(1P WeaponMesh1P는 SetOnlyOwnerSee(true):83 — 정확한 대칭 반대값, 실측 확인). 함수 비대해지면 RefreshThirdPersonWeaponVisual 분리 판단(구현 시).

[B 콘텐츠] 3P 로코모션 AnimBP(BCC_01_BroBot_AnimBlueprint 배선됨): 복제 Velocity 기반 IdleRun_Blendspace 점검 + 발사/재장전 몽타주 슬롯 추가. 발사/재장전 몽타주 부재 → (i) 우선 로코모션만 검증 or (ii) Content/Assets/Characters/Mannequins/Anims/Rifle 리타깃 검토. DA_Weapon 3P 필드 채우기(자원 있는 무기부터).

[B 함정] ⚠️Acceleration은 시뮬레이트 프록시에서 복제/유효하지 않으므로 3P AnimBP를 **Velocity 기반으로만** 설계(Accel 참조 금지). 신규 복제 프로퍼티 추가 지양(로코모션=CharacterMovement 복제 Velocity, 발사=기존 MulticastFireCosmetics, 재장전=복제 bReloading 폴링). **[DBNO 관전 회귀 — 핵심 미해결]** §2-13 관전은 아군을 SetViewTarget으로 보고 CalcCamera가 비로컬 시 GetBaseAimRotation으로 1P 카메라 구동. UE OwnerNoSee/OnlyOwnerSee는 로컬 PC 뷰타깃 기준 평가 — 관전 시 아군 1P 팔+아군 3P 바디 동시 렌더 여부·오버랩 흉함은 **엔진 동작 단정 말고 PIE로 확정**(PROGRESS "흉하면 v2 관전카메라"가 이 판정, 이 패스에서 결론). ⚠️ 실측: MulticastFireCosmetics 관전자 게이트(1091~)가 관전 시 아군 1P muzzle/montage를 이미 렌더하므로, 3P 무기 메시(SetOwnerNoSee)와 겹쳐 관전 시 이중 표시 가능 — PIE로 확인. efdb07f가 관전 시 아군 1P 무기 표시 해소했으니 그 회귀도 함께 확인.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 도메인 C — 적 애니메이션 (⚠️기법 다름: VAT 파이프라인 확장, 스켈+AnimBP 직결 금지)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
동시 200-300(캡500) 적 이동/공격/사망 애니를 액터당 CPU 애님 ~0에 근접. ⚠️ VAT '도입' 아님 — **이미 배선·머지된 VAT 파이프라인 확장**. 제1원리="액터당 비용 최소"→스웜 300에 SkeletalMesh+AnimBP 직결 절대 금지(A/B 기법 복붙 금지). 공백=현 VAT가 단일 AutoPlay 워크 루프만 재생(상태전환/속도매핑/거리LOD 없음) → 이동속도→재생속도·상태전환·거리티어 LOD를 C++로 배선.

[현 코드/콘텐츠 사실 — grep·PROGRESS 확인, 지어내지 말 것]
- AFPSREnemyBase(Enemy/FPSREnemyBase.*): 메시=UStaticMeshComponent Mesh(생성자 cpp:34, /Engine/BasicShapes/Cube 폴백, NoCollision). **SkeletalMesh·AnimBP 없음. 애니 파라미터/다이내믹 머티리얼 배선 전무**(CreateDynamicMaterial/SetScalarParameter/AnimationIndex 0건 grep 확인).
- **VAT 파이프라인 이미 존재(콘텐츠 머지·PIE 대기)**: 스웜 Content/Assets/Characters/Paragon/VAT/SM_Minion_Melee_VAT(41본/55프레임)·엘리트 SM_Minion_Siege_VAT(39본/57프레임) + DA_Minion_Melee_VAT/DA_Minion_Siege_VAT/MI들. 제네릭 마스터 Content/Assets/Characters/BroBot/VAT/M_BroBot_VAT(Use Material Attributes + MF_BoneAnimation, BONE 모드). BP_EnemyBase.Mesh CDO가 VAT StaticMesh로 스왑됨. AnimToTexture 플러그인 enable(uproject).
- **현 VAT 한계=단일 AutoPlay 루프**: DA_*_VAT가 AnimSequences=[Walk/Jog 1종]·bAutoPlay=true·AnimationIndex=0(Archive/guides/AssetWork_CharEnv_Resume.md:46). idle/attack/death 상태전환·속도반영·거리정지 없음=공백.
- UAnimToTextureDataAsset 필드(Archive/guides/AssetWork_CharEnv_Resume.md:73): AnimSequences[]·AnimationIndex·Frame·bAutoPlay·SampleRate. **이동/공격/사망을 별도 시퀀스로 베이크해 배열에 넣고 C++에서 AnimationIndex/재생속도를 MID로 구동**하는 것이 확장 경로. ⚠️M_BroBot_VAT/MF_BoneAnimation이 노출하는 실제 파라미터명(프레임·애니인덱스·재생속도)은 이 클론에서 확인 불가 → **플랜에서 에디터/엔진 소스로 파라미터명 grep 확인 후 배선(지어내지 말 것)**.
- 이동=서버권위. TickServerMovement(FPSREnemyBase.cpp:207)가 위치/Yaw 갱신, 속도=CurrentMoveSpeed(cpp:110, MoveSpeed×±10% on Activate). 복제=Transform만(§5). 이동 애니 속도=복제 Transform 변화율 또는 기존 값 추정(신규 복제 금지).
- 거리 LOD 이미 존재: UFPSREnemySpawnSubsystem 배치 패스가 최근접 플레이어 거리로 티어 계산→UpdateStride(1/2/4/8)+NetFreq. 반경 constexpr TierS0RadiusSq=1500²·TierS1=3500²·TierS2=6000²(SpawnSubsystem.h:150-152). **현재 이동 stride·net freq에만 쓰이고 애니엔 미연결** → 애니 LOD(원거리 정지)를 여기 얹음(신규 거리계산 금지).
- SignificanceManager 플러그인=uproject enable but 적 코드 미사용(자체 티어). 플러그인 enable ≠ 최적화(§5-1 경고).
- 공격 신호원: ServerTickAttack(h:112)이 서버에서 근접/차징 결정. 원거리=AFPSRRangedEnemyBase가 ClientNotifyRangedTarget(FPSRRangedEnemyBase.cpp:209→PC client RPC, FPSRPlayerController.h:118 시그니처=int32 SourceId,FVector SourceLocation,bool bActive) 사전경고. 사망=HandleDeath(h:128), 피격=ApplyKnockback(h:117). ⚠️ClientNotifyRangedTarget은 "대상 PC 방향경고"용이지 적 애니 트리거 아님 — 재사용 검토는 하되 근접/사망 애니엔 별도 신호(함정②).
- 엘리트/보스 스켈 레퍼런스: 플레이어/로비 3P는 이미 풀 스켈+AnimBP(BP_LobbyDisplayPawn). 세터=USkeletalMeshComponent::SetSkeletalMeshAsset(UE5.7 문서화, 프로젝트 실사용 FPSRCharacter.cpp:958/976=무기메시 — 캐릭터 바디 아님, 패턴만 참고).

[C 재사용 앵커] AFPSREnemyBase::Mesh(UStaticMeshComponent, VAT 스왑됨) cpp:34=MID 생성·파라미터 대상. Content VAT 자산(SM_Minion_*_VAT, DA_Minion_*_VAT, M_BroBot_VAT+MF_BoneAnimation)=확장할 기존 자산. UAnimToTextureDataAsset AnimSequences[]/AnimationIndex/Frame(Resume.md:73). TickServerMovement:207/CurrentMoveSpeed:110. SpawnSubsystem 티어(h:150-152). Activate/Deactivate(virtual, h:62,67)=풀 재사용 훅(애니 리셋). ServerTickAttack(h:112)/HandleDeath(h:128)/ApplyKnockback(h:117)=트리거 신호원. ClientNotifyRangedTarget(PC.h:118)=원거리 사전경고 RPC(오남용 주의). UFPSREnemyRosterDataAsset/UFPSREnemySpawnRule(EditInlineNew 폴리모픽)=아키타입별 렌더방식 데이터드리븐 선택 인프라.

[C 플랜 결정 — VAT 채택 여부는 결정 아님(이미 배선). 아래는 '확장' 설계]
1) VAT 상태전환 배선(스웜 주공정): 이동/idle/공격/사망을 DA_*_VAT.AnimSequences[]에 다중 베이크(콘텐츠=사용자) + C++에서 상태별 AnimationIndex/재생속도를 Mesh의 MID로 세팅. **확인 지시(엔진/에디터, 지어내지 말 것)**: (i) M_BroBot_VAT/MF_BoneAnimation 실제 파라미터명(애니인덱스/프레임/재생속도/시퀀스 시작·길이) — 에디터 파라미터 목록 or 엔진 AnimToTexture MF_* grep. (ii) 다중 시퀀스 프레임 오프셋/길이 per-actor 전달 정석(엔진 예제 대조). (iii) CurrentMoveSpeed→재생속도 매핑. **⚠️per-actor MID 300개=MID 300개 비용** → 티어별 MID 갱신 빈도 제한(S2 이하 갱신 스킵) 또는 CustomPrimitiveData가 더 싼지 엔진 소스 비교 후 채택(⛔ A/B의 per-instance 스켈 AnimInstance 접근을 여기 복붙 금지).
2) 거리 LOD 애니: 기존 TierS*RadiusSq 재사용 — S0=풀 갱신, S1=저빈도, S2이하=정지/프레임고정(§5-1). ⚠️VAT는 머티리얼 AutoPlay라 원거리도 GPU가 계속 돌 수 있음 → 정지=재생속도0 또는 갱신 스킵으로 CPU측 파라미터 라이트 제거. 신규 per-enemy 월드쿼리 금지.
3) 엘리트/보스 스켈 하이브리드: 소수(≤수십)라 스켈+AnimBP 감당. SM_Minion_Siege_VAT를 스켈 승격 vs VAT 유지+보스만 스켈 결정. 아키타입 렌더방식=UFPSREnemyRosterDataAsset/BP DataAsset 데이터드리븐(중앙 enum/switch 지양).
4) 애니 상태 소스·트리거 신호(복제 계약 — Opus 직접): 애니=100% 클라 코스메틱. 이동=복제 Transform 변화율 추정(신규 복제 금지). 공격/사망 트리거=서버→클라 신호 필요 판단 — ClientNotifyRangedTarget은 방향경고용 부적합, 근접·사망은 §5("히트/사망 코스메틱=GameplayMessage/Cue") 따라 **GameplayMessageSubsystem(U8 완료) 브로드캐스트 재사용 우선**(신규 복제 프로퍼티/RPC 추가 시 300적×대역폭 비용 명시). ⚠️**U8 GMS 핫패스 후속(PM 감사 2026-07-02): BroadcastMessageInternal(FPSRGameplayMessageSubsystem.cpp:37)이 브로드캐스트마다 리스너 `TArray<FFPSRMessageListenerData> ListenerArray(List->Listeners)` 통복사 → C 적 사망 상주 리스너 붙이는 순간 ×수백/프레임에서 §5 "발행 핫패스 힙할당 금지" 위반. 통합 함정 ③의 소유권 규칙(U13 공유) 따라 상주 리스너 배선 전에 통복사를 재진입 가드 in-place 순회/예약 스크래치 버퍼로 교체 + 채널단위 early-out 검토.**
5) §8 큐브/폴백=BP CDO 레벨(콘텐츠). C++ = 상태전환·속도·LOD 파라미터 훅 + MID/CustomPrimitiveData 관리 + Activate/Deactivate 애니 리셋 + 엘리트 스켈 슬롯. 경로 하드코딩 금지.

[C 산출물] C++: AFPSREnemyBase(또는 신규 경량 애님 컴포넌트 — DESIGN-FIRST로 기존 Mesh·Activate/Deactivate·Health/Feedback 컴포넌트 재사용 우선 grep 후 결정)에 (a) VAT 파라미터 구동(상태→AnimationIndex/재생속도, MID or CustomPrimitiveData) (b) 이동상태→속도 매핑 (c) 거리티어 연동 애니 LOD (d) 공격/사망/피격 신호→트리거(코스메틱) (e) Activate/Deactivate 리셋 (f) 엘리트/보스 스켈(SetSkeletalMeshAsset 패턴). 신규 복제/RPC 최소화(추가 시 근거). 콘텐츠(사용자, 이 유닛 밖): 추가 애니 시퀀스(idle/attack/death) VAT 베이크→AnimSequences[] 확장 + bAutoPlay=false(C++ 구동) + 엘리트/보스 스켈메시·AnimBP + 아키타입별 렌더방식 DataAsset.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 통합 함정 (제1원리 — 세 도메인 위험 프로파일이 다름)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
① 위험 프로파일 분기(핵심): A/B=플레이어측 경량 스켈(≤4 액터, 자기 3P는 SetOwnerNoSee로 로컬 렌더 0 → 비용 무관, **제1원리 성능 제약의 명시적 예외 도메인**) vs C=적 perf-critical(200-300, 캡500 → 액터당 비용=프레임예산, VAT GPU 필수). ⛔ **A/B의 SkeletalMesh+AnimBP+몽타주 기법을 C에 복붙 절대 금지**(적 300 스켈직결·per-instance AnimInstance 접근=제1원리 정면 위반, 이미 VAT 회피됨).
② 복제 계약도 도메인별: A=수치/복제 불변(순수 로컬, bReloading 이미 복제·소비만·RepNotify 아님), B=복제 carve-out(원격 가시 3P 트리거=서버권위=Opus 직접, 신규 복제 지양·기존 MulticastFireCosmetics(:1067)/bReloading 폴링 재사용, Accel 참조 금지), C=신규 복제 절대 금지(300적×복제=대역폭 폭증, 이동=Transform 추정·공격/사망=GMS 재사용).
③ U8 GMS 핫패스 후속 — **수정 소유권=U13과 공유**: BroadcastMessageInternal(cpp:37, `TArray ListenerArray(List->Listeners)` 통복사)는 U13(핑/Gibs, phase/p7-vfx-audio)과 이 패스 C(적 사망 GMS 리스너)가 **같은 breach를 공유** — 적 사망에 상주 리스너를 먼저 붙이는 쪽이 GMS 수정을 소유한다(원문은 이 수정을 U13 산출물에 배정했으나, 이 패스가 먼저 리스너를 붙이면 여기가 breach 도입 지점이므로 여기가 소유). 착수 플랜에서 U13 진행 상태 확인: 이 패스가 먼저 랜딩→여기서 GMS 통복사를 in-place 재진입 가드/예약 스크래치 버퍼로 교체(+채널단위 early-out) & Opus 직접; U13이 먼저→rebase로 흡수(수정 중복·머지충돌 회피). ⚠️ B/C가 GMS 상주 리스너를 안 붙이면(폴링/멀티캐스트만 사용) 이 패스는 GMS 수정 불요 → U13에 남김.
④ 도메인 경계 명확: 3P 무기 애님=도메인 B(not A — A는 1P 로컬만, PlayWeaponFireCosmetics(:1036)=로컬 오너 전용, 원격 3P는 MulticastFireCosmetics(:1067)), 적=VAT(not 스켈), U12 발사 케이던스/블룸 스프레드 '수치'는 U12 유지(A는 그립 BlendByEnum '애님'만 흡수, 중복 구현 금지 — U15는 게임플레이 수치·트레이스 불변). 전역 프리즈(§2-2) 중 몽타주 정지(A 재장전 훅도 IsRunPaused 확인, StartReload는 이미 게이트·타이머 Pause). Demo/Mannequin 폴더 사용 금지, 에셋 경로 C++ 하드코딩 금지(전부 DA Soft ref). ⚠️앵커 드리프트: MulticastFireCosmetics=1067, 3P 바디=GetMesh()→BodyMesh:95-97, WeaponMesh1P SetOnlyOwnerSee:83 — 착수 전 심볼로 재grep.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 통합 검증 (도메인별 PIE 체크포인트 + 공통 무회귀)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- 공통: 빌드(-WaitMutex) Succeeded + 헤드리스 스모크(ModuleLoads Success). 각 스테이지 끝에 독립 빌드+PIE.
- 도메인 A PIE(1P, single-client): 발사 시 팔↔무기 노리쇠 왕복 동기, 재장전 애님↔ReloadTime 정합, 무기별 모듈러 부품 정확 부착, 무기 교체 시 SK/부품 갱신, 칼=정적 폴백 유지, 프리즈 중 몽타주 정지, 게임플레이 수치(반동/탄약/데미지/연사·점사·차징) 무회귀.
- 도메인 B PIE(2-client 이상, net.AllowPIESeamlessTravel=1): (a) 상대 3P 바디 상호 가시 (b) 이동 로코모션 (c) 발사 시 3P 발사 반응 (d) 재장전 시 3P 재장전 (e) 자기 화면=3P 안 보이고 1P 팔만 (f) DBNO 관전 시 아군 1P 뷰 무회귀 + 1P/3P 오버랩·관전자 muzzle중복 흉함 판정(v2 필요 시 후속 등재) (g) 상하 조준 3P 반영.
- 도메인 C PIE(먼저 baseline VAT 재생 확인): 이동/공격/사망 상태전환 재생, 이동속도별 재생속도 반영, 거리별 애니 LOD(원거리 정지), 200-300 스폰 시 프레임/hitch 관찰(정량 실측은 U14 perf 패스 이월 — §5 가독성 가드레일과 함께). GMS 리스너 배선 시 적 사망 ×수백/프레임 힙할당 0 확인(통복사 교체 검증).
- Scripts/codex-review.ps1 -Base main(B 복제/서버권위 트리거 + C 상태소스·신호 게이트 + GMS 통복사 교체 = Opus 직접 검토).
- PROGRESS 갱신 + TaskPrompts §B에 통합 애니메이션 패스(U15/U19/U20 흡수) ✅ + 사용자 콘텐츠 동반 커밋 질문 + --no-ff 머지 + PIE 라운드2/3 후속(3P 바디 가시성/관전 오버랩) 해소 기록 + (GMS 수정 소유 시) U13 프롬프트에서 GMS 항목 제거 기록.
```

### U15 — 1P 무기 애니메이션 시스템 (모듈러 + 노리쇠 + 1P 팔·손 강화)

> ▶ **애니메이션 통합 패스(위 🎬)로 U15+U19+U20 병행 실행** — 실행은 통합 프롬프트 사용. 아래는 도메인 A(1P 무기) 원안 참조용.
...
> **프롬프트 갱신 2026-07-02**: 코드베이스 그라운딩 + 적대 검증(ultracode, verdict PASS). ⚠️ **원안 stale 교정**: "설정 인프라 전무"라 기술했으나 오디오 설정 MVP로 **`UFPSRGameUserSettings`·설정 위젯 `UFPSRSettingsWidget`·메인메뉴 Settings 진입·WBP 5개 모두 이미 존재**. 실제 남은 작업 = **CrosshairScale 추가(기존 MasterVolume 패턴 복제)+슬라이더 항목+크로스헤어 위젯 바인딩**뿐. 핵심 drift: 오디오 슬라이더는 `UFPSRAudioSubsystem` 경유지만 크로스헤어는 서브시스템 없이 GUS 직접+델리게이트→HUD.

```
[작업] U17 — 플레이어 설정 시스템의 두 번째 슬라이스 = 크로스헤어 크기(Clamp 0.5~2.5, 기본 1.0)를 인게임 슬라이더로 조절·세션 간 영속·HUD 실시간 반영. **주의: 이 프롬프트는 오디오 설정 MVP 이후 코드베이스로 실측 그라운딩됨(검증자 Opus가 각 앵커를 Grep/Read 대조). 설정 인프라는 이미 존재 — 확장만 한다.**

■ 먼저 읽을 것
- Game.md(§0-1 라우팅 가이드)로 이 작업 도메인 SSOT만 선별 + PROGRESS.md 최신 핸드오프
- Docs/SSOT/Architecture.md §3-4(설정/세이브 위치) — 오디오 설정 반영분 확인
- Docs/SSOT/PlayerFeel.md §2-14(HUD/크로스헤어), Docs/SSOT/Workflow.md §6·§6-7
- Docs/SoundSettings_Handoff.md — 오디오 MVP가 GUS/위젯/메뉴/서브시스템을 어떻게 배선했는지(동형으로 크로스헤어 추가)
- 메모리 [[vibeue-mcp-capabilities]], [[vibeue-umg-button-binding]], [[vibeue-buildgraph-pie-worldleak]]

■ 브랜치 / 모델 정책
- main에서 phase/settings-system 분기(§6-7), 검증 후 --no-ff 머지
- 플랜 우선 → 승인 후 **구현=Sonnet 위임 / 검증=Opus 직접**(설정 영속+델리게이트 배선 세밀 검증)
- 순수 로컬 비복제라 안티치트/서버권위 표면 없음 → 위임 카브아웃 해당 없음

■ 현재 상태 — 이미 된 것 vs 남은 것 (⚠️ 기존 인프라 재생성 금지)
[이미 존재 — 재사용, 재생성 절대 금지 (전부 실측 확인)]
- UFPSRGameUserSettings : UGameUserSettings [Source/FPSRoguelite/Public/Settings/FPSRGameUserSettings.h + Private/Settings/FPSRGameUserSettings.cpp] — MasterVolume(UPROPERTY(config) float, protected) + static Get() + GetMasterVolume()(BlueprintPure) + SetMasterVolume(float,bool bSave){Clamp(0,1); if(bSave)SaveSettings();} + 생성자 초기화. **이미 구현됨.** 클래스 신설 금지. (주의: 오디오 세터엔 Broadcast 없음 — 델리게이트는 이번 유닛 신규.)
- Config/DefaultEngine.ini:25 GameUserSettingsClassName=/Script/FPSRoguelite.FPSRGameUserSettings **이미 등록됨(라인25 실측).** ini 추가 편집 불필요.
- UFPSRSettingsWidget : UCommonActivatableWidget(Abstract) [Source/FPSRoguelite/Public/UI/FPSRSettingsWidget.h + Private/UI/FPSRSettingsWidget.cpp] — MasterVolumeSlider(**meta=BindWidget**) + NativeOnInitialized에서 SetMinValue(0)/SetMaxValue(1) + OnValueChanged/OnMouseCaptureEnd/OnControllerCaptureEnd 배선 + NativeOnActivated→SyncFromSettings + UpdateValueText. WBP_Settings가 이 클래스 서브클래스. **위젯 신설 금지 — 여기에 슬라이더 하나 추가.** ⚠️ 중요 drift: 오디오 슬라이더 핸들러는 **UFPSRAudioSubsystem::SetMasterVolume를 호출**(GUS 직접 아님)이고 SyncFromSettings도 **오디오 서브시스템**에서 읽는다 — 크로스헤어는 서브시스템이 없으므로 GUS를 직접 호출/조회(아래 산출물의 비대칭 근거).
- 메인메뉴 Settings 진입 **완성**: UFPSRMainMenuWidget::HandleSettingsClicked [FPSRMainMenuWidget.cpp:61] → Layout->PushWidgetToLayer(RequestGameplayTag("UI.Layer.Menu"), SettingsWidgetClass)(라인79). 인게임 Esc(IA_Menu)도 동일 위젯 push. WBP_MainMenu/WBP_SettingsButton/WBP_BackButton 에셋 존재. **진입/네비 작업 0.**
- WBP_BasicCrosshair.uasset(CrosshairRoot) [Content/UI/HUD/] 존재(§C V3 정적 크로스헤어).
- ⚠️ TaskPrompts_Master.md의 이 프롬프트 원안은 WBP_Settings를 '신규'라 적었으나 자산 실재 → **'기존 확장'으로 정정. 재생성 금지.**
[남은 것 — 이번 유닛 실제 작업]
- CrosshairScale 필드/게터/세터/**변경 델리게이트**를 UFPSRGameUserSettings에 추가(오디오엔 델리게이트 없음 — HUD 실시간 반영 위해 신규).
- 기존 WBP_Settings/UFPSRSettingsWidget에 크로스헤어 슬라이더 항목 추가.
- WBP_BasicCrosshair가 CrosshairScale 읽어 RenderTransform Scale 적용 + 델리게이트 구독(실시간).

■ 조사(먼저 Grep/Read로 재확인 — 아래는 검증자 실측치, 착수 전 재대조)
- FPSRGameUserSettings.h/.cpp 전문 — MasterVolume 게터/세터/SaveSettings/생성자 패턴을 CrosshairScale에 동형 복제(단 Broadcast 추가).
- FPSRSettingsWidget.h/.cpp 전문 — MasterVolumeSlider의 NativeOnInitialized SetMin/Max + OnValueChanged(라이브,bSave=false)/OnMouseCaptureEnd/OnControllerCaptureEnd(커밋,bSave=true) + SyncFromSettings + UpdateValueText 패턴. 크로스헤어 슬라이더를 동형으로(호출 대상만 GUS 직접).
- 델리게이트 컨벤션: FPSRReviveComponent.h:10 (DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam) / FPSRSaveGameSubsystem.h:22 (…_TwoParams) — 둘 다 UPROPERTY(BlueprintAssignable). 무인자면 DECLARE_DYNAMIC_MULTICAST_DELEGATE(접미사 없음).
- CrosshairScale grep = 코드 0건 → 실제 미구현 확인.

■ C++ 산출물
1. UFPSRGameUserSettings 확장(신설 아님):
   - 헤더 상단 델리게이트 선언 — 무인자 DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnCrosshairSettingsChanged)(HUD가 Get으로 재조회) 또는 OneParam(float NewScale). 프로젝트 컨벤션(Revive=OneParam) 참고, 플랜에서 결정.
   - UPROPERTY(config) float CrosshairScale = 1.0f; (MasterVolume와 동일하게 protected 배치)
   - UFUNCTION(BlueprintPure, Category="FPSR|Crosshair") float GetCrosshairScale() const { return CrosshairScale; }
   - UFUNCTION(BlueprintCallable, Category="FPSR|Crosshair") void SetCrosshairScale(float InScale, bool bSave = true); — FMath::Clamp(InScale,0.5f,2.5f); if(bSave)SaveSettings(); 그리고 **OnCrosshairSettingsChanged.Broadcast(...)**(broadcast는 bSave와 무관하게 항상 — 라이브 드래그도 HUD 반영. save만 릴리즈 시. 오디오에선 서브시스템이 라이브 apply를 담당했으나 크로스헤어는 broadcast가 그 역할).
   - UPROPERTY(BlueprintAssignable, Category="FPSR|Crosshair") FOnCrosshairSettingsChanged OnCrosshairSettingsChanged;
   - 생성자에 CrosshairScale=1.0f 초기화(MasterVolume와 동형).
2. UFPSRSettingsWidget 확장(신설 아님):
   - UPROPERTY(meta=(BindWidgetOptional)) TObjectPtr<USlider> CrosshairScaleSlider; (+선택 CrosshairScaleValueText는 BindWidgetOptional). **BindWidgetOptional** — 기존 WBP_Settings가 슬라이더 추가 전까지 컴파일 유지(MasterVolumeValueText/BackButton 방어 패턴과 동일). 주의: MasterVolumeSlider는 필수 BindWidget이지만 신규 크로스헤어 슬라이더는 Optional로 두어 순차 작업 안전.
   - NativeOnInitialized: if(CrosshairScaleSlider){ SetMinValue(0.5)/SetMaxValue(2.5) + OnValueChanged/OnMouseCaptureEnd/OnControllerCaptureEnd 바인딩 } (오디오 슬라이더와 동형, null 가드).
   - HandleCrosshairScaleChanged(float Value): **UFPSRGameUserSettings::Get()->SetCrosshairScale(Value, false)** — ⚠️ 오디오와 달리 서브시스템 경유 없이 GUS 직접 호출(가청적용 등가물 없음, 소비자=HUD; Get() null 가드). UpdateCrosshairText.
   - HandleCrosshairScaleCommitted(): GUS->SetCrosshairScale(CrosshairScaleSlider->GetValue(), true).
   - SyncFromSettings에 크로스헤어 슬라이더 초기화 추가 — **GUS(UFPSRGameUserSettings::Get())->GetCrosshairScale**에서 읽음(오디오는 GetAudioSubsystem()에서 읽으나 크로스헤어는 GUS 직접).
   - (주의: 서브시스템 신설하지 말 것 — 플랜 결정 항목 참조)

■ 콘텐츠 산출물
- WBP_Settings(기존): 크로스헤어 크기 슬라이더 + 레이블(+선택 값 텍스트) 추가, 위젯명 CrosshairScaleSlider로 BindWidgetOptional 매칭. 마스터볼륨 슬라이더 아래 배치. (VibeUE로 편집 후 에디터 재시작 후 PIE — [[vibeue-buildgraph-pie-worldleak]]).
- WBP_BasicCrosshair(기존): Event Construct에서 GetGameUserSettings→Cast<UFPSRGameUserSettings>→GetCrosshairScale로 CrosshairRoot RenderTransform Scale 설정 + OnCrosshairSettingsChanged 구독→핸들러에서 스케일 재적용(라이브 반영). 위젯 파괴 시 구독 정리(런 종료 시 위젯 소멸이나 델리게이트 스코프는 Destruct에서 명시 해제 권장).

■ 플랜 결정 항목(플랜에 명시)
- 서브시스템 신설 여부: 오디오는 UFPSRAudioSubsystem(값=GUS, 가청적용=서브시스템)로 분리했으나, 크로스헤어는 가청적용 등가물이 없고 소비자가 HUD 위젯이므로 **서브시스템 없이 슬라이더→GUS 직접 + 델리게이트→HUD** 구조. 이 비대칭(오디오와 다른 이유 = 소비자 위치)을 3줄 근거로.
- 델리게이트 시그니처: 무인자(HUD가 Get 재조회) vs OneParam(float). 프로젝트 컨벤션(Revive/Save 모두 Param 있음) + HUD 재조회 비용으로 결정.
- Broadcast 타이밍: 라이브 드래그(bSave=false)에서도 Broadcast해 실시간 반영, Save만 릴리즈에서.
- 슬라이더 배선 C++(BindWidget/Optional) vs 순수 BP: 오디오 일관성 위해 C++ 권장. 단 신규 슬라이더는 BindWidgetOptional로.

■ 함정(제1원리 3줄)
① 제1원리: 로컬 코스메틱 선호값 → 기존 UFPSRGameUserSettings 필드 추가가 최소비용(신규 클래스/위젯/서브시스템 0). ② (참고) Lyra=Local+Shared 분리지만 계정 없음 → Local 단일(오디오 MVP가 이미 확정). ③ 정합: 오디오와 값 소유(GUS)는 공유하되, 크로스헤어는 서브시스템 대신 델리게이트로 HUD 실시간 — 비대칭의 근거는 소비자 위치 차이(HUD 위젯 vs 오디오엔진).
- 에셋 경로 C++ 하드코딩 금지. GameUserSettingsClassName은 이미 등록됨(ini:25) — ini 재편집 금지.
- CrosshairScaleSlider는 BindWidgetOptional(기존 WBP 컴파일 보호). MasterVolumeSlider가 필수 BindWidget이라고 신규도 필수로 두지 말 것.
- ⚠️ 오디오 슬라이더 핸들러/SyncFromSettings는 서브시스템 경유 — 크로스헤어를 같은 패턴으로 복붙하면 존재하지 않는 크로스헤어 서브시스템을 호출하게 됨. GUS 직접 호출로 명시.
- 범위 = 크기만. 색/두께/불투명도·감도·FOV = 후속(같은 패턴 확장). 인게임 일시정지 메뉴는 U17 범위 밖(이미 인게임 Esc 오버레이로 진입 가능).
- VibeUE 위젯바인딩 getter는 컴파일 후, 편집 후 에디터 재시작 후 PIE([[vibeue-buildgraph-pie-worldleak]]).

■ 검증
빌드+헤드리스 스모크(코드) → 사용자 PIE: 메인메뉴 Settings→크로스헤어 슬라이더로 크기 실시간 변경(HUD 반영은 런 진입 후 확인)→런 진입 후 값 적용→에디터 재시작 후에도 유지(GameUserSettings.ini 영속)→기본값(1.0)·마스터볼륨 무회귀 → codex-review.ps1 -Base main → PROGRESS 갱신+✅+사용자 콘텐츠 동반 커밋 질문+--no-ff 머지.
```

### U19 — 3P 팀원 캐릭터 애니메이션 (협동 가시성) — ✨신규 2026-07-02(캐릭터 애님 사용자 추가)

> ▶ **애니메이션 통합 패스(§C 🎬)로 병행 실행** — 실행은 통합 프롬프트. 아래는 도메인 B(3P 팀원) 원안 참조용.
...
> ▶ **애니메이션 통합 패스(§C 🎬)로 병행 실행** — 실행은 통합 프롬프트. 아래는 도메인 C(적 VAT) 원안 참조용.
...
- Docs/Archive/guides/AssetWork_CharEnv_Resume.md (VAT 베이크 레시피·DataAsset 필드·`UAnimToTextureBPLibrary`/`UAnimToTextureDataAsset` API 목록 — 이 문서가 기존 VAT 계약의 SSOT)
...
- **현 VAT 한계 = 단일 AutoPlay 루프**: `DA_*_VAT`가 `AnimSequences=[Walk/Jog 1종]`·`bAutoPlay=true`·`AnimationIndex=0`으로 세팅(Archive/guides/AssetWork_CharEnv_Resume.md:46 확인). 즉 지금은 걷기 한 클립을 머티리얼이 GPU에서 무한 자가재생 = 액터별 로직 0. **idle/attack/death 상태전환·이동속도 반영·거리별 정지 없음 = 이 유닛의 공백.**
- `UAnimToTextureDataAsset` 필드(Archive/guides/AssetWork_CharEnv_Resume.md:73 확인): `AnimSequences[]`(배열 — 다중 애니 가능)·`AnimationIndex`·`Frame`·`bAutoPlay`·`SampleRate`. **이동/공격/사망을 별도 시퀀스로 베이크해 배열에 넣고, C++에서 `AnimationIndex`/재생속도 파라미터를 다이내믹 머티리얼로 구동**하는 것이 확장 경로. **단, `M_BroBot_VAT`/`MF_BoneAnimation`이 노출하는 실제 머티리얼 파라미터명(프레임·애니인덱스·재생속도)은 이 클론에서 확인 불가(엔진/에셋 접근 없음) → 플랜에서 에디터/엔진 소스로 파라미터명 grep 확인 후 배선. 지어내지 말 것.**
...
- `UAnimToTextureDataAsset` `AnimSequences[]`/`AnimationIndex`/`Frame`/`SampleRate`/`bAutoPlay` (Archive/guides/AssetWork_CharEnv_Resume.md:73) — 다중 애니·상태전환의 데이터 계약
...
| G1 | [Review/20260705-multimap-budget-regroup.md](Review/20260705-multimap-budget-regroup.md) | **#3 다중맵 아키텍처 설계 수렴**(Codex 5R): 전역 공유 캡 map-aware allocator · per-map 플로우필드 레지스트리 · NetCull→RepGraph→스트리밍 · 명제 "솔로=정찰/2인+=본게임" · Tier 0/1/2 | 문서(SSOT) | Performance §5·Architecture §3-4·RunFlow §2-1 | ✅반영(2026-07-05) |
| G2 | 〃 | **Tier 0 실행 프롬프트**(코어 인프라: 레벨 스트리밍·per-map 필드·allocator v0·NetCull·최소 진입시드·빈맵 드레인·문파괴 복제·전환 grace·stream-in fallback) | 코드(신규 유닛) | #3 Tier 0 = U 연속필드 구현·머지 `34b5eea` | ✅ 완료 |
...
| H1 | [Review/20260707-data-tooling-p0.md](Review/20260707-data-tooling-p0.md) | **P0 데이터 검증 시임 = `FPSRogueliteEditor` 에디터 모듈 신설**(Codex 5R 수렴): 앵커 교차검증기 3종(CardPool/RunSchedule/LoadoutPool, usecase 게이트)+per-asset sanity 보강+얇은 `FFPSRAnchoredValidationService`(reachable+orphan)+`ValidateAnchoredDataCommandlet`(NumInvalid>0 return1·앵커0=에러·orphan 경고)+`validate-data.ps1`+`Tools>FPSR>Validate Anchored Data` 메뉴명령. Slate/편집UI/CSV/자동수정 제외 | 코드(신규 모듈) | `phase/editor-tooling-seam` (백로그 "Batch DataAsset Validator" 정식구현) | ✅main 머지(`57270c5`) |
| H2 | 〃 | 🙋**라우팅 스펙 정합 선행**: 행동프래그먼트/feature-unlock(`UnlockableFeatures`)/무기-unlock(`WeaponUnlockCards`)/레벨업(`Cards`) surface를 코드(`FPSRCardSubsystem.cpp:131`)↔SSOT↔메모리 [[card-pool-routing]] 간 확정 → 이후에만 "라우팅 누수" 검증 추가(P0 제외). **P1 편집툴은 비차단**(자명누수=preflight 흡수, 애매부만 경고; H4 참조) | 🙋사용자결정+문서 | CombatWeaponCard §2-3-4 | 🙋결정대기 |
| H3 | 〃 | 미션 튜닝(ZoneRadius/HoldSeconds 등) BP CDO→DA-소유 통합 여부(단일편집+`ServerActivate(Data)` 주입). **미션 타임라인 *편집*(P2)의 선행**; P1 read-only 바 프리뷰는 무관 | 문서(SSOT)/백로그 | RunFlow §2-8 / ToolingBacklog | 파킹(P0 후) |
| H4 | [Review/20260707-data-tooling-p1.md](Review/20260707-data-tooling-p1.md) | **P1 편집 툴 = FPSR Data Editor**(Codex 5R 수렴): 단일 도킹 탭(`Tools>FPSR>Data Editor`, 기존 모듈)·anchor-agnostic 셸(서비스 앵커/고아 재사용+`IDetailsView`)·route-인지 preflight·카드 (효과단위)magnitude 그리드(존재티어 in-place)·미션 read-only 타임라인 바·`UFPSRCardEffect` WITH_EDITOR 시임(GridSummary/EligibleRoutes)·닫힌 enum `EFPSRCardRoute`·Save+Rescan(디스크기준 stale 명시)·헬퍼 라운드트립 테스트2. 무기매트릭스=Property Matrix 문서화(커스텀 미구현). 하이브리드 C++Slate. **공수 L, 2브랜치**(`phase/editor-tooling-p1-wiring`→`-magnitude`) | 코드(모듈 확장) | ToolingBacklog(Card Catalog Matrix·Run Schedule Timeline 갱신) | 🙋플랜 승인대기 |

> **✅ G1 SSOT 반영 완료 (2026-07-05)**: #3 다중맵 설계 수렴(Codex 5R 컨설트)을 Performance §5(다중맵 예산 모델)·Architecture §3-4(레벨 다중맵 심리스·allocator/레지스트리)·RunFlow §2-1(명제·Tier 구조)에 반영. Tier 0 = U 연속필드로 구현·머지 완료(`34b5eea`, `Architecture §4-1`). G5 디렉터 결정은 Tier 0 비블로킹(Tier 2 튜닝 방향).
