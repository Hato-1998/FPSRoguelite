...
> 작성 2026-09-03, 개정 2026-09-04. **새 세션이 그대로 복붙해 착수**하는 실행 프롬프트다. **브랜치 `proto/arcade-look` 은 2026-09-04 main 에 머지됐다(`037e681f`)** — 재개 시 main 에서 새 브랜치(`phase/<키워드>`)를 딴다.
...
§3-1 생성·§3-3 임포트·§3-2 WPN1 코드(e181d137)·§3-4 소켓은 끝났다. 2026-09-04 마감: 라이플 DA 는 원래 Synty 모델로 원복(§0-A).
재개 = §0-A 를 먼저 읽고, 전제(ADR 0015: 총이 1P 비주얼의 전부) 위에서 모델링 재작업 + §3-5 머티리얼을 한 묶음으로 계획한다.
```

### 0-A. 2026-09-04 마감 상태 — 재개하는 세션은 여기부터

사용자(2026-09-04): *"기존 원래 쓰던 모델링으로 바꿨어. 지금은 파츠도 저작 안 되어 있고 모델링 자체도 다시 만들어야 하니까 여기서 마무리해."*
...
| §3-4 소켓 | ✅ `ffa6d5ca` — 몸통 8 · 총열 Muzzle · 조준경 2종 Aim, `sockets_ok=True`. **+ 손 소켓 회전** `2fbc6af4`(Synty 회전 기저 변환 이식) → 데이터 경로 `6b0fbb2b`(생성기 `BODY_SOCKET_ROTATIONS` → manifest `body_socket_rotations` → 스크립트 add/upd·verify 회전 포함, 라운드트립 12/12). Troubleshooting F4 ⚠️ **ARM1(2026-09-04) 이후 양손 소켓은 런타임 미사용**(DA 소켓 None = 손 IK 퇴역). 에셋엔 보존 |
| §3-6 DA | ↩️ **원복(사용자, 2026-09-04)** — `DA_Weapon_Rifle` 은 다시 원래 스켈레탈 `SK_Wep_Mod_A_Body_01` + Synty 파츠 7(`WeaponMeshStatic=None`). HS 몸통 배선(`f75ecc06`)은 커밋 이력에만. 양손 그립 소켓 None(ADR 0015) |
| §3-7 PIE | ◐ **몸통 위치·방향 통과**(2026-09-04) → **WPN1 PIE #1: 파츠 부착 ✓ · 스코프 교체 ✓ · 손 방향 이상** = 몸통 손 소켓 회전 누락(코드 아님) → 수정 후 **재판정 대기**(방향 정상? 위치 오차 cm?). ⓑ 원격 클라·스왑 · ⓓ 왼손 몸통 소켓 · ⓔ ADS 정렬 · ⓕ 총구화염 · 로그 `:2480` 부재 · 대조군 4종 = 미판정(명세 §12-6·§13) **ARM1 병행**: 손만 표시(`MI_FPArms_HandsOnly`, 커밋 `90b91f20`) + IK 퇴역 → 재판정은 BP 배선 뒤(라이플·SMG 총 위치, 손목 절단, 왼손-핸드가드 간격) ↩️ **동결(2026-09-04 마감)** — 사용자: *"파츠도 저작 안 되어 있고 모델링 자체도 다시 만들어야 하니까 여기서 마무리"*. 재개 시 §0-A 참조 |
...
| `LeftHandSocket` / `RightHandSocket` | `SOCKET_LeftHand` / `SOCKET_RightHand` (몸통 메시에 실재 — 실측 확인) → **None(2026-09-04 사용자 결정, ARM1)** — 손 IK 퇴역. 몸통 소켓은 남아 있다 |
