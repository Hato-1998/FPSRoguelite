...
- **PM 보드 행 갱신**(`handoff` 스킬과 동형, `Workflow.md` §6-9): 행 페이지에 ① 현재까지 한 일 ② **남은 에디터 작업(구체적: 대상 에셋 경로 + 작성해 둔 스크립트 경로 + 매핑/의도)** ③ 블로커 ④ 미커밋 `Content/` 목록을 append하고 `담당`을 비운다.
...
- **새 세션 재개 프롬프트 출력**(복붙용, 아래 형식):
  ...
  ■ 먼저 보기: 보드 행 <URL> 인계 메모(§6-9 클레임) + Game.md(§0-1) + 관련 Docs/SSOT
...
## 참고
- 이 스킬은 "에디터 연결 시도"가 핵심 — 되면 실행, 안 되면 깔끔한 인계로 다음 세션이 즉시 재개하게 하는 것이 목적. 모델 정책(§6-5): 스크립트 작성=Sonnet 위임 가능하나 서버권위/파괴적 작업·검증은 Opus 직접. **코어·구조설계·리팩토링이 얽히면 §6-5-2**(플랜·지시·검증 = Fable).
