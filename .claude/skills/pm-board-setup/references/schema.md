...
🔴 **조회는 셋으로 가른다. 「전수 조회」는 쓰지 않는다** — 원형이 2026-09-09 에 폐기했다(§F-4).

**① 작업집합**(세션 시작 1회 — 하드 게이트가 읽는 전부)
```sql
SELECT "작업명","상태","우선순위","담당","주요경로",url
FROM "collection://<TASKS_DS>"
WHERE "상태" IN ('진행중','차단') OR "우선순위"='크리티컬'
```
크리티컬 확인과 병렬 충돌 검사에 필요한 것이 전부 여기 있다. 원형 실측 **2행**.

**② 표적**(무제한) — 착수할 행만. `notion-search` + `data_source_url`, 또는 `WHERE "작업명" LIKE '%키워드%'`.
행 본문 = `notion-fetch <행 URL>`. ⚠️ DB URL 을 fetch 하면 스키마·뷰만 나오고 행은 안 나온다.

**③ 대기열**("다음에 뭘 하지" 일 때만)
```sql
SELECT "작업명","우선순위",<!-- IF:MILESTONE -->"마일스톤","크기",<!-- /IF:MILESTONE -->"선행작업","주요경로",url
FROM "collection://<TASKS_DS>"
WHERE "상태"='대기'<!-- IF:MILESTONE --> AND "마일스톤" LIKE '%<현재 마일스톤 page ID>%'<!-- /IF:MILESTONE -->
ORDER BY "우선순위" LIMIT 20
```

### F-4. 왜 「전수 조회」를 버렸나 (원형 실사고 2026-09-09)

종전 레시피는 `WHERE "상태" IN ('진행중','대기','차단','결정대기')` 한 방이었다. 활성 행이 늘자 **SQL 이 100행에서 조용히 잘렸다**(`has_more: true`). `ORDER BY` 도 없어 **어느 행이 사라지는지 정해져 있지도 않았다.**

실측: 활성 114행 중 100행 반환. 잘린 꼬리에 **진행중 2건 중 1건**이 있었고, 그 행의 `주요경로` 에 해당하는 미커밋 파일이 그 시각 작업 트리에 있었다 — **병렬 충돌 검사가 뚫린 채로 돌고 있었다.**

- 📌 **설치기가 물려받아야 할 판정 기준: 조회 비용은 활성 행 수와 무관해야 한다.** ①은 *진행중·차단 행 수*에만 비례하므로 백로그가 몇 백 건이 되어도 커지지 않는다. 새 조회를 추가할 때 이 기준을 어기면 같은 사고가 재발한다.
- ⚠️ **어떤 조회든 `has_more` 를 확인하라.** `true` 면 결과는 불완전하다. 종전 프로토콜은 이 필드를 한 번도 언급하지 않았고, 그래서 잘림이 한 달 넘게 안 보였다.
- ⚠️ **보드 전체를 훑는 작업**(백필·감사·DAG 유지)은 위 셋을 기준 삼지 말 것 — 목적에 맞게 `상태` 필터를 다시 짜고 **`LIMIT/OFFSET` 으로 `has_more` 가 `false` 가 될 때까지** 페이지네이션한다.
- ⚠️ 무료 플랜은 SQL 호출 한도가 있다 — **세션당 1~2회**. 프로토콜 문서 (10) 의 로컬 스냅샷을 켜면 ①이 REST 로 빠져 이 예산은 ②·③ 전용이 된다.
- 🔒 `검증중`·`보류`·`완료` 를 조회에서 빼는 것은 의도다 — 사람이 확인해야 끝나는 것이라 에이전트가 집을 작업이 아니고, 넣으면 세션 시작 목록만 길어진다.
