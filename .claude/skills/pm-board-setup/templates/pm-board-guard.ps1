...
#
# 배선(공유되지 않음): .claude/settings.json 은 보통 .gitignore 대상(머신 로컬)이라 이 스크립트만 커밋된다.
#   새 클론에서 경계를 켜려면 settings.json 의 hooks.PreToolUse 에 이 스크립트를 물려야 한다 — 프로토콜 문서 (7).
#
# 드라이런:
...
상태·담당·브랜치·우선순위·완료커밋 변경은 출력의 '## 실행 필요(메인)' 에 도구 인자로 적어 메인 세션에 넘기세요. (프로토콜 문서 (7))
"@
