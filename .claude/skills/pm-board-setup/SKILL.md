---
name: pm-board-setup
description: Bootstrap a Notion-based PM task board ("작업 보드") for a project and wire the repo-side harness that goes with it — Notion hub page + task DB (14 columns, dual self-relation for 선행/후행, 7 views) + optional milestone DB, plus .mcp.json, the `pm-board` subagent, the `/board` slash command, the PreToolUse guard hook, the SessionStart reminder hook, a PROGRESS.md pointer, a protocol doc, and a CLAUDE.md hard-gate paragraph. Use whenever the user wants to set up, install, replicate, or migrate this task-board workflow into a new or existing project — Korean phrasings like PM 보드 만들기, 작업 보드 도입, 노션 보드 세팅, 보드 형태 옮기기, 클레임 게이트 도입. Also use when a project already has the board and the user asks to repair, audit, or extend the schema (add milestone axis, add a column), since references/schema.md holds the verified DDL and pitfalls. Do NOT use for day-to-day board operations (claim/close/audit) in a project that already has `/board` installed — that is the installed `/board` command's job.
---
...
3. 가드 훅 드라이런: `echo '{"agent_type":"pm-board","tool_input":{"command":"update_properties"}}' | powershell -NoProfile -File Scripts/pm-board-guard.ps1` → `permissionDecision: deny` JSON. `insert_content`로 바꾸면 출력 없이 종료.
4. 사용자에게 보고할 것: 허브 URL · DB URL · 생성 파일 목록 · **다음 세션에서 `/board`로 첫 조회를 해 보라**(에이전트 로드 확인) · settings.json은 클론마다 수동 배선 · 기존 작업 목록을 보드로 옮기는 것은 `/board`가 아니라 사용자 또는 이관 세션이 `notion-create-pages`로 한다(`출처=기존이관`).
5. 커밋은 사용자가 요청할 때만. 커밋 시 `.mcp.json`은 시크릿이 없으므로 포함해도 된다(OAuth는 사용자 레벨).
...
- **전수 백필** — 활성 조회 SQL(`검증중`·`보류`·`완료`·`폐기` 제외)을 기준으로 삼지 말 것. 목적에 맞게 `상태` 필터를 다시 짠다(원형에서 `검증중` 12행을 놓칠 뻔했다).
...
- **SQL 조회는 무료 플랜 한도가 있다**(세션당 1~2회). 표적 조회는 `notion-search`+`data_source_url`, 행 본문은 `notion-fetch` — 둘은 무제한.
