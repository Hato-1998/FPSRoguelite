...
7. **읽는 문서 층** — `references/workspace-structure.md` 를 따라 만든다. 순서: 분야 페이지 4개(§B) → 결정 기록 DB(§D) → 문서 DB(§C) → 아이디어·논의 DB(§E) → **릴레이션 배선(§F, DB 3개가 다 생긴 뒤 한 번에)**.
   ...
   - ⚠️ **작업 보드에는 새 열이 생기지 않아야 한다.** 결정·아이디어에서 작업으로 가는 관계는 **단방향**이다(§F-2). DUAL 로 만들면 보드에 아무도 안 보는 열이 늘어난다.
   - 🔒 **문서 ↔ 작업 보드 릴레이션은 만들지 않는다**(§F-1) — 원형이 만들었다가 **한 건도 안 채워진 채**(보드 252행·문서 41행 양방향 공백) 폐기했다. 릴레이션을 만들기 전에 **"누가, 언제 채우나"** 를 답할 것.
