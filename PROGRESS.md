...
- **P4-B-3 코드 완료(브랜치 `phase/p4b3-missions`, 미푸시·미머지)** — 미션 **6종** 코드 완료·검증(빌드+스모크 통과, 5종은 Codex 클린 다회 하드닝, LimitedVision은 머지게이트 대기):
  ...
  - **배치 C(2/2) LimitedVision 완료(2026-06-11, Opus 검증)**: `AFPSRMission_LimitedVision`(시야 극제한 N초 버티기) + 서버권위 복제 `AFPSRGameState::bVisionRestricted`(`bRunPaused` 미러) + `AFPSRCharacter` 로컬 카메라 PP(할당 시 `VisionRestrictionMaterial` 블렌더블, **미할당 시 내장 비네트 폴백**=콘텐츠 없이 PIE 가능). 바인딩=로컬게이트 없이 `OnRunStateChanged` 구독·적용만 `IsLocallyControlled()` 게이트(possession 타이밍 안전), 미션 종료/`EndPlay` 어느 경로든 시야 복구 보장. 빌드 Succeeded+스모크 Success. **PP 시각 효과는 사용자 PIE 검증**(헤드리스 불가). 설계근거 Game.MD §2-8.
  - **공용 PointSet 범용화 + MovingZone/CollectOrbs(2026-06-11, Opus 검증)**: 단일앵커+`RelativeWaypoints` 연속이동 폐지 → 공용 `AFPSRMissionPointSet`(**자식 Scene/Billboard 컴포넌트=월드 점 목록**, attach 순서, 비복제). 디렉터가 미션 **CDO 가상함수**(`UsesPointSet`/`AssignPointSet`)로 일반 선택·주입(미션별 cast 제거), `SelectMissionPointSet`(태그 가중랜덤, 개수 무관). **MovingZone**=점별 `RequiredHoldSeconds` 홀드→즉시 다음 점 텔레포트→전점 완료(셋 미배치 시 1점 폴백). **CollectOrbs**=각 월드 점에 오브 스폰(셋 미배치 시 `OrbRelativeLocations`/기본 오프셋 폴백). `AFPSRMovingZoneRoute`→`AFPSRMissionPointSet` rename(stale 0). 태그 `Mission.Spawn.MovingZone`/`Mission.Spawn.CollectOrbs`. 빌드 Succeeded+스모크 Success. **PIE는 사용자**(아래). 설계근거 Game.MD §2-8.
  - **⚠️ 다음(머지 전 필수)**: ① **콘텐츠**(사용자, 한 번에) — 미션별 BP 서브클래스(튜닝 디폴트: StandStill `RequiredStillSeconds`, **MovingZone/CollectOrbs=`AFPSRMissionPointSet` BP 배치[자식 Scene/Billboard 컴포넌트=점, MovingZone용 `PointSetTag=Mission.Spawn.MovingZone`·Orb용 `PointSetTag=Mission.Spawn.CollectOrbs`, `Weight`] + MovingZone BP `ZoneRadius`/점당 `RequiredHoldSeconds`, 미션 DA `SpawnPointTag`=해당 태그**, **LimitedVision `RequiredSeconds`+선택 `VisionRestrictionMaterial`** 등) + `UFPSRMissionDataAsset`(MissionClass=BP, MovingZone DA는 `SpawnPointTag=Mission.Spawn.MovingZone`) + `DA_RunSchedule` MissionEvents 등록 + (선택)스폰포인트/오브·타깃 BP 메시·고체력 MaxHealth + **(선택)캐릭터 BP에 시야제한 PP머티리얼 할당**. ② **PIE 검증**(`FPSR.MissionTrigger [index]`로 각 미션 → 진행/완료/보상프리즈, 프리즈 중 미진행 확인, MovingZone=한 route 시작·점 순차전환·전점 완료, LimitedVision=비네트 적용·완료 시 복구, `FPSR.MissionClear`). ③ **Codex 머지게이트** `codex-review.ps1 -Base main` 일괄. ④ `--no-ff` main 머지(§6-7).
...
- **스케줄러 = 시간 윈도우+미션 풀 랜덤(2026-06-11, 코드 완료, 브랜치 `phase/p4b3-missions`)**: `FFPSRMissionEvent{TriggerTime,Mission}` → `FFPSRMissionWindow{MinTime,MaxTime,MissionPool[]}`. 런 시작 시 범위 내 랜덤 트리거 시각 롤 + 발화 시 풀 균등 랜덤 1개. 디버그 `FPSR.MissionTrigger [windowIndex] [poolIndex]`. 빌드+스모크 통과. 콘텐츠=`DA_RunSchedule` 윈도우/풀 재작성(사용자). Game.MD §2-8.
