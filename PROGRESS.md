...
- **P4-B-3 코드 완료(브랜치 `phase/p4b3-missions`, 미푸시·미머지)** — 미션 **6종** 코드 완료·검증(빌드+스모크 통과, 5종은 Codex 클린 다회 하드닝, LimitedVision은 머지게이트 대기):
  ...
  - **⚠️ 다음(머지 전 필수)**: ① **콘텐츠**(사용자, 한 번에) — 미션별 BP 서브클래스(튜닝 디폴트: StandStill `RequiredStillSeconds`, MovingZone `RelativeWaypoints`, **LimitedVision `RequiredSeconds`+선택 `VisionRestrictionMaterial`** 등) + `UFPSRMissionDataAsset`(MissionClass=BP) + `DA_RunSchedule` MissionEvents 등록 + (선택)스폰포인트/오브·타깃 BP 메시·고체력 MaxHealth + **(선택)캐릭터 BP에 시야제한 PP머티리얼 할당**. ② **PIE 검증**(`FPSR.MissionTrigger [index]`로 각 미션 → 진행/완료/보상프리즈, 프리즈 중 미진행 확인, LimitedVision=비네트 적용·완료 시 복구, `FPSR.MissionClear`). ③ **Codex 머지게이트** `codex-review.ps1 -Base main` 일괄. ④ `--no-ff` main 머지(§6-7).
