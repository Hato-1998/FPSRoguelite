...
**1) `Game.MD` 작성 (미작성, 최우선)** — 프로젝트 전체를 한 문서로:
   - 기획/게임디자인(장르·루프·카드·무기·적·미션·메타), 프로그래밍 구조(모듈·클래스·GAS·네트워크·컴포넌트), 작업 방식/규칙(프로덕션 원칙·핸드오프·모델정책), 현재 구현 상태, 방향성/로드맵
   - 작성 전 `DESIGN.md`·`PROGRESS.md`·`CLAUDE.md`·`AGENTS.md`·`git log --oneline`·`Source/` 전수 파악
