...
## 한 줄 요약
**P0~P3-D → main 머지 완료**. **P4-A(재설계: 라운드제 폐지 → 레벨업 전역 프리즈 + 시간 미션 + 보스) 코드 완료** — 브랜치 `phase/p4a-runflow-missions`에서 빌드+헤드리스 스모크+Codex(교정 포함) 통과. **레벨업 시 즉시 전역 프리즈(적·플레이어 정지)→전원 카드 선택→재개**(예전 방식 복귀, Game.MD §2-2 반전). → 다음 **사용자 콘텐츠(스케줄 DA 재설정·미션·적 BP) + PIE 검증 → `--no-ff` main 머지**, 이후 **P4-B**(무기 모디파이어 Fragment + weapon-scope 카드 실적용).
...
- **P4-A 코드 완료(재설계)**: **레벨업 전역 프리즈** — `AFPSRGameState.bRunPaused`(복제, 페이즈독립) + `RefreshPauseState`(전원 보류픽 기준 프리즈/재개+오퍼 제시), `AddSharedXP` 레벨업 즉시 프리즈. `ERunPhase`=Combat/Boss(Breather 제거). **프리즈 게이팅**: 스폰 스킵·적 이동/공격 동결(제자리)·플레이어 입력/속도 동결·발사GA 게이트. **오퍼 일반화** `EFPSROfferType{OpeningSeed,LevelUp,MissionReward}`(P3-D 보안경로 유지) + `AFPSRPlayerState::MissionRewardPicksPending` + `ApplyCard` 타입별(weapon-scope 수락·소비, GE적용 P4-B). **디렉터**(`UFPSRRunDirectorSubsystem`): 라운드 제거 → 런클럭+시간 미션스케줄(`FFPSRMissionEvent`)+`BossTime`+시간 스폰스케일링, 오프닝홀드(스폰지연), 보스 미션 유실 방지. **미션 프레임워크**(`AFPSRMissionActor`+`UFPSRMissionDataAsset`+레퍼런스 `AFPSRMission_HoldZone`+`AFPSRMissionSpawnPoint`) 재사용, 클리어→즉시 프리즈 보상. 적 클래스 설정화(`BP_Enemy` via GameMode). 디버그: AddXP/RunTimeScale/SkipToBoss/MissionTrigger/MissionClear/Pause/KillAllEnemies/RunDebug. Game.MD §2-1/2-2/2-7/2-8/4-1 재설계 반영.
- **다음 단계**: ① **사용자 콘텐츠** — [`Docs/P4-A_UserContent_Guide.md`](Docs/P4-A_UserContent_Guide.md) 따라 **`DA_RunSchedule` 재설정**(MissionEvents[]+BossTime — 기존 Rounds 제거됨) + `DA/BP_Mission_HoldZone`(+존 데칼) + `BP_Enemy`(XP) + `BP_FPSRGameMode`에 RunSchedule/EnemyClass 할당. ② **PIE 검증**(오프닝2장 동결→전투→레벨업 즉시 프리즈→카드→재개→미션클리어 프리즈보상→보스). ③ 통과 시 `--no-ff` main 머지(§6-7) + 브랜치 정리.
- **⚠️ 임시 테스트값(프로덕션 전환 시 노티·원복)**: 미션 60/120/180s·보스 300s(프로덕션 5/10/15분·보스 20분). 메모리 `p4a-temp-test-values`. (XP는 프로덕션 공식: `XPPerLevel=50`/적 `XPReward=5`, 사용자 튜닝)
