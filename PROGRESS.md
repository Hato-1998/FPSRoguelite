...
> 게임 루프를 "메뉴 Play → **로비 허브** → 인게임 → (패배/디버그) → **로비 복귀** → 런 리셋"의 멀티 루프로 닫음. 백로그 D5(세션)+신규 로비/Seamless 트래블 통합. 보스(U3/U4) 선행 아님 — 패배·디버그 트리거로 복귀 검증, 보스 승리 자동복귀는 U11b. 플랜: `Docs/P7-MultiplayerLoop_Plan.md`. **확정(사용자 2026-06-16)**: Play→항상 로비(솔로도 1인 로비), 결과창 복귀처=로비, 세션=Steam만(app id 480, Null/LAN 폴백 없음), **로비에서 각자 무기 1정 선택=유일한 총기**.
