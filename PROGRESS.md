...
- **⚠️ 임시 테스트값(프로덕션 전환 시 노티·원복)**: 라운드 2/2/1분(프로덕션 5/10/15분). 메모리 `p4a-temp-test-values`. (XP는 프로덕션 공식 원복 완료: `XPPerLevel=50`/적 `XPReward=5`, 사용자 튜닝)
- **P4 잔여(이월)**: **P4-B** 무기 모디파이어 Fragment + `ApplyCard` weapon-scope 활성(현재 reject) + 미션 보상 카드 Breather 선택·적용(P3-D 오퍼 모드 enum 일반화) + 나머지 6종 미션. **P4-C** 무기 7종. **P4-D** 게임필(히트마커/핑/위협 인디케이터/사각 오디오) + PickupRadius/XPGain + **라운드 타이머/미션/보상 HUD 위젯**(데이터는 GameState에 복제됨: `GetRoundTimeRemaining`/`GetCurrentRound`/`GetBankedMissionRewards`/`GetRunClockSeconds` → WBP 바인딩만 남음).
