...
4. ~~**`BS_Wall_Vertical`** 1D BlendSpace~~ **폐기**(사용자 결정 2026-08-02) — **벽 관련은 `wall_slip` 애니 하나로 통일**한다. 매달리기·등반·미끄러짐이 전부 같은 그림이 되고, `wall_topout`의 "넘어가는 순간"도 자세로는 안 보인다(코드의 `WallTopBoostForward/Up`이 캐릭터를 밀어주긴 함). 최대 1.5초(`WallHangMaxDuration`)짜리 동작이라 감수.
   - ⚠️ **`wall_slip`은 몸이 벽에 대해 86° 옆으로 선 포즈**다(어깨선 실측 +86.2°, 나머지 벽 3포즈는 0°). 이건 **의도된 것**(사용자 확정 2026-08-02).

### 🔧 3b 코드에 추가 결정 — 루트 요를 벽 법선에 맞추기 (사용자 승인 2026-08-02)
