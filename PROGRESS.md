...
## 🔴 핸드오프 (2026-06-18, 브랜치 `phase/p7-mp-loop-lobby`) — 인게임 이동 블로커 미해결
> **이번 세션 한 일**: ① WBP_Lobby 버튼 OnClicked 11개 배선(클릭 무반응 버그 수정·콘텐츠) ② 로비 무기 선택 골드 하이라이트(콘텐츠) ③ **코드**: 트래블 입력/무기 수정 — `net.AllowPIESeamlessTravel=1`(DefaultEngine.ini) + `AFPSRPlayerController` IMC를 `ApplyDefaultMappingContext()` 헬퍼로 추출 후 `SetupInputComponent`/`OnPossess`/`AcknowledgePossession`/(폰)`AFPSRCharacter::SetupPlayerInputComponent`에서 적용 + 진단 Log. **풀빌드 Succeeded.**
> **🔴 미해결 블로커 — 로비→런 트래블 후 인게임 이동/조작 안 됨**: 로그로 확인 = IMC가 4개 훅 전부에서 정상 적용 + 액션 바인딩 실행 + 프리즈/사망 게이트 해제 + 입력모드 Game + 뷰포트 포커스 = **입력 셋업은 완벽**. 그런데 이동키가 **엔진 Gameplay Debugger(`DebugManager.CycleToPreviousColumn`)로 소비**됨 → 캐릭터에 안 닿음. standalone 직접 PIE는 정상, 리슨서버(로비 경유)만 실패. (무기는 seamless로 해결됨 — 선택 무기 장착 확인.)
> **유력 가설(미검증)**: 사용자 PC의 **심레이싱 컨트롤러/휠(CAMMUS·vJoy·페달·G HUB)** 축이 떠서 그 게임패드 입력이 Gameplay Debugger를 띄우고 키 입력을 가로챔. **다음 세션 1순위: 컨트롤러 분리하고 PIE 이동 테스트.**
> **다음 코드 작업(구체)**: ① 컨트롤러 분리로 재현되면 → Gameplay Debugger 비활성(`.uproject` GameplayDebugger 플러그인 Disable 또는 `[/Script/GameplayDebugger.GameplayDebuggerConfig]` 입력 키 해제) 또는 입력 디바이스 필터. ② 안 고쳐지면 → `AFPSRCharacter::Input_MoveForward`/`Input_Look`에 진단 Log 추가(빌드 1회)해 WASD/마우스가 핸들러에 도달하는지 디스크 로그(`Saved/Logs/FPSRoguelite.log`)로 확정 → 도달X면 CommonUI/포커스/입력우선순위, 도달O면 다운스트림. ③ 이동 해결 후 **진단 Log 정리**(`ApplyDefaultMappingContext` caller Log → Verbose 환원, `Char::SetupPlayerInputComponent ran` Log 제거).
> **검증 게이트**: 풀빌드 `Result: Succeeded`(컴파일 OK). 헤드리스 스모크는 이번 세션 미재실행. 이동 미동작은 **코드 컴파일 문제 아님**(별개 입력 라우팅 블로커).
> **미커밋 콘텐츠(사용자 작업으로 남김)**: `WBP_Lobby`(버튼 배선+하이라이트), `L_Lobby`/`L_Transition`, `BP_FPSRLobbyGameMode`/`BP_FPSRLobbyPC`/`BP_LobbyDisplayPawn`, `DA_LoadoutPool`, `IMC_*`, `Content/Input/Actions/` 등 (`git status` Content/). **이 config도 미커밋 유지**: `DefaultGame.ini`/`DefaultInput.ini`/`DefaultEditor.ini`(콘텐츠 에셋 참조라 콘텐츠와 함께 묶음).

## 🗂️ 마스터 작업 분해 — **`Docs/TaskPrompts_Master.md` (2026-06-13 신설, 최상위 계획)**
> 남은 로드맵 전체를 의존성 기준 유닛으로 분해 + 유닛별 새 세션 복붙 실행 프롬프트. **순서: ✅V0·V1·V3·U16·U2 완료 → [2-클론 병렬, 2026-06-16] 클론 A: U3a(약점)→U3(보스 스캐폴드)→U4(보스 콘텐츠) ‖ 클론 B: U11a(로비/세션/트래블 루프, Play→로비 허브·Steam·보스 제외) → 합류 → W1(전체 검증) → U1(게이트)/V2 → 2차 트랙(U5~U15·U17) → U11b(보스-승리 멀티 통합+E2E)**. 병렬은 2026-06-15 폐지했으나 U11a는 파일 도메인 분리+충돌점 GameMode 1파일 근거로 한정 부활(사용자 결정). U15=무기 애님(U1 후 조건부).
