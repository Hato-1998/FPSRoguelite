...
0. 🔴 **1인칭 뷰 v1 은 커밋됐지만 에디터 검증을 안 했다.** 먼저 켜서 읽어온 카메라↔팔 상대값을
   실측 기대치 `(-21, 10, -165)`/yaw −95 · FOV 90 과 대조할 것(그 기대치는 **옛 NeonV 팔 기준**이라
   지금 팔과 다를 수 있고, 다르면 그 자체가 발견이다).
   그다음 **별도 도킹 탭 + 화면비 고정**으로 교체 — 사용자 요청이고 설계는 인계문서 §3 에 확정돼 있다
   (핵심: `bUseControllingActorViewInfo` + `ControllingActorViewInfo.bConstrainAspectRatio`,
   `EditorViewportClient.cpp:1158`. FHD·QHD 는 둘 다 16:9 라 구도가 같다 — 프리셋은 종횡비로 묶을 것).

1. ~~**어셈블러에 1인칭 뷰 + 조준 기준선**~~ ← v1 완료(미검증). 원 요청 맥락은 아래 유지.
