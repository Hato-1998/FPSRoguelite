...
> **검증(코드측, Opus 직접)**: 빌드 Succeeded(증분 20s) + 헤드리스 스모크 `Result={Success}`(OnlineSubsystem/Steam/Utils 플러그인 마운트·Steam SDK 로드 확인; 헤드리스라 Steam 인스턴스는 NULL 폴백 — 정상, 모듈로드 보장). IOnlineSession/CopyProperties/Online::GetSubsystem 시그니처 UE5.7 소스 대조. **Codex 머지게이트=샌드박스 실패**(rg 미존재+git "blocked by policy"+dubious ownership — diff 미수집, V1과 동일 이슈) → 5분 워치독 정책으로 스킵, 코드측은 Opus 직접(빌드+스모크+엔진소스대조+self-review). [[codex-gate-5min-watchdog]]
