...
- ⚠️ **먼저 확인할 것: Blu 스켈레톤에 라이플 애니가 이미 한 벌 있다.** `Content/Characters/Blu/Anims/` 103개 — `Blu_MF_Rifle_*` 17개(8방향 walk/jog + Idle_ADS) · `Blu_MM_Rifle_*`(Fire/DryFire/Reload/Equip/Jump/ADS 에임포즈) · **`Blu_AO_Rifle` 에임오프셋 완성품**. `d0150852`/`216e7cca`에서 마네킹→Blu 리타게팅 완료(손가락·쇄골 체인 포함). **힙에서 총이 안 보이는 것도 이것 때문**이다 — 바디가 `Blu_ABP_Unarmed`(로코모션 전용, 무기 포즈 없음)를 재생해 팔이 옆으로 내려가 있다. 4단계는 이 에셋들을 AnimBP에 무는 것부터가 최단 경로일 수 있다
  - 리타게터가 **둘**이 되었다(`RTG_Manny_to_Blu` = Blu 팩 계열 97개 산출 / `RTG_UE4Man_to_Blu` = ADR 0002 스파이크, `_Spike/` 4개). 3단계에서 `Rifle_01`(774개, 제자리회전·8방향 시작 등 팩이 더 넓다)을 마저 옮길지, 있는 것으로 갈지부터 정할 것
