...
> **🟢 다음(U11a 마무리)**: ① **풀 루프 PIE 재검증**(로비→Ready→트래블→플레이→사망/`FPSR.EndRun defeat`→로비 복귀→재Ready→재트래블) — 이동 블로커 해소로 처음 끝까지 가능 ② 남은 로비 콘텐츠 폴리시(가이드 §3: 플레이어목록 ForEach·포디움 무기 표시) ③ 2-PC Steam E2E → `Scripts/codex-review.ps1 -Base main` → `--no-ff` 머지.
