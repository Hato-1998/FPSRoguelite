...
## 🎯 U3a 약점 부위 데미지 (헤드샷/디자이너 지정 존) — **코드 완료(2026-06-16, 브랜치 `phase/p6-weakpoint-damage`). 빌드+스모크 통과 / 사용자 PIE+콘텐츠 후 `--no-ff` 머지 대기**
...
> **검증**: 빌드 Succeeded + 헤드리스 스모크(ModuleLoads) `Result={Success}`. Codex 플랜게이트 P1(AOE 무회귀=전용채널로 해소)·머지게이트 P2(비활성 약점 sphere 스킵, `accf279` 교정) 반영. **남은 검증=사용자 PIE**(약점 컴포넌트 배치+마커 Weak 틴트 필요 → 가이드 §1~3): 약점=추가뎀+구분마커 / 본체=현행동일 / 관통 약점+본체 겹침 1회 / AOE 무반응 / 스웜 다수 무회귀. → 통과 시 `--no-ff` 머지. **U3 보스가 이 인프라 소비.**
