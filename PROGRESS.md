...
| 2 | A3 AOE+ChargeLaser **(분할)** | `phase/p4c-aoe-charge` | **A3a AOE=✅완료·코드 커밋**(`UFPSRGA_WeaponFire_Projectile`+`OnProjectileSpawn`훅+투사체 스탯, 서버권위 AcquireProjectile). **A3b ChargeLaser=대기**(차징 cadence+서버권위 차징 RPC+`ModifyChargeTime`+차징량 비례 데미지, 레이저=히트스캔 §2-10). 클라예측 cosmetic 스폰=후속 | 무기 DA/BP | A1✅ |
