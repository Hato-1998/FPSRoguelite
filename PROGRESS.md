...
## 한 줄 요약
**P0~P4-A + P4-B-1 → main 머지 완료**. **P4-B-2(무기 행동 Fragment 시스템)** = `phase/p4b2-weapon-fragment` 브랜치에 **구현+빌드+스모크+Codex(이슈없음) 통과**(2026-06-05), **origin 푸시 완료, PIE 검증·콘텐츠·머지 대기**(다른 컴퓨터에서 이어받기). 합성형 발사 훅(`UFPSRWeaponFragment` 무상태 DataAsset) + 미션 클리어 시 장착 무기 `AvailableModifiers`에서 Fragment 카드 택1. → 다음 **P4-B-3**(미션 6종).

## ▶▶ 새 세션 우선 작업 = P4-B-2 마무리 (PIE 검증 + 콘텐츠 + main 머지)
- **브랜치**: `phase/p4b2-weapon-fragment`(구현 커밋·origin 푸시 완료). **다른 컴퓨터에서**: `git fetch && git checkout phase/p4b2-weapon-fragment`로 이어받기. **아직 main 머지 금지** — 콘텐츠 작성 + PIE 검증 후 `--no-ff` 머지(§6-7).
- **P4-B-2 구현 완료(브랜치)**: `UFPSRWeaponFragment`(UPrimaryDataAsset 무상태 behavior+설정, 훅=virtual `PreFire/ModifyShotCount/OnHitActor/PostFire`) + 레퍼런스 2종(`UFPSRFragment_MultiShot{ExtraShots}`, `UFPSRFragment_OnHitBonusDamage{BonusDamage}`) / `FFPSRFireContext`(plain struct) / `UFPSRWeaponInstance.ActiveFragments[]`(복제 참조)+`AddFragment/HasFragment` / `GA_WeaponFire_Hitscan` 훅 배선(컨텍스트→PreFire→ModifyShotCount=**NumShots 루프**→히트당 OnHitActor→PostFire, 탄약 활성화당 1발) / `UFPSRCardDataAsset.GrantedFragment`(ThisWeapon→AddFragment) / `UFPSRWeaponDataAsset.AvailableModifiers[]` / `UFPSRCardSubsystem::DrawWeaponModifierOffer`(장착 무기 미보유 Fragment 최대 3장 셔플) + ApplyCard fragment 분기 / `PlayerController::RequestCardOffer(MissionReward)` 무기소싱(오버라이드 카드 없을 때) + `GrantMissionReward(null)` 픽 부여 / 디버그 `FPSR.GrantMissionRewardPick`.
  - **⚠️ 콘텐츠(사용자, 머지 전 필수)**: ① Fragment DataAsset(Data Asset → 클래스 `FPSRFragment_MultiShot` ExtraShots=1 / `FPSRFragment_OnHitBonusDamage` BonusDamage). ② Fragment 카드 `UFPSRCardDataAsset`(`Scope=ThisWeapon`, `GrantedFragment`=Fragment DA, `RarityTiers` 1개[Magnitude 미사용]). ③ 무기 DA `AvailableModifiers`에 Fragment 카드 등록.
  - **⚠️ PIE 검증(헤드리스 불가)**: `FPSR.GrantMissionRewardPick`→프리즈+무기 Fragment 카드 출현→MultiShot 택1→발사 시 **디버그 트레이스 N개**(멀티샷) / OnHitBonusDamage→히트 데미지↑ / 디듀프(보유 Fragment 재제시 안 됨) / 2인 PIE 복제(클라 N샷 예측).
  - **설계 메모**: ThisWeapon=현재 무기(프리즈 중 equip 차단으로 결정적). AllWeapons는 스탯 전용(Fragment는 ThisWeapon만). 미션 DA `RewardCard`는 선택적 오버라이드(설정 시 그 카드, 미설정 시 무기소싱). ModifyChargeTime/OnProjectileSpawn 훅·Melee Fragment·Fragment 제거/교체는 후속.
