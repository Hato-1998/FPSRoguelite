 ...
 *
 *  IMPORTANT: every PERSISTED field must carry the `SaveGame` UPROPERTY specifier — the SaveGame archive
 *  (ArIsSaveGame) serializes only SaveGame-flagged properties, so an unflagged field silently reloads from its
 *  class default instead of from disk. P0-③ fields must follow suit. */
UCLASS()
...
public:
	...
	/** Schema version this instance was written at (drives MigrateIfNeeded). SaveGame-flagged so it persists. */
	UPROPERTY(SaveGame)
	int32 SaveVersion = CurrentSaveVersion;

	/** Neutral placeholder — exercises serialization round-trip + migration only, NO meta meaning. P0-③ replaces
	 *  the real schema. Do not read this as gameplay state. SaveGame-flagged so the round-trip actually persists. */
	UPROPERTY(SaveGame)
	int64 Reserved0 = 0;
