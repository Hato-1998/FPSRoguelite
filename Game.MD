...
- **구현 상태(P4-B-3, 2026-06-08 / LimitedVision 2026-06-11)**: 미션 종류 **6종** 추가(코드 완료, 콘텐츠+PIE 대기 — 각 자체완결로 레벨 배치 없이 `FPSR.MissionTrigger [index]`로 테스트 가능):
  - `AFPSRMission_StandStill`(전원 정지 N초), `AFPSRMission_MovingZone`(**Route 세트 순차 점령** — 아래), `AFPSRMission_CollectOrbs`(자체 스폰 오브 N개 수집), `AFPSRMission_CarryNoHit`(오브 소지 무피격 N초 — 캐리어 Health 폴링), `AFPSRMission_DefeatFleeing`(독립 고체력 도망 타깃 `AFPSRMissionFleeTarget` 처치), `AFPSRMission_LimitedVision`(시야 극제한 N초 버티기).
  - 공유 인프라: `AFPSRMissionOrb`(서버 오버랩 수령·이동 복제 액터). 스폰 액터 정리는 **`EndPlay`**(직접 파괴 시 고아 방지).
  - **MovingZone Route 세트 순차 점령(2026-06-11 재설계)**: 디자이너 배치 `AFPSRMovingZoneRoute`(**자식 Scene 컴포넌트 = 순서 있는 점령 마커**, attach 순서=점령 순서; `RouteTag`/`Weight`/`MinPlayerDistance`, 비복제 — 스폰포인트 패턴). BP에서 Scene(또는 Billboard) 컴포넌트를 점마다 추가·뷰포트 드래그. 디렉터 `SelectMovingZoneRoute`(태그매칭+가중랜덤, `Mission.Spawn.MovingZone`)로 레벨의 **route 중 1개 선택**(개수 무관 — 0개면 폴백, N개면 가중랜덤; 각 route의 점 개수도 무관)→`SetRoute`. 미션은 점0부터 **점별 `RequiredHoldSeconds` 홀드→점령 시 즉시 다음 점 텔레포트**(존 이동 없음, `SetActorLocation` 복제), 전 점 점령 시 완료. 진행도=(점령수+현점 홀드비율)/총점. Route 미배치 시 **스폰 위치 1점 폴백**(테스트 가능). 기존 연속이동+`RelativeWaypoints` 폐지. **제1원리**: 세트=한 액터의 자식 컴포넌트 마커 배열(뷰포트 드래그·1액터=전체 세트 자기완결), 서버권위 비복제 데이터.
