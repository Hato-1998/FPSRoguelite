...
- **구현 상태(P4-B-3, 2026-06-08 / LimitedVision 2026-06-11)**: 미션 종류 **6종** 추가(코드 완료, 콘텐츠+PIE 대기 — 각 자체완결로 레벨 배치 없이 `FPSR.MissionTrigger [index]`로 테스트 가능):
  - `AFPSRMission_StandStill`(전원 정지 N초), `AFPSRMission_MovingZone`(**PointSet 순차 점령** — 아래), `AFPSRMission_CollectOrbs`(PointSet/자체 스폰 오브 수집), `AFPSRMission_CarryNoHit`(오브 소지 무피격 N초 — 캐리어 Health 폴링), `AFPSRMission_DefeatFleeing`(독립 고체력 도망 타깃 `AFPSRMissionFleeTarget` 처치), `AFPSRMission_LimitedVision`(시야 극제한 N초 버티기).
  - 공유 인프라: `AFPSRMissionOrb`(서버 오버랩 수령·이동 복제 액터). 스폰 액터 정리는 **`EndPlay`**(직접 파괴 시 고아 방지).
  - **공용 `AFPSRMissionPointSet`(2026-06-11 범용화)**: 디자이너 배치 액터, **자식 Scene/Billboard 컴포넌트 = 월드 점 목록**(attach 순서=순서). `PointSetTag`/`Weight`/`MinPlayerDistance`/`bEnabled`, 비복제 서버권위(스폰포인트 패턴). `GetWorldPoints`/`GetFirstPointTransform`. **MovingZone·CollectOrbs 공용** — 점 목록을 미션이 알아서 소비(순회 vs 각 점 스폰). 디렉터는 미션 **CDO 가상함수**(`UsesPointSet`/`AssignPointSet`)로 일반 선택·주입(미션별 cast 없음): `SelectMissionPointSet`(태그매칭+가중랜덤, 개수 무관, 0개면 미션 폴백)→스폰 전 셋 결정·`AssignPointSet`.
  - **MovingZone(PointSet 순차 점령)**: `AssignPointSet` 셋의 점들을 점0부터 **점별 `RequiredHoldSeconds` 홀드→점령 시 즉시 다음 점 텔레포트**(존 이동 없음, `SetActorLocation` 복제), 전 점 점령 시 완료. 진행도=(점령수+현점 홀드비율)/총점. 셋 미배치 시 **스폰 위치 1점 폴백**. 기존 단일앵커+`RelativeWaypoints` 연속이동 폐지.
  - **CollectOrbs(PointSet 스폰)**: 셋 있으면 **각 월드 점에 오브 스폰**, 없으면 `OrbRelativeLocations`(또는 기본 테스트셋) `Origin+오프셋` 폴백. 전부 수집 시 완료(기존 로직 유지).
  - **베이스 강화**: `AFPSRMissionActor::Tick`이 `bRunPaused`(카드 프리즈) 중 미션 진행/시간제한 게이트(§2-2 정합, 전 미션 일괄). 디버그 `FPSR.MissionTrigger [index]`.
  - **LimitedVision 시야 제한(2026-06-11)**: 서버권위 복제 `AFPSRGameState::bVisionRestricted`(`bRunPaused` 미러). 각 로컬 클라가 `OnRunStateChanged` 구독→자기 `FirstPersonCamera` PostProcess 적용(할당 시 `VisionRestrictionMaterial` 블렌더블, 미할당 시 **내장 비네트 폴백**=콘텐츠 없이 PIE 가능). cosmetic·게임플레이 중립. 미션 시작=set true, 완료/실패/`EndPlay` 어느 경로든 set false(시야 복구 보장). **제1원리**: 전원 동시 cosmetic→GameState bool 1개가 per-character 복제보다 단순(카메라 PP는 로컬 적용).
- **보스 실물**은 후속(P6).
- **스케줄 = 시간 윈도우 + 미션 풀 랜덤(2026-06-11 재설계)**: `DA_RunSchedule.MissionWindows`(`FFPSRMissionWindow{MinTime, MaxTime, MissionPool[]}`). 각 윈도우 = 런 시작 시 `[MinTime,MaxTime]` 내 **랜덤 트리거 시각 롤**(서버권위, 런마다 변주) + 발화 시 `MissionPool`에서 **균등 랜덤 1개** 스폰. 윈도우별 풀로 시간대 제한(예: 초반 풀에서 HoldZone 제외). 빈 풀=스킵·경고. 디버그 `FPSR.MissionTrigger [windowIndex] [poolIndex]`(poolIndex 생략=랜덤). 기존 `{TriggerTime, Mission}` 단건 폐지.
