...
- **구현 상태(P4-B-3, 2026-06-08 / LimitedVision 2026-06-11)**: 미션 종류 **6종** 추가(코드 완료, 콘텐츠+PIE 대기 — 각 자체완결로 레벨 배치 없이 `FPSR.MissionTrigger [index]`로 테스트 가능):
  - `AFPSRMission_StandStill`(전원 정지 N초), `AFPSRMission_MovingZone`(웨이포인트 이동 점령), `AFPSRMission_CollectOrbs`(자체 스폰 오브 N개 수집), `AFPSRMission_CarryNoHit`(오브 소지 무피격 N초 — 캐리어 Health 폴링), `AFPSRMission_DefeatFleeing`(독립 고체력 도망 타깃 `AFPSRMissionFleeTarget` 처치), `AFPSRMission_LimitedVision`(시야 극제한 N초 버티기).
